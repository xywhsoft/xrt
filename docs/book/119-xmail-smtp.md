...
`DataBegin/DataWrite/DataEnd` 接受任意分块：内部增量状态机**跨片段保留 CRLF 与行首状态**、直接发送 dot-transparent 片段——**不创建整报文副本**（大附件流式的发送侧）。`xrtSmtpClientData` 连续输入便利入口：先完整验证 CRLF 再委托增量状态机。**调用方仍负责** MIME 字段、传输编码与服务器 SIZE 限制——客户端管线路不管内容（内容归第 116 章模块）。
...
基础客户端明文；`smtp_client_tls` 层加两形态：`XMAIL_SECURITY_TLS`（隐式——banner 前完成握手，465 端口形态）；`XMAIL_SECURITY_STARTTLS`（升级——EHLO 声明 STARTTLS、验证 `220` 切换、原流上接管 TLS、**握手后重发 EHLO**）。两条硬规则：**升级不沿用升级前的能力快照**（中间人可能剥能力——重协商才是真相）；**必须提供验证器**——不静默跳过证书验证（第 85 章"不验证不是选项"的邮件版）。
