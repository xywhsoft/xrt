...
| 129 | 129-build.md | practice | 4,087 | 2 | 1 | 3 | 3 | P24 |
| 130 | 130-trim.md | practice | 4,168 | 2 | 2 | 3 | 3 | P24 |
| 131 | 131-package.md | practice | 4,017 | 2 | 2 | 3 | 3 | P24 |
| 132 | 132-testing.md | practice | 4,038 | 2 | 2 | 3 | 3 | P24 |
| 133 | 133-test-oom.md | practice | 4,166 | 2 | 1 | 3 | 3 | P24 |
| 134 | 134-embed.md | practice | 4,114 | 2 | 1 | 3 | 3 | P24 |
| 135 | 135-perf-analysis.md | practice | 4,216 | 2 | 1 | 3 | 3 | P24 |
...
- 卷十三项目上已完成的 2 章：ch137 项目一日志统计工具（project 型
  ...
  测试四象限/交付形态/七条可迁移经验——项目章形态的首个范本）、
  ch138 配置中心设计（新增章 concept 型：六 UC+六验收/数据模型三选一
  （value 装载域+结构体快照域）/七层管线/原子交接协议（不可变快照+
  引用计数+原子指针）/校验三层与错误路径化/模板注入安全边界/
  设计文档形态（决策五要素）——项目二的设计前置）。
- 全书第二十四次重编号：双插入 config-design(138)/chat-design(140)，
...
- 门禁拦下：XRT_STR_C_LIT 不存在（改 strlen 视图构造）、xrtStrBufData
  ...
  项目章的额外内容以 h3 挂靠概念/示例节）、io/line 等 term 与示例
  头注释逐字对齐。

## P24 阶段记录

- 卷十二工程实践 8 章（卷十二全部收官）：ch129 构建（modules.json
  单一事实源/双轨编译/指纹缓存/断点续跑/扩展叠加）、ch130 裁剪（正向
  选择闭包展开/ALL+EXCLUDE 语义/粒度三问/体积档案符号禁入）、ch131 打包
  （产物矩阵/verify 消费者端到端/amalgamate 拓扑合成与一致性检查）、
  ch132 测试（四武器：正向/线程/变异负向/OOM+结构化模糊）、ch133 OOM 与
  故障注入（新增章：逐分配点轮转/失败分配器三形态/错误系统枯竭存活——
  185 个 _oom 文件的方法论放大）、ch134 嵌入（分配器接管时序/错误映射/
  裁剪主权/stats 常开/七项验收清单）、ch135 性能分析方法（新增章：档案化
  基准/环境基线十四要素/双离差门限/方向性回归阈值/sweep 扫描/提问纪律/
  基准报告五节解剖）、ch136 调优实战（三级观测阶梯/池化批量两杠杆——
  12 倍批量证据/内存调试三件套实战流/协程后端选择——卷十二收官）。
- 全书第二十三次重编号：双插入 test-oom(133)/perf-analysis(135)，
  8 文件位移，全书 142 章；卷十二 129-136 共 8 章成型。
- 门禁拦下：xrtTls/xrtPattern 等禁入符号串改为描述（forbid_symbols
  字符串值非代码符号）、虚构内部宏改示意名、xorshift 小写词改描述、
  xrt_to_host 改宿主映射函数、allocator_tour 第二行 term 与头注释逐字
  对齐、ch135/136 初稿 text 坑改 c bad/good 对照、ch129-135 多章初稿
  字数不足红线（各补设计观察/方法论节）。
- 行文章号修正 5 处（perf→136）。
