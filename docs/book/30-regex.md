...
volume: 卷四 文本与结构化数据
type: practice
lead: 非回溯引擎的最坏线性时间、不可变编译对象与独占匹配器、命名捕获与替换——收尾引入路由形态的受限表亲 pattern。
api: regex, pattern
---
...
`xregex` 对两个事故都免疫：非回溯引擎的最坏时间是线性的——不存在"坏模式"，只有"慢一点的模式"；命名捕获 `(?<key>\w+)=(?<value>\d+)` 按名字取值——加组、改组都不影响既有取值代码。再加上"编译对象不可变可共享、匹配器独占缓存"的生命周期设计，它就是为服务端长期运行准备的引擎。本章收尾还会请出它的**受限表亲**——`xpattern`：当匹配的形状是"路由/配置键"这类 `{name}` 捕获结构、且要一次对几十条模式分发时，它比正则更快也更省心。
...
### 当正则太重：结构化模式匹配（pattern）

regex 回答"**一个文本对一个模式**"；pattern 回答"**一个文本对一组模式**"——路由表、配置键、命令分发的标准形态。它的语法只有三种元素：字面字段按字节精确匹配；`{name}` 捕获一个非空字段（不跨越分隔符——默认 `/`，`xpatternconfig.Separators` 可配成字节集合）；`{*name}` 只能位于模式末尾、捕获剩余文本且允许为空。`{{` 与 `}}` 转义字面花括号。没有元语言、没有量词与分支——三个直接后果：**非回溯天然成立**（无从爆炸）；**没有转义陷阱**（用户输入永远是被匹配的文本而不是模式成分——坑 2 在这里结构性消失）；**匹配锚定完整输入**（不做路径规范化、URL 解码或大小写折叠——那些是第 101、104 章的职责，分层即此意）。

结果用三态 `xpatternresult` 表达：`XPATTERN_ERROR` / `XPATTERN_NONE` / `XPATTERN_MATCH`——**未命中不是错误**（与第 9 章 `xwaitresult` 同一哲学：控制流与错误分开）。多模式的生命周期与 regex 同构：`xrtPatternBuilderCreate` 增量注册几十条模式、`xrtPatternBuilderCompile` 一次编译成单一确定性程序（不可变、可长期持有，构建器随即释放）；命中后 `Match.Value` 直接返回注册时给的业务标识（如 `"user"`）、`Match.Index` 给模式编号——**分发键内建**，不需要再解析捕获组语义。一次性场景可走 `xrtPatternExtract` 免编译整串提取。预算四上限（模式字节/模式数/捕获数/编译产物字节）挡住不可信模式的内存攻击面，超限错误带模式内字节偏移（`xrtPatternErrorOffset`）。与 regex 的分工一句话：**要"形状描述 + 量词分支"上正则；要"字段切分 + 多模式分发"上 pattern**——第 105 章的路由表与第 138 章的日志归类都站在它这边。
...
### 完整程序：pattern——路由风格的 {name} 捕获

来自仓库范例 `examples/text/pattern/main.c`——一条路由模式，命中即分发：
...
**刚才发生了什么。** ① `xpatternspec` 规格四元组：模板、业务标识（`"user"`——命中的分发键）、预算标志；真实路由表会注册几十条再一次编译。② Builder 增量注册 → `Compile` 产出不可变对象 → **构建器即可释放**——与 regex 编译对象同一生命周期纪律。③ `xrtPatternMatch` 捕获按模板声明顺序写入调用方数组（`group` 在前、`id` 在后），捕获是**借用输入文本的视图**（第 3 章纪律）。④ 输出行一次拿到三个分发要素：`Match.Value`（哪类路由）、两个捕获段（参数值）——HTTP 路由处理器的全部输入就在这一行里。
...
- **pattern 边界**：完整串锚定；捕获借用输入文本；错误在 `xrt.pattern` 域、`xrtPatternErrorOffset` 给模式内字节位置；未命中是 NONE 不是错误。

### 从示例到工程：正则的三个宿主

**验证层**：输入合法性检查（用户名规则、SKU 格式）——模式静态编译为全局，每请求只建匹配器、`FullTest` 判整串。验证模式的三个纪律：尽量锚定（`^...$` 或 FullTest 语义）、字符集白名单优于黑名单、失败消息带上格式说明。**提取层**：从半结构化文本抽数据（日志字段、配置行）——命名捕获 + `Next` 全扫，提取结果进值树或业务结构；这是第 36 章文本管线的中间站之一。**替换层**：模板化改写（格式转换、脱敏）——`Replace` 的捕获引用把重组逻辑留在替换串里。三层都适用同一条性能纪律：**编译在启动、匹配在热路径**——坑 1 的 good 形态是三层共同的代码骨架。
...
正则模式是浓缩的代码——它该享受代码的待遇。三个习惯：**模式命名**——`static const char* PATTERN_SKU = "..."` 加一句注释说明意图，不要散落在调用点；**边界用例集**——每个模式配"应命中/不应命中/边界"三组样例（第 26 章边界值思想的移植），模式修改时回归；**性能抽样**——用第 6 章统计或简单计时对代表性输入跑一遍，确认无常数意外。第 11 章的可复现随机（固定种子生成畸形输入）给模式的模糊测试提供了现成工具——这些纪律在第 133 章测试体系会再次系统化。
...
用命名组匹配 `[WARN] 2026-04-02 disk 90%`——级别、日期、模块、数值四个组各自捕获；统计一段日志里 WARN 与 ERROR 的计数差。提示：日期组捕获后再用第 40 章时间解析（如果有）或保持字符串即可。
...
| pattern 表亲 | `{name}` 捕获+字面段；Builder 多模式一次编译；三态不含错误；Match.Value 即分发键 |
