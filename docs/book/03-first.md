...
| `xtime` | `int64` | Unix Epoch 微秒绝对时间（第 36 章展开） |
| `xseek` | 枚举 | `XSEEK_START` / `XSEEK_CURRENT` / `XSEEK_END`，文件与通用 IO 共用的移动基准 |

两个容易被老 C 手忽视的细节。其一，`ptr` 而非 `void*` 直书出现在所有回调签名里——用户数据参数统一是它，配合强制转换往返；其二，`xtime` 用**微秒**而非秒或毫秒作单位，且是绝对时间戳——这与系统调用的精度对齐，避免了到处乘除 1000 的口径混乱。时间体系（时钟、时区、休眠）在第 36 章展开，本章只需要记住"看到 `xtime`，想微秒"。
