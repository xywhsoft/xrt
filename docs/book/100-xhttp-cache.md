...
**刚才发生了什么。** ① 缓存策略是**独立协议模块**（`http_cache_policy`）：请求的 Cache-Control 指令、响应的可存储性、新鲜度与年龄——客户端缓存层只是它的使用者（与第 90 章 Body 复用 Plan、第 96 章压缩复用协商层同一模式）。② 本示例离线验证策略判定：给定请求/响应头，可存储吗？新鲜多久？需要验证吗？——**缓存行为的每一步都是可单测的纯函数**，这是"自动行为不失控"的工程保证。③ 巡检族还包括 `cache_control`（指令解析）、`cache_time`（年龄计算）、`cache_validate`（验证器选择）、`cache_range`（Range 组合）、`cache_store`（存储契约）、`cache_status`（状态头生成）——六个协议模块撑起自动缓存，全部独立可裁剪。
