...
第 62～65 章解决了"地址从哪来"——解析、端口、缓冲与 DNS。本章开始真正传输数据：建立 Engine、监听端口、发起连接、收发字节，最后干净地关闭一切。TCP 模块是后面所有内容的地基——第 67 章的背压与服务端形态、卷八的 TLS、卷九的 HTTP，全部构建在 `xnetstream` 这一个对象之上。本章先掌握两个面：阻塞的同步面（工具程序的最少代码路径）与事件回调面（生产服务的骨架），并把"状态机"和"关闭时序"这两件最容易出错的事彻底讲清楚。
