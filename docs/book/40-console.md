...
**刚才发生了什么。** ① 正常输出走 stdout、诊断走 stderr——`2>/dev/null` 的对照实验直接验证了分流。② 管道组合性：`./a.exe | grep service` 只会命中 stdout 行——机器接口与人类接口互不越界。③ 这个分流与第 38/39 章的 Logger 汇流：Logger 的控制台 Sink 同样遵守"INFO 以上 stdout、ERROR 以下 stderr"或按配置——控制台模块是它的底层流约定。
