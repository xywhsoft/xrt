...
**刚才发生了什么。** ① 文件路径：Parse 与 Stringify 直接对接文件——配置加载与导出的文件形态。② 流式写：Writer 逐元素写、计数 23 个元素 9 个引号——"边生成边输出"的实证。③ sink 写：Writer 输出直接接目标（缓冲/文件）——序列化与 IO 的组合形态，省掉中间字符串。四种姿势覆盖了 JSON 与文件系统（第 44 章）的全部交汇点。
