...
The configuration (`xprocessconfig`) declares the runtime environment: working directory, environment variables (incremental override of Chapter 42's process environment), standard-stream handling (discard/inherit/capture/pipe), timeout and kill ladder (on timeout send SIGTERM first, SIGKILL after the grace period — the sending end of Chapter 48's send/receive division).
...
| Timeout | the config declares the TERM→grace→KILL ladder - the sending end, across from Chapter 48 |
