...
Symptom: the asynchronous consumer reads scrambled fragment data or crashes — the same dangling-view family as Chapter 95.
...
Symptom: a Worker blocked — every other connection on the same Worker stalls (the server-side cost of Chapter 63's Worker economics).
...
static void onRequest(xhttpserver* pS, xhttpconn* pC,
	...
	submit_to_pool(pC, pReq);   /* hand to the execution pool (Chapter 58), retaining the Connection */
