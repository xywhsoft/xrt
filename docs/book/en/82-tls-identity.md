...
Two parties in TLS must "prove themselves": the client proves "whom I trust" (verifier, Chapter 84), the server proves "who I am" (**identity**, this chapter). `xtlsidentity` packages one DER certificate chain and one private key into an **immutable shared object**: at creation it deep-copies all material and completes cross-validation (does the private key really match the certificate's public key? Which TLS schemes can this certificate sign?), after which multiple threads and server configurations share it concurrently, and the handshake uses it to produce CertificateVerify signatures. Four strongly-typed constructors (RSA/P-256/P-384/Ed25519) cover mainstream servers; `xrtTlsIdentityCreate`'s extension interface outsources the signing capability to an HSM, the system key store, or a remote signer. Client-certificate authentication is currently unimplemented (Chapter 81's contract), so identity today is a **server-side** concept — but the object itself is decoupled from the role.
...
```diagram flow
- Load: PEM/file -> DER (Chapter 77); chain + private key handed to a constructor
