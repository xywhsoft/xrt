...
POP3 streaming receipt → feed line by line into Chapter 115's message view / incremental MIME parsing → extract headers and body → write to a local mbox/JSONL store. A 20 MB message. Acceptance criteria: memory constant throughout (MB scale); after archiving, Chapter 115's parsing restores the whole; resumable receipt (UIDL reconciliation).
