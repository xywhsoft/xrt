...
The five extension libraries of Chapters 110–129 each carry a `config/modules.json` — with `--manifest extlibs/xhttp/config/modules.json` stacked, the same build.py compiles and tests the extension closure (including "the core trim closure the extension requires"). xruntime's README shows the full gate sequence: the build regression, the amalgamate consistency check (`--check`), the release-maturity check, package verification — **library and extensions, one toolchain, one discipline**.
