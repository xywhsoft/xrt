...
### The isomorphism with Chapter 25's strings and Chapter 77's DER

You will find this chapter's APIs isomorphic with Chapter 25 (Trim/Cut's view pipeline) and Chapter 77 (the DER cursor): **borrowed input → strict validation → view output → convert on demand**. It is XRT's library-wide philosophy of text processing — zero-copy runs through to the protocol layer; "parse once, reference everywhere".
