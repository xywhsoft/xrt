...
Chapter 133 presented OOM testing as one of the four weapons; this chapter **magnifies it into a full discipline** — behind 185 `_oom` files stands a rigorous methodology. Four blocks: **per-allocation-point rotation** (the looping enumeration of "the Nth allocation fails" — "every allocation failing" is verified individually, not "some allocation fails"); **the three failing-allocator forms** (`xrtMemDebugFailAfter` counting / hand-written quota (Limited+Remaining — the map OOM test's shape) / fail-always (test_oom's `FailAllocator`) — each with its use); **the error system's survival under exhaustion** (error reporting in OOM allocates no memory — when `xrtErrorCreate` fails, the slot keeps `XERR_MEMORY`: reporting "no memory" itself needs no memory); **the debug heap weaponized** (FailAfter combined with the event stream — injected failures are auditable; the quarantine catches double-free/UAF — fault shapes go beyond "allocation failure"). One goal: **"no resources" goes from a runtime assumption to an exhaustively enumerated fact**.
