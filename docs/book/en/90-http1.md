...
This chapter's APIs stop at "the head block ends". Chapter 91's `MessageParse` combines "head + body" in one pass (fits small messages), and the `Body` family streams the body (fits a proxy forwarding as it receives) — both layers share this chapter's head parsing.
...
**What just happened.** (1) Line 1 verifies the limits model: target length within the limit passes, over the limit refuses — both sides of the defense parameter. (2) Line 2 is the lookup combination: find a field by name + transfer-coding token iteration (Chapter 89's TokenNext applied to a Head). (3) Line 3 previews Chapter 91: the request body's Plan (fixed-length/chunked verdict) is available immediately after head parsing. (4) Line 4 verifies chunk size line generation (`5\r\n` — extensions allowed). (5) Line 5 is the Message layer: one parse of "head + body", the body a borrowed view `ok` — the thriftiest path for small messages. (6) Line 6 verifies trailer array rebinding and trailer block parsing — a Chapter 91 preview. Six lines together: this chapter's plus the next's API surface.
...
- **Three-state return**: `READY`/`NEXT`/`ERROR`; `NEXT` consumes nothing and reports no error; failure does not advance (retry semantics as in Chapter 72's framing).
...
- **Method semantics**: response parsing carries the request-method context (the HEAD/204 no-body rule is executed by Chapter 91's Plan).
...
```c good
/* append-style buffer (Chapter 65): everything received accumulates in one span */
...
| Message layer | head + body in one pass (small messages); the Body layer streams (Chapter 91) |
