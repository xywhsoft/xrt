...
The base client is plaintext; the `smtp_client_tls` layer adds two forms: `XMAIL_SECURITY_TLS` (implicit — the handshake completes before the banner, the port-465 shape); `XMAIL_SECURITY_STARTTLS` (upgrade — EHLO declares STARTTLS, validate the `220` switch, take over TLS on the original stream, **re-send EHLO after the handshake**). Two hard rules: **the upgrade does not carry over the pre-upgrade capability snapshot** (a man in the middle may strip capabilities — the renegotiation is the truth); **a verifier must be provided** — certificate validation is never silently skipped (Chapter 84's "not verifying is not an option", mail edition).
