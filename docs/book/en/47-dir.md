...
- **Defense in depth**: lexical validation (Chapter 44) as fast filter + system-level root semantics as backstop — the two layers cooperate, not duplicate.
...
| Defense in depth | lexical validation (Chapter 44) fast-filters + root-handle semantics backstop - the cheap absorbs volume, the sturdy catches |
