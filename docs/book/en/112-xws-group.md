...
Cause: the group's responsibility is **membership**, not connection lifecycle — Remove only returns a reference; closing is `xrtWsConnClose/Abort`'s business (Chapter 110). The separation of responsibilities makes combinations like "out of the broadcast group but still managed" possible.
