...
SSH's "serialization format" is just five basic types: `boolean` (1B), `uint32` (4B big-endian), `uint64` (8B), `string` (4B length prefix + bytes), `mpint` (string's integer form — canonical non-negative encoding, surplus sign bits stripped). `xsshwriter`/`xsshreader` are a zero-allocation cursor pair (isomorphic to Chapter 78's DER cursors): `WriterInit` binds the caller's buffer, `WriterWrite*` writes each type, `Size` reports output; on the reader side `ReaderInit/Read*` borrows the input and reads type by type. **Strictness**: truncation rejected, trailing data (at the message layer) rejected, non-canonical mpint rejected — the entire vocabulary of an SSH message parser.
