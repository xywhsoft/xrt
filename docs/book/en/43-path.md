...
`xrtPathIterInit/Next` walks segment by segment with zero allocation (borrowing — each segment is a view slicing the original path, Chapter 25's pipeline thinking); segment counting and level-by-level directory creation (Chapter 46) both rely on it. System-path queries: home (user directory), cwd (current directory), real (the true path after resolving symbolic links — this one **does** touch the file system, the module's sole exception).
