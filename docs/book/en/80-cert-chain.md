...
- Identity: MatchHost matches the hostname (Chapter 79)
- Revocation: CrlValidate + CrlCheck look up the serial (or upper-layer policy decides soft fail)
```

Chapter 81 will show the TLS client wrapping these steps into context configuration — but every step's behavioral boundary is exactly what this chapter defines.
