...
The three-step loop is **fully isomorphic** between client and server (the client uses `xrtTlsClientDrive`) — master one side and the other comes free. The four feed forms correspond to four ownerships (borrow/take/reference/buffer chain — Chapter 65's four append kinds replayed here); the Send queue's Size/Front/Consume consumption trio is the same vocabulary as Chapter 65's buffer chain. The TLS 1.3 full-certificate flight generates ServerHello, EncryptedExtensions, Certificate, CertificateVerify, Finished in sequence inside Drive (Chapter 84's timeline), and after verifying the client's Finished it **atomically switches** to the application epoch and READY.
...
- **Unimplemented**: client-certificate authentication, 0-RTT, asynchronous identity selection; TCP composition entries belong to Chapter 87's Stream layer.
...
Change `exampleTlsMove`'s "haul whole Spans" into "at most 7 bytes per haul" random fragmentation — simulating a hostile transport. Tally the upper bound of Drive rounds to handshake completion. Acceptance: at any fragment size (1 to a whole flight) the handshake succeeds (the protocol machine supports cross-record reassembly — Chapter 84); the worst round count is bounded and explainable (flight size ÷ fragment-size order); no memory growth (Chapter 6's stats).
