...
- **IPv6 ZoneID**: RFC 9844 withdrew extensions like `[fe80::1%25eth0]` — the parser rejects (local-scope expression belongs to Chapter 70's network-address layer; two separate things).
...
- query two layers (Chapter 102): zero-allocation traversal + owning containers (& domain)
- HTTP specializations (Chapters 88/91): Host-header authority, semicolon parameters of field values (; domain)
...
Implement `next_url(当前URL, Location头值)` (current URL, Location header value): cover the five Location shapes (absolute, network path, absolute path, relative path, empty-fragment inheritance), expanding with `UrlResolve` — handling `#` per Chapter 99's inherit/block rules. Replay a real site's redirect chain (recorded via curl -L) for verification. Acceptance criteria: expansions match curl's followed final URLs; fragment inheritance follows RFC/Chapter 99; zero intermediate allocations throughout (Resolve direct-write form).
