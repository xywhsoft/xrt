...
**SendRef's release callback**: fired exactly once when the data is sent (or the stream destroyed) — the same discipline as Chapter 64's AppendRef; SendRefs is the multi-span variant (a whole span chain in one send). **SendFile's range**: offset + length bounded — chunked file sending controls memory (a kernel buffer per chunk, not the whole file).
...
**In-Worker APIs** (Buffer/Read/Consume etc. — must be called on the Stream's owning Worker thread): the consumption face of direct-move buffers (Buffer fetches the chain, Consume advances — Chapter 64's operations used directly on engine data); **the cross-thread entry** (`xrtNetPost` delivers a task to the Stream's Worker — the engine edition of Chapter 55's scheduler Post); **the wait family** (Wait/WaitAvailable blocking — rendezvous from non-Worker threads; WaitAvailableAsync Future-style — for orchestration). **Why the constraint**: the precondition of lock-free direct-move buffer operations is "one thread" — the Worker constraint is the engine edition of Chapter 55's scheduler-binding discipline; crossing threads transfers execution via Post rather than adding locks — the zero-lock discipline throughout.
...
Listener Close (stop admissions — drain in-flight accepts), the connection cap (ServerConfig's queue depth — admission backpressure: when full, refuse new connections rather than queueing forever), shutdown order (Listener → Streams → Engine — Chapter 66's order, server edition). **Connection storms' handling point**: Accept queue-depth limits + fast refusal (low error cost) — rather than accepting then disconnecting (resources already allocated).
...
Symptoms: the receiver occasionally gets dirty data — the send callback hasn't arrived and the data source is already freed; same family as Chapter 64's Pitfall 1.
