...
**What just happened.** (1) `xrtEnvSet` writes the real process environment — **child processes inherit it** (Chapter 53); the putenv semantic differences between Windows and POSIX are smoothed over by the implementation (a returned bool reports success uniformly). (2) `xrtEnvGet` reads back an **owning copy** — even if the environment changes afterward, this string is unaffected; free with `xrtFree` when done. (3) `xrtEnvRemove` deletes — the three-step closed loop (Set/Get/Remove) is the minimal complete set of environment management. The system-info queries (system name/version/machine identity mentioned in Concepts) live in the same header family, used for diagnostic headers and node-registration reports.
...
- **Child inheritance**: this process's environment changes affect child processes created afterward (Chapter 53's inheritance semantics).
...
Placing this chapter into the whole: startup assembly reads the three layers (file defaults < environment overrides < command line highest), the assembled result enters a value tree (Chapter 31) for the whole program to consume; the assembly log prints final values and their sources; required items are validated with fast failure. This assembler is the full form of Chapter 36's pipeline "config station" — exercise two's challenge version is exactly it. Assemble once, read-only globally: hot config updates (runtime changes) take the "new value tree atomically swapped in + notification" path (Chapter 31's three hosts), never runtime writes to the environment — the two disciplines echo again.
...
Implement `node_info()`: system name/version (this module) + machine identity + process start time (Chapter 42) + version number (Chapter 8), packed into a JSON value tree (Chapter 31) — the report body of distributed node registration. Acceptance criteria: run twice — static fields stable, the start-time field changes; the JSON parses under a standard parser; full-type fields (time as the time type — Chapter 33).
...
| Place in the whole | this chapter is the foundation of Chapter 36's pipeline config station; hot updates go through value-tree swap, never the environment |
