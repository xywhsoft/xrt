...
**What just happened.** (1) Normal output goes to stdout, diagnostics to stderr — the `2>/dev/null` contrast experiment verifies the routing directly. (2) Pipe composability: `./a.exe | grep service` hits only the stdout lines — machine interface and human interface never cross into each other's territory. (3) This routing converges with Chapters 38/39's Logger: the Logger's console Sink likewise follows "INFO and above to stdout, ERROR and below to stderr" or per configuration — the console module is its underlying stream convention.
...
### The relation to Chapters 38/39
