...
This layer's existence makes xhttp's learning curve a three-step staircase: this chapter (one-line calls) → Chapter 98 (builders and the runtime panorama) → Chapter 103 (the server).
...
- Runtime (Chapter 98): request builder + Client + call options + connection pool/timeouts/cancellation
- Automatic-behavior layer (Chapters 99/100): redirects/retries/cookies/decompression/caching - config switches
...
- **Deliberate refusals**: Header/streaming/forms/auth — sink to the builder (Chapter 98); no middle state.
