...
### The three-level comparison: Chapter 71 framing, Chapter 90 HTTP framing, this chapter

Chapter 71 is generic length-prefix/line framing (custom protocols); Chapter 90 is HTTP body framing (three delimittings + smuggling defense); this chapter is WebSocket framing (masking + direction rules + control-frame interleaving + fragment reassembly). The three levels share one design vocabulary: three-state returns, failure-does-not-advance, incremental absorption of sticky and partial packets — **having learned the first two levels, this chapter's only new things are the protocol-specific rules**.
...
- **Header three states**: READY/MORE/ERROR; MORE consumes nothing and reports no error; failure does not advance (as Chapters 71/90).
...
- **RSV bits**: nonzero RSV means an extension (Chapter 96's permessage-deflate) — receiving nonzero RSV without a negotiated extension is a protocol error.
- **Trimming**: `WEBSOCKET_FRAME`/`WEBSOCKET_MESSAGE` frame and message layers are independent of the Stream layer (Chapter 95) — usable without a transport.
...
| RSV bits | nonzero = an extension negotiated (Chapter 96 deflate); receiving it un-negotiated is a protocol error |
