...
`CertificateVerify` verifies only the TBSCertificate's **cryptographic signature**. What it explicitly does not do: it does not compare issuer/subject Names (chain matching is Chapter 80's duty), does not check time, does not check BasicConstraints/KeyUsage, does not check unknown critical extensions, does not query revocation, does not judge trust anchors. This boundary lets root-certificate health checks (self-signed certificates self-verified) and CA material audits (offline verification of every certificate's signature integrity) reuse the same entry — they need only mathematics, not policy.
...
- (Chapter 80) Chain: follow the Issuer to a trust anchor + revocation check
```

Note "time" mixed in — it is neither signature nor identity, but as an independent check it always executes at the same moment; Chapter 80's path validation orchestrates these three together with chain building and policy.
...
- **Lower-layer entry**: `SignatureVerify(方案, 内容, 签名, 公钥)` accepts the open protocol model — universal for certificates/CRL/OCSP/custom objects; the scheme comes from Chapter 78's algorithm translation layer.
...
Cause: signature verification's answer is "a private-key holder signed it", not "a CA you recognize signed it". Trust is the chain's and trust store's judgment (Chapter 80).
...
```c good
/* signature + identity + (Chapter 80) chain to a trust anchor — all three questions pass before accept */
if ( xrtX509CertificateVerify(&Cert, &Issuer) &&
		xrtX509MatchHost(&Cert, XRT_STR_LITERAL(sHost),
			&Presented) == X509_VALUE &&
		xrtX509ValidAt(&Cert, xrtNow()) &&
		trusted_to_anchor(&Cert /* Chapter 80 */) ) {
	accept_connection();
}
```
...
| --- | --- |
| Three-question model | signature (this chapter) → identity (this chapter) → trust chain (Chapter 80); series gates, none omittable |
