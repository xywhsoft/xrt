...
- Relative Location: resolved against the current effective URL (Chapter 101's RFC 3986 reference resolution)
...
The precise boundary of method rewriting: only the **standard method** `POST` matches — a custom method `post` does not (HTTP methods are case-sensitive, the same rule family as Chapter 90); 307/308's keep path requires the body to declare replayability (Chapter 98's `xhttpbody` capability bit) — a streaming producer body cannot be redone; failing explicitly beats dropping quietly. The whole chain shares one total deadline (Chapter 98) — ten hops do not inflate the budget. Per-hop choices are independent: inherit/follow/return the original response/treat as error (the four modes of the redirect field in the call options' `xhttpcalloptions`).
