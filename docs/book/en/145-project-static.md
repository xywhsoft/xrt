...
```diagram flow
- Security entrance: the TLS Server (certificates + SNI) - Chapters 86/87
- HTTP layer: Server + Router + middleware (logging) - Chapters 104/105
- Static layer: http_server_static - directory mapping/traversal protection/MIME/ETag/Range - Chapter 105
- Compression layer: reply_compress negotiation - Chapter 109
- Observation: logger JSON Lines + stats - Chapters 38/6
- Lifecycle: the five-timeout defaults + Drain - Chapter 104
```
...
int main(int argc, char** argv)
{
	...
	wsv_defaults(&Cfg);
	wsv_env_override(&Cfg);            /* Chapter 43 */
	...
	Ident = wsv_load_identity(&Cfg);  /* Chapter 83: startup-time verification */
	...
	if ( !wsv_mount(Router, &Cfg) ||          /* the directory-mapping array */
		!xrtHttpServerUse(Router, wsv_log, &Cfg) ||  /* Chapter 105 */
...
All chapters walked by webserv (and the six-project volume) — inventoried by volume: **Volume 1** (Chapters 3–8: the first program to trimming — the foundation of everything); **Volume 2** (Chapters 10–12: numbers and hashing — the fingerprint principle of ETags); **Volume 3** (Chapters 13–23: containers — the base of mapping tables and request queues); **Volume 4** (Chapters 25–36: text and data — the parsing chain of URL/headers/JSON logs); **Volume 5** (Chapters 37–50: system services — the daily face of logging/files/paths/directories/asynchronous IO); **Volume 6** (Chapters 51–61: concurrency — the correctness foundation of connection concurrency); **Volume 7** (Chapters 62–72: networking — the transport base of TCP/buffers/engines); **Volume 8** (Chapters 73–88: security — every layer of the TLS entrance); **Volume 9** (Chapters 89–97: the protocol core — HTTP/WS byte-level semantics); **Volume 10** (Chapters 98–109: xhttp — this service's direct toolbox); **Volume 11** (Chapters 110–129: extension libraries — the tool sources of pushd/xdl); **Volume 12** (Chapters 130–137: engineering practice — the discipline of deployment/testing/tuning); **Volume 13** (Chapters 138–145: projects — the spiral of lessons). **One hundred forty-four chapters, six projects, seventeen common-sense rules, about 650,000 words of exposition** — that is this tutorial's entire estate. What you take away should not be an API quick reference (the website has one) but **the layered eye and the disciplined hand** — they hold on any C codebase.
...
Cause: the URL decodes once before routing (Chapter 90) — whether decoding happens **before** or **after** your check decides the defense line's position. The static layer's mapping check runs on the decoded path (correct); a hand-spliced mapping validating before decoding — encoded traversal.
...
/* self-managed: decode first (Chapter 102) -> normalize -> check prefix - recheck after every decoding layer */
```
...
| Static semantics | mapping/MIME/ETag/Range/compression/cache headers — Chapter 105 owns them |
