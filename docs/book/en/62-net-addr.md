...
Volume 7 opens with two things: **a network engine overview** (xnetengine — an architectural bird's-eye view of the five-backend event engine, the industrial carrier of Chapter 60's skeleton) and **the address model** (`xnetaddr` — the unified IPv4/IPv6 address abstraction: parsing, endpoint serialization, classification, zone IDs, sockaddr round-trips). Addresses are the whole volume's foundation — TCP/UDP/DNS/proxies all use `xnetaddr` as currency; the engine overview tells every later chapter which engine layer it lives on.
...
The second thing — the engine overview: the layering of the XRT network engine (Engine → event port → Stream/Listener → protocol layer) and the five backends (IOCP/epoll/kqueue/io_uring/select). Chapter 60's skeleton spoke of "scheduler pump + offloads" — this chapter unfolds the pump's underlayer as the event port (Chapter 63), and the Stream as the engine form of the skeleton's "connection coroutine" (Chapter 66's gold standard).
...
- **Port 0**: listening with 0 lets the system assign and ListenerLocal retrieves — the standard posture for concurrency tests (Chapter 66).
