...
- **Directory mapping**: URL prefix ↔ disk directory — `/static/*` mapped to the build-artifacts directory; traversal protection (`../` attacks intercepted at the mapping layer — Chapter 47's dir-sandbox idea).
...
/* or when self-managing: normalize first (UrlPathPathNormalize, Chapter 102) then verify the prefix */
