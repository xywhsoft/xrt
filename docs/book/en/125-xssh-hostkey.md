...
Wiring this chapter's tools to Chapter 122's event: query the store before `HostKeyAccept` — MATCH passes; NEW per policy (interactive prompt/out-of-band fingerprint check then record/configured allowlist); CONFLICT rejects and alarms. The fingerprint is printed in both NEW and CHANGED states — for verification in the former, for reporting in the latter. This assembly has its standard form in Chapter 127's client runtime.
...
Implement `verify_host(主机, 密钥, 库)` (host, key, store): query → branch by state (MATCH silent pass / NEW print fingerprint and prompt / CHANGED alarm and reject) — wired to Chapter 122's event. Acceptance criteria: the three paths behave correctly; the fingerprint output carries the algorithm prefix (SHA256: form).
...
| Interface with Chapter 119 | VERIFY_HOST_KEY event + this chapter's store query = the complete trust judgment |
