...
**认证**：SOCKS5 无认证/用户密码（RFC 1929）；HTTP Basic/自定义头。**通道安全**：代理本身不加密——TLS over 代理（CONNECT 后跑 TLS 握手——第 78 章组合）；**代理链**：理论上可串联（拨号经代理 A 到代理 B 到目标——配置层组合）。
