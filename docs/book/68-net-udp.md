...
UDP 与 TCP 的本质差异不是"可靠 vs 不可靠"，是**数据边界**：TCP 是字节流（消息边界自管——第 64 章缓冲链+第 76 章分帧为此存在）；UDP 是数据报（一次 send 一个报文、一次 recv 一个完整报文——边界内核保管）。这个差异让 UDP 的 API 形态完全不同：没有发送预算（报文要么进队列要么不进）、没有接收链（一个报文一块缓冲）、没有流控（不可靠是特性——实时音视频宁可丢也不积压）。
