...
- [test.c](../test.c) 已经成为统一 runner。
- `test/` 目录下的测试已接入 runner 注册表，并支持 `SINGLE / PRESET / ALL`。
- `Thread` 重复入口已经收口到 [test_thread_core.h](../test/test_thread_core.h)，旧 [test_thread.h](../test/test_thread.h) 仅保留兼容壳。
- `FutureCore` 已不再反向依赖 `dev/test_future_core.c`，实现落在 [test_future_core_impl.h](../test/test_future_core_impl.h)。
- `memtelemetry baseline` 已进入 `test/` 体系：[test_memtelemetry_baseline.h](../test/test_memtelemetry_baseline.h)。
- `dev/test*.c` 独立入口已经清空。
- 默认脚本 [build_test.sh](../build_test.sh) 与 [build_GCC_TEST_x64.bat](../build_GCC_TEST_x64.bat) 已直接编译并调用 [test.c](../test.c)，并在脚本末尾补跑 `singlehead` smoke。
- Windows 脚本不再切换到 `release/x64` 目录执行 runner，避免 TLS/HTTP/WS 测试因为工作目录变化而丢失相对路径夹具。
- `xnet_native_core` 专项脚本已在两端补齐：
	- `build_test_xnet_native_core.sh`
	- `build_GCC_TEST_XNET_NATIVE_CORE_x64.bat`
	并且都支持“无参数直接跑默认轮次、需要时再覆盖参数”。
- 测试产物清理脚本已补齐：
	- `build_clean_test.sh`
	- `build_GCC_CLEAN_TEST_x64.bat`
	只清理 runner/专项脚本生成的可执行文件与派生输出，不删除跟踪夹具。

当前保留的策略：

- `build_test_xnet_native_core.sh` 继续保留为专项 native backend 回归脚本。
- `build_GCC_TEST_XNET_NATIVE_CORE_x64.bat` 作为 Windows 对应专项脚本一并保留。
- 顶层默认脚本只跑 curated modern smoke，不把 `xnet_native_core` 混入默认批量回归。
- 阶段性 runner 产物目前主要通过 [.gitignore](../.gitignore) 收口，物理删除可留到人工清理阶段。
- 如需物理清理，可直接运行 `build_clean_test.sh` 或 `build_GCC_CLEAN_TEST_x64.bat`。

## 1. 背景

改造开始前，测试体系存在以下问题：

- 根入口 [test.c](../test.c) 依赖手工注释开关，无法作为稳定 runner 使用。
- `test/` 目录中已有 59 个测试头，但 [test.c](../test.c) 只接入了其中一部分。
- 默认构建入口分叉：
	- [build_GCC_TEST_x64.bat](../build_GCC_TEST_x64.bat) 与 [build_test.sh](../build_test.sh) 默认走 modern 分支。
	- `build_GCC_TEST_LEGACY_x64.bat` 与 `build_test_legacy.sh` 仍然编译 [test.c](../test.c)。
- `dev/test*.c` 中存在大量独立入口，部分只是包装器，部分已经与 `test/` 中的测试出现同类重复。

本次改造目标是把 [test.c](../test.c) 变成唯一手工回归入口，同时保留“直接运行 `test.exe`、不记命令行、控制台尽量干净”的使用习惯。
...
- 不强依赖命令行参数。
- 日常快速验证时，只需要改 [test.c](../test.c) 顶部一个配置区即可切换测试目标。
- 单测运行时默认只输出当前选中测试的内容，不额外展开无关模块。

### 2.2 工程侧目标

- [test.c](../test.c) 成为统一 runner。
- `test/` 目录中的测试全部纳入 runner 注册表。
- 同类测试逐步合并，移除 `dev/test*.c` 中纯包装器式入口。
- 最终将默认测试构建脚本重新收敛到 [test.c](../test.c)。

### 2.3 非目标

- `examples/`、`dev/bench/`、`fuzz`、`singlehead` 不并入 [test.c](../test.c)。
...
在 [test.c](../test.c) 顶部保留一个小而稳定的配置区，例如：
...
- [test.c](../test.c) 能调度 `test/` 目录下全部 59 个测试头。
...
- `xnet` 测试当前仍通过 [test_xnet_internal_env.h](../test/test_xnet_internal_env.h) 接入内部实现环境；虽然命名已收口，但后续仍可继续评估是否要把这层环境进一步模块化。
...
1. 重写 [test.c](../test.c) 顶部配置区与 runner 主循环。
...
1. 视需要继续收口 [test_xnet_internal_env.h](../test/test_xnet_internal_env.h) 的职责边界。
