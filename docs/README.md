...
如果你想直接看当前已有的组合案例，建议先走 `case/` 的主线必读：

1. [用 xvalue + json 构造配置系统](case/json-config-system.md)
2. [用 Template 渲染一个 HTML 页面](case/template-render-html.md)
...
8. [用 XRT 写一个最小 HTTP 服务](case/minimal-http-service.md)
9. [用 XRT 调用一个流式 LLM API](case/streaming-llm-api.md)
10. [一个完整的 HTTP + JSON + Template 服务链路](case/http-json-template-chain.md)
11. [用 Charset + Regex + Crypto 导入一个签名规则包](case/signed-rule-bundle.md)
12. [用 MemPool + AVLTree + List 组织一个会话索引与回收池](case/session-registry-pool-index.md)
13. [把配置、日志、任务、网络和模板串成一个本地控制台服务](case/local-console-service.md)

更重的缓存 / 归档 / 恢复 / 共识 / 快照 / 租约 / journal 页面，已经不再作为默认主阅读路线无限往后接。

请直接在 [案例入口](case/README.md) 里按 `主线必读 / 进阶组合 / 高级控制面专题` 3 层选择阅读。
