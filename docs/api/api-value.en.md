...
### Shared roots and stable views

Shared mode does not mean every view API is automatically safe without rules.

The recommended model is:

- use explicit shared roots for cross-thread ownership
- use root-level `Lock/Unlock` semantics where the container API requires them
- treat pointer views, iterators, and walk-style access as operations that may need stronger coordination than simple set/get calls

In other words:

- shared mode gives you an explicit contract
- it does not remove the need for ownership and synchronization discipline
