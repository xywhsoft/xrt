...
任何收集完整消息或持有正文的上层都必须设置符合自身内存预算的有限上限。XRT stream 层已提供自己的
连接级默认限制。
...
旧连接对象、Writer、Future/协程桥接、重连与心跳策略、连接组和广播已经从 XRT 核心退出，原实现、
测试、示例和设计文档保存在 `extlibs/xws/archive`。基础 TCP/TLS 通信、关闭、背压和连接级压缩留在
XRT；后续 `xws` 只在这条稳定路径上增加高级抽象。
