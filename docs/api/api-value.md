...
### shared root 与 root lock

shared root 解决的是“跨线程共享是否合法”，不等于“任何公开操作都天然无锁安全”。

当前主线推荐这样理解：

- shared array root：使用 `xrtArrayLock()` / `xrtArrayUnlock()`
- shared ptrarray root：使用 `xrtPtrArrayLock()` / `xrtPtrArrayUnlock()`
- shared dict root：使用 `xrtDictLock()` / `xrtDictUnlock()`
- shared list root：使用 `xrtListLock()` / `xrtListUnlock()`
- shared coll / tree root：使用 `xrtAVLTreeLock()` / `xrtAVLTreeUnlock()`

当你需要执行下面这些“直接暴露内部结构”的路径时，应该显式持有 root 锁：

- walk
- iterator
- pointer view
- 需要在多步读取过程中保持一致视图的组合读取

简单的 root-level `Set / Get / Remove / Exists` 仍然优先通过公开包装接口进行，不要绕回内部无锁基元。

### shared 模式的推荐边界

当前更稳的写法是：

- 把真正跨线程共享的顶层容器显式创建成 shared root
- 通过 root-level public API 修改它
- 复杂遍历时显式加锁
- 本地临时容器保持 local，避免无意义地把所有对象都提升成 shared
...
- 当 shared `coll` 需要做 walk / iterator / 多步稳定读取时，应显式使用底层 root lock 保护整个读取窗口
