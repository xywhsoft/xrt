...
- `xvalue` 负责统一数据承载
- `json` 负责外部交换与加载
- `xhttpd / xhttp` 负责把模板渲染结果送到网络边界

也就是说，模板引擎不是孤立模块，而是 `xvalue + json + http` 主线中的渲染层。
...
// 准备数据
xvalue data = xvoCreateTable();
xvoTableSetText(data, "name", 0, "张三", 0, FALSE);
xvoTableSetInt(data, "age", 0, 28);
xvoTableSetBool(data, "vip", 0, TRUE);
xvoTableSetInt(data, "vipLevel", 0, 5);
...
- 页面级 / 文档级数据：放进 `tblVal`
- 跨模板共享的全局参数：放进 `tblENV`
- 复杂结构优先使用 `xvalue + json` 先整理好，再交给模板引擎

不推荐继续维护一套与 `xvalue` 脱节的独立数据层。
...
xvalue PrepareUserData(User* user) {
    xvalue data = xvoCreateTable();
    xvoTableSetInt(data, "id", 0, user->id);
    xvoTableSetText(data, "name", 0, user->name, 0, FALSE);
    xvoTableSetText(data, "email", 0, user->email, 0, FALSE);
    xvoTableSetBool(data, "vip", 0, user->is_vip);
    return data;
}
...
模板引擎最自然的输入，就是已经整理好的 `xvalue` 数据。

推荐链路：

1. 用 `xrtParseJSON()` 解析配置或业务数据
2. 用 `xvoTableSet*()` / `xvoArrayAppend*()` 补充运行时字段
3. 把最终 `xvalue` 交给 `xteMake()`

这样模板引擎、JSON 和动态类型系统会共用同一套数据模型，而不是让模板层再次承担数据搬运工作。
...
- `HTTP 服务端 HTML 渲染`
- `把结构化数据渲染为 prompt / 文本片段 / 配置片段`

在 AI 场景里，模板不一定只用来生成网页，也可以用来：
