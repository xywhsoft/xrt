...
phase-2 之后，`FSMemPool` 也已经进入 owner-thread 主线。
...
- 本线程分配/释放走快路径
- 错线程访问会被拒绝并设置当前线程错误
- shared 语义仍属于逐步收口中的能力，不应默认把所有固定池都当成可无条件跨线程共享

### 当前主线调试能力

固定池当前也不只是性能工具，还承担调试辅助角色。

配合当前运行时与内存调试能力，通常可以更容易暴露：

- 重复释放
- 非法释放
- 生命周期错误
- 已释放对象继续回到池结构
- 错线程访问 owner-thread local 固定池
...
- **GC 支持**：支持标记-清除垃圾回收
- **线程诊断**：错线程访问不会再静默破坏内部链表
- **调试友好**：适合纳入当前运行时的对象生命周期排查链路
