...
JSONL 将逐行记录序列与 xvalue Array 相互转换，每个元素对应一条记录；单条值沿用 JSON 的类型和编解码规则。适用于日志、批量导入和文件交换。
...
头文件为 `<xrt/jsonl.h>`，也可经总头 `<xrt.h>` 引入。单独选择读取或写出不引入文件模块。
...
- `Valid` 沿用 JSON 的无 DOM 语法验证：忽略空白行，检查默认累计预算，不执行重复键 DOM 策略。
...
| 常量 | 值 | 语义 |
|---|---|---|
| `XJSONL_READ_REJECT_EMPTY_LINES` | `UINT32_C(0x00000001)` | 空白行报错，默认关闭。 |

配置必须经 Init 初始化；所有预算非零。Record 的默认值由现有 JSON 初始化函数提供。整体输入/输出默认 64 MiB，记录数与语法值数默认 1000000。
...
错误经 `xrtGetError()` 取得，格式域为 `xrt.jsonl`；参数与底层 OOM 也可直接返回 Core 错误。RECORD/OUTPUT/IO 包装保留 Cause 的 Kind。若包装自身分配失败，保留原始原因错误，不承诺定位数据。

| 值 | 语义 |
|---|---|
| `XJSONL_ERROR_CONFIG` | 配置未初始化、保留位非零、预算为零或启用 PRETTY。 |
| `XJSONL_ERROR_SYNTAX` | 严格空行模式下遇到空白行。 |
| `XJSONL_ERROR_LIMIT` | 整体或单条预算耗尽。 |
| `XJSONL_ERROR_RECORD` | 单条值编解码或加入 Array 失败；Cause 保留具体原因。 |
| `XJSONL_ERROR_TYPE` | 写出根值不是 Array。 |
| `XJSONL_ERROR_OUTPUT` | 同步输出回调失败。 |
...
| 字段 | 类型 | 语义 |
|---|---|---|
| `Record` | `xjsonreadconfig` | 单条配置；MaxInputBytes/MaxOutputBytes 不包含 CRLF/LF 分隔符，深度从每条根值开始计算。 |
| `Flags` | `uint32` | 默认 0；XJSONL_READ_REJECT_EMPTY_LINES 启用空白行报错。 |
...
| 字段 | 类型 | 语义 |
|---|---|---|
| `Record` | `xjsonwriteconfig` | 单条配置；MaxInputBytes/MaxOutputBytes 不包含 CRLF/LF 分隔符，深度从每条根值开始计算。 |
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlErrorLocation 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlReadConfigInit 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlParse 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlRead 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlValid 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlWriteConfigInit 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlStringify 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlWrite 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlParseFile 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlReadFile 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlStringifyFile 调用，失败转入统一清理。
...
[jsonl](../../examples/data/jsonl/main.c) · 完整程序中的 xrtJsonlWriteFile 调用，失败转入统一清理。
