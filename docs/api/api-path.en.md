...
- Returns the empty-string sentinel if the path contains no directory separator
...
- Returns the empty-string sentinel if the path ends with a separator
...
- Returns the empty-string sentinel if there is no extension
...
- Returns the empty-string sentinel if the path exceeds 4094 characters
...
void ParsePath(str path) {
    ...
    printf("Directory: %s\n", dir[0] != 0 ? (char*)dir : "(none)");
    printf("Filename: %s\n", name[0] != 0 ? (char*)name : "(none)");
    printf("Extension: %s\n", ext[0] != 0 ? (char*)ext : "(none)");
    
    if ( dir[0] != 0 ) xrtFree(dir);
    if ( name[0] != 0 ) xrtFree(name);
    if ( ext[0] != 0 ) xrtFree(ext);
}
