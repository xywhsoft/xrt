...
**补充说明：**
- 如果 `sText` 为 `NULL` 或长度为 0，返回空字符串哨兵
- 返回的字符串已自动添加 `\0` 结尾符
...
**补充说明：**
- 输出为大写十六进制字符
- 编码后长度等于输入长度 × 2，可直接通过 `strlen((char*)...)` 获取
...
- 包含非法字符时返回错误并设置当前线程错误，可通过 `xrtGetError()` 获取
- 示例中的解码长度可根据已知输入直接确定；需要显式长度输出时，优先选择带输出参数的 API
