...
`case/README.md` 之前还存在另一类结构问题：

- 入口页把大量恢复 / 共识 / 快照 / 租约 / journal 页面继续接成同一条主案例梯度
- 结果是“案例数量更多了”，但“第一次该怎么读”反而更不清楚

这个问题的本质不是页面没价值，而是分类不对。

更合理的做法是：

- `主线必读` 保持收敛，优先承载广覆盖的组合案例
- `进阶组合` 承载缓存、归档、回温、恢复、跨介质编排这类业务专题
- `高级控制面专题` 单独承载恢复 / 共识 / 快照 / 租约 / journal 这条更重长链
...
- 案例入口已按 `主线必读 / 进阶组合 / 高级控制面专题` 三层重组，冻结“同一控制面长链继续无限扩张”的默认入口写法
