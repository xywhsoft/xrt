...
假设你已经有了前面那个本地控制台服务，现在需要同时管理“当前热点”和“仍然有效但已经降温的数据”：
...
如果把冷层直接当成 archive，一个对象进入冷层往往会被误解成：
...
2. 长期归档
	决定冷层对象何时落到更长期的文件层。
3. 多来源恢复
	决定本地文件、包文件和外部工具链之间怎样切换。

如果你想对照相邻案例，建议一起阅读：

- [把配置、日志、任务、网络和模板串成一个本地控制台服务](local-console-service.md)
- [把本地控制台服务升级成一个跨介质编排面板](cross-media-orchestration-dashboard.md)
