...
- 日常程序开发：优先 `xrtParseJSON()` / `xrtStringifyJSON()` + `xvalue`
...
- `xvalue` 负责程序内部的数据组织
- JSON 负责外部交换、存储、网络传输
- 如果一份结构既要在程序内部频繁处理，又要序列化为 JSON，对外层应尽量保持“一份 `xvalue` 主数据 + 按需 stringify”的组织方式
...
xvoArrayAppendText(tags, "developer", 0, FALSE);
xvoArrayAppendText(tags, "golang", 0, FALSE);
...
JSON 本身不是独立的并发模型，它当前完全建立在 `xvalue` 之上。

因此：

- local JSON 结构：默认使用 `xvoCreateTable()` / `xvoCreateArray()`
- 跨线程共享 JSON 结构：显式使用 `xvoCreateTableEx(XRT_OBJMODE_SHARED)` / `xvoCreateArrayEx(XRT_OBJMODE_SHARED)`

示例：

```c
xvalue pConfig = xvoCreateTableEx(XRT_OBJMODE_SHARED);
xvalue pServers = xvoCreateArrayEx(XRT_OBJMODE_SHARED);

xvoArrayAppendText(pServers, "api.example.com", 0, FALSE);
xvoTableSetValue(pConfig, "servers", 0, pServers, TRUE);
```

如果把 local 容器直接写入 shared JSON root，当前主线会拒绝该操作并设置当前线程错误，而不是继续静默混用。
...
- `HTTP 请求体 -> JSON -> xvalue -> 业务逻辑`
- `xvalue -> JSON -> HTTP 响应体`
- `xvalue -> template -> HTML`
- `xvalue -> JSON -> LLM API 请求`
- `LLM API 返回 JSON / SSE 片段 -> xvalue -> 后续处理`

也就是说，JSON 在 XRT 里更像“交换层”，而 `xvalue` 才是统一的数据主线。
