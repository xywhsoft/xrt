...
- 与 `future / task / promise / wait-source` 主线组合
- 与 xnet/event-loop 的非阻塞等待集成
...
除了原生 `xcoevent`，当前 XRT 还提供了两条推荐组合路径：

- `future -> xrtNetFutureWaitCo*`
- `wait-source -> xrtNetWaitSourceWaitCo*`

这意味着协程不必自己维护另一套“异步结果协议”。推荐理解方式是：

- 协程：负责执行和挂起
- future：负责结果、错误、取消和完成态
- wait-source：负责把网络事件、future、datagram、listener 等统一成等待对象
- scheduler：负责调度、post、timer、deadline

如果业务逻辑已经建立在 `future/task/promise` 上，协程优先等待 `future` 或 `wait-source`，而不是自行重复封装轮询逻辑。
...
- `ThenCurrent / CatchCurrent / FinallyCurrent` 这类 current-thread deferred continuation，在当前线程调用 `xFutureWait*()` 时会自动 pump；如果线程没有进入 future wait 主路径，也可以手工调用 `xFuturePumpCurrentContinuations()`。
