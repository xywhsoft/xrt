...
- `xrtThreadCreate()` 创建的线程退出时会自动执行线程 cleanup 栈，因此运行时相关临时资源、错误状态、默认随机数都会跟随线程一起回收

### Cleanup 回调推荐用法

当线程内部拥有“必须在线程退出时释放”的资源时，推荐使用 cleanup 栈，而不是把释放逻辑散落在多条返回路径上。

```c
static void ThreadCleanup(ptr pArg)
{
	FILE* pFile = (FILE*)pArg;
	if ( pFile != NULL ) {
		fclose(pFile);
	}
}
...
{
	FILE* pFile = fopen("worker.log", "ab");
	if ( pFile == NULL ) {
		return 1;
	}

	xrtThreadPushCleanup(ThreadCleanup, pFile);

	/* ... 正常工作 ... */

	xrtThreadPopCleanup(ThreadCleanup, pFile);
	fclose(pFile);
	return 0;
}
```
...
- 用 `xrtThreadPushCleanup()` 注册“线程退出时必须做”的清理动作
- 当资源已经在正常路径释放后，再调用 `xrtThreadPopCleanup()` 将其移出
- cleanup 回调应尽量只做轻量释放，不要在其中阻塞等待其他线程
...
下面这些能力都属于“线程绑定运行时能力”，在线程已附加到 XRT 运行时后才能安全使用：

- `xrtGetError()`
- `xrtSetError()`
- `xrtTempMemory()`
- `xrtRand32()` / `xrtRand64()`
- 依赖线程状态的 future / coroutine / wait-source 路径

因此：

- 主线程：在 `xrtInit()` 之后直接用
- `xrtThreadCreate()` 创建的线程：在线程函数中直接用
- 宿主自行创建的线程：先 `xrtThreadAttachCurrent()` 再用

### 6. Destroy 的语义

`xrtThreadDestroy()` 的职责是“释放线程管理对象”，不是“结束线程”。

这条合同应该长期保持不变：

- 线程是否结束，由线程函数自然返回或显式 stop 协议决定
- `xrtThreadWait()` 负责 join
- `xrtThreadDestroy()` 负责释放管理对象和句柄
- 如果线程还在运行，`Destroy` 会拒绝执行，而不是偷偷 `detach` 或强杀
