...
static const xrt_type_desc __xrtTestEnumType;

static const xrtenumfield __xrtTestEnumOkFields[] = {
	{
		.Name = "value",
		.NameSize = 5,
		.Type = &__xrtTestPointType,
		.Offset = sizeof(uint32)
	}
};

static const xrtenumvariant __xrtTestEnumVariants[] = {
	{
		.Name = "ok",
		.NameSize = 2,
		.Value = 0,
		.FieldCount = 1,
		.Fields = __xrtTestEnumOkFields,
		.Flags = XRT_ENUM_VARIANT_HAS_PAYLOAD
	},
	{
		.Name = "error",
		.NameSize = 5,
...
static const xrtenum __xrtTestEnumInfo = {
	.Type = &__xrtTestEnumType,
	.BackingType = NULL,
	.VariantCount = 2,
	.Variants = __xrtTestEnumVariants,
	.Flags = XRT_ENUM_FLAG_TAGGED
};

static const xrt_type_desc __xrtTestEnumType = {
	.TypeId = 10006,
	.Kind = XRT_TYPE_KIND_ENUM,
	.Name = "XrtTestResult",
	.NameSize = 13,
	.AbiName = "XrtTestResult",
	.AbiNameSize = 13,
	.Size = sizeof(uint32) + sizeof(XrtTestPoint),
	.Align = sizeof(int64),
	.Extra = &__xrtTestEnumInfo
};
...
int Test_RuntimeType(xrtGlobalData* pCore)
{
	...
	(void)pCore;
	{
		const xrtenum* pEnum = xrtEnumInfo(&__xrtTestEnumType);
		const xrtenumvariant* pVariant;
		const xrtenumfield* pField;

		if ( pEnum != &__xrtTestEnumInfo ||
		     xrtEnumInfo(xrtTypeInt()) != NULL ||
		     xrtEnumVariantAt(pEnum, 2) != NULL ) {
			return 157;
		}
		pVariant = xrtEnumFindVariant(pEnum, "ok", 0);
		if ( pVariant != &__xrtTestEnumVariants[0] ||
		     xrtEnumFindValue(pEnum, 1) != &__xrtTestEnumVariants[1] ||
		     xrtEnumFindVariant(pEnum, "missing", 0) != NULL ) {
			return 158;
		}
		pField = xrtEnumFindField(pVariant, "value", 0);
		if ( pField != &__xrtTestEnumOkFields[0] ||
		     xrtEnumFieldAt(pVariant, 1) != NULL ||
		     xrtEnumFindField(pVariant, "missing", 0) != NULL ) {
			return 159;
		}
	}
	...
	/*
		typed container 可以作为 opaque handle 进入 xvalue。
		xvoTypeDesc 必须保留具体容器描述符，动态语言层才能继续按容器协议派发。
	*/
	vHandle = xvoCreateHandle(&__xrtTestPointArrayType, &iHandleObj, XRT_HANDLE_FLAG_BORROWED);
	if ( vHandle == NULL ) {
		return 157;
	}
	if ( !xrtTypeSame(xvoTypeDesc(vHandle), &__xrtTestPointArrayType) ) {
		xvoUnref(vHandle);
		return 158;
	}
	xvoUnref(vHandle);
	...
	xvoUnref(vBoxed);
	if ( !xrtTypedArraySwap(pTypedArray, 1, 2) ) {
		xrtTypedArrayDestroy(pTypedArray);
		return 151;
	}
	pTypedInt = (int64*)xrtTypedArrayGet(pTypedArray, 1);
	if ( pTypedInt == NULL || *pTypedInt != 7 ) {
		xrtTypedArrayDestroy(pTypedArray);
		return 152;
	}
	pTypedInt = (int64*)xrtTypedArrayGet(pTypedArray, 2);
	if ( pTypedInt == NULL || *pTypedInt != 99 ) {
		xrtTypedArrayDestroy(pTypedArray);
		return 153;
	}
