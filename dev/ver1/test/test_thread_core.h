...
typedef struct {
	xrt_once Once;
	volatile int iInitCount;
	volatile int iPublishedValue;
} __test_thread_core_once_ctx;

typedef struct {
	xrt_once First;
	xrt_once Second;
	volatile int iFirstCount;
	volatile int iSecondCount;
	volatile int iPublishedValue;
} __test_thread_core_once_graph_ctx;


static void __Test_ThreadCore_OnceGraphFirst(ptr pArg);


static void __Test_ThreadCore_OnceGraphSecond(ptr pArg)
{
	__test_thread_core_once_graph_ctx* pCtx =
		(__test_thread_core_once_graph_ctx*)pArg;

	pCtx->iSecondCount++;
	xrtOnceRunGraph(
		&pCtx->First,
		__Test_ThreadCore_OnceGraphFirst,
		pCtx);
	pCtx->iPublishedValue |= 2;
}
...
static void __Test_ThreadCore_OnceGraphFirst(ptr pArg)
{
	...
	xrtOnceRunGraph(
		&pCtx->Second,
		__Test_ThreadCore_OnceGraphSecond,
		pCtx);
	pCtx->iPublishedValue |= 1;
}


static uint32 __Test_ThreadCore_OnceGraphWorker(ptr pArg)
{
	__test_thread_core_once_graph_ctx* pCtx =
		(__test_thread_core_once_graph_ctx*)pArg;
	...
	xrtOnceRunGraph(
		...
		__Test_ThreadCore_OnceGraphFirst,
		pCtx);
	return pCtx->iPublishedValue == 3 ? 0u : 1u;
}


static void __Test_ThreadCore_OnceInit(ptr pArg)
{
	__test_thread_core_once_ctx* pCtx = (__test_thread_core_once_ctx*)pArg;

	xrtSleep(20);
	pCtx->iPublishedValue = 12345;
	pCtx->iInitCount++;
}


static uint32 __Test_ThreadCore_OnceWorker(ptr pArg)
{
	__test_thread_core_once_ctx* pCtx = (__test_thread_core_once_ctx*)pArg;

	xrtOnceRun(&pCtx->Once, __Test_ThreadCore_OnceInit, pCtx);
	return pCtx->iPublishedValue == 12345 ? 0u : 1u;
}
...
static int Test_ThreadCore(void)
{
	...
	__test_thread_core_once_ctx tOnceCtx;
	__test_thread_core_once_graph_ctx tOnceGraphCtx;
	xthread arrOnceThreads[8] = { 0 };
	xthread arrOnceGraphThreads[8] = { 0 };
	...
	memset(&tOnceCtx, 0, sizeof(tOnceCtx));
	for ( i = 0; i < 8; ++i ) {
		arrOnceThreads[i] = xrtThreadCreate(
			(ptr)__Test_ThreadCore_OnceWorker,
			&tOnceCtx,
			0);
		iFail += __Test_ThreadCore_Check(
			"once worker create",
			arrOnceThreads[i] != NULL);
	}
	for ( i = 0; i < 8; ++i ) {
		if ( arrOnceThreads[i] != NULL ) {
			xrtThreadWait(arrOnceThreads[i]);
			iFail += __Test_ThreadCore_Check(
				"once worker published value",
				xrtThreadGetExitCode(arrOnceThreads[i]) == 0u);
			xrtThreadDestroy(arrOnceThreads[i]);
		}
	}
	iFail += __Test_ThreadCore_Check(
		"once callback executes exactly once",
		tOnceCtx.iInitCount == 1);

	memset(&tOnceGraphCtx, 0, sizeof(tOnceGraphCtx));
	for ( i = 0; i < 8; ++i ) {
		arrOnceGraphThreads[i] = xrtThreadCreate(
			(ptr)__Test_ThreadCore_OnceGraphWorker,
			&tOnceGraphCtx,
			0);
		iFail += __Test_ThreadCore_Check(
			"once graph worker create",
			arrOnceGraphThreads[i] != NULL);
	}
	for ( i = 0; i < 8; ++i ) {
		if ( arrOnceGraphThreads[i] != NULL ) {
			xrtThreadWait(arrOnceGraphThreads[i]);
			iFail += __Test_ThreadCore_Check(
				"once graph worker published value",
				xrtThreadGetExitCode(arrOnceGraphThreads[i]) == 0u);
			xrtThreadDestroy(arrOnceGraphThreads[i]);
		}
	}
	iFail += __Test_ThreadCore_Check(
		"once graph first callback executes exactly once",
		tOnceGraphCtx.iFirstCount == 1);
	iFail += __Test_ThreadCore_Check(
		"once graph second callback executes exactly once",
		tOnceGraphCtx.iSecondCount == 1);
