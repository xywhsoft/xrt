...
> 详细模块级评估见 [MODULE_ASSESSMENT.md](MODULE_ASSESSMENT.md)。下面优先级清单已纳入批次 A（基础运行时）评估发现的关键修复项。
...
#### 7.1.1 批次 A 立即修复（正确性 / 安全 / 数据竞争）

这些是基础运行时模块自身的缺陷，与三大主干独立，应先于或并行于主干推进：

- `time.h`: `xrtDecodeSerial` 改闭合公式（当前 O(400) 线性循环）；修复 `xrtUTCToLocal` 使用当前偏移导致历史时间戳错误；修复 `xrtDateSerial` 公元前路径；明确 `xrtWeekday` 的 epoch 假设
- `string.h`: case-insensitive 搜索消除"两份小写副本"分配；`xrtStrLastIndexOf` 改右向左扫描或 BMH；`xrtReplace` 的 `strncpy` 改 `memcpy`
- `charset.h`: UTF-8 严格校验（拒绝 overlong encoding / 代理区码点 / 超出 U+10FFFF），并提供 `xrtUTF8Validate`
- `base.h`: 修复 `xrtSetErrorU16/U32` 先 free 后 convert 的错误丢失顺序
- `math.h`: 修复 `xrtIntApprox` 在 `maxAbs==0` 时误返 TRUE 的边界
- `xid.h`: 核对 `xrtMalloc(24)` 与 `xid` 结构体大小是否匹配（疑似越界）；`xrtDecodeXID` 失败应返回 NULL 而非 sNull
- `type.h`: `xrt_callable->RefCount` 改为原子操作（跨线程持有 callable 当前是数据竞争）
