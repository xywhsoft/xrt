...
if (-not $OutDir) {
	$OutDir = Join-Path $root "dev/bench/sweeps/queue_latency/windows"
}
...
$itemsPerProducer = if ($env:XQUEUE_LATENCY_SWEEP_ITEMS_PER_PRODUCER) { [uint32]$env:XQUEUE_LATENCY_SWEEP_ITEMS_PER_PRODUCER } else { [uint32]100000 }
$capacity = if ($env:XQUEUE_LATENCY_SWEEP_CAPACITY) { [uint32]$env:XQUEUE_LATENCY_SWEEP_CAPACITY } else { [uint32]4096 }
$mpscProducers = if ($env:XQUEUE_LATENCY_SWEEP_MPSC_PRODUCERS) { [uint32]$env:XQUEUE_LATENCY_SWEEP_MPSC_PRODUCERS } else { [uint32]4 }
$mpmcProducers = if ($env:XQUEUE_LATENCY_SWEEP_MPMC_PRODUCERS) { [uint32]$env:XQUEUE_LATENCY_SWEEP_MPMC_PRODUCERS } else { [uint32]4 }
$mpmcConsumers = if ($env:XQUEUE_LATENCY_SWEEP_MPMC_CONSUMERS) { [uint32]$env:XQUEUE_LATENCY_SWEEP_MPMC_CONSUMERS } else { [uint32]4 }
...
$benchExe = Join-Path $root ("dev/bench/queue/bench_queue_latency_win_{0}_{1}.exe" -f $PID, $benchStamp)
...
$batchSizeList = Get-UIntList -EnvValue $env:XQUEUE_LATENCY_SWEEP_BATCH_SIZE_LIST -Default @([uint32]1, [uint32]4, [uint32]8, [uint32]16, [uint32]32, [uint32]64)

Write-Output "queue latency sweep (windows)"
