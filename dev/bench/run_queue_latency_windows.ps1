...
$mpmcProducers = if ($env:XQUEUE_LATENCY_MPMC_PRODUCERS) { [uint32]$env:XQUEUE_LATENCY_MPMC_PRODUCERS } else { [uint32]4 }
$mpmcConsumers = if ($env:XQUEUE_LATENCY_MPMC_CONSUMERS) { [uint32]$env:XQUEUE_LATENCY_MPMC_CONSUMERS } else { [uint32]4 }
$batchSize = if ($env:XQUEUE_LATENCY_BATCH_SIZE) { [uint32]$env:XQUEUE_LATENCY_BATCH_SIZE } else { [uint32]32 }
...
Write-Output "queue latency matrix (windows)"
