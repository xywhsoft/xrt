...
- 2026-09-07 周期全量复审 #9 · 任务终验（全防线本地复跑）：
  ...
  深化（不在 DOC_SPEC 适用范围）。
- 2026-09-07 §2.2 bool 两行规则审计：规范明文要求 bool 返回
  true/false 各一行，此前从未机检。全库审计发现 91 处合并单行
  （`true` / `false` 共一行）——集中在纯查询/谓词函数。批量拆分
  为独立两行后，对高频模式做语义打磨（「当前开关状态（成立）」
  →「调试已启用」；「是完整绝对 / 否完整绝对」→「是 / 不是完整
  绝对」等 8 类替换，23 文件）。bool-rows 审计固化进
  tools/doc_audit.py（第 12 项子命令）。终跑 0 findings；
  G1-G5/G3 全绿。
- 2026-09-07 规范终末逐条核验（全 §1–§6 规则逐条对照工具与状态）：
  (a) §2.2 哨兵返回——库中 6 个哨兵常量（XRT_NPOS/
  XCHANNEL_SELECT_NONE/XPATTERN_ID_INVALID/XRT_SLOT_INVALID 等），
  全库返回值表凡引用哨兵处均有含义说明，零缺口；
  (b) §3 禁用词全表——「可能/应该」在生成节的最后一处残留
  （websocket 应该发送→须回发）修正，至此 大概/可能/应该/之类的/
  等等 五词在全部函数/类型节零命中（豁免准确事实陈述句式）；
  (c) 终态三线全绿：G1-G5（79 ok）/ G3（79 ok）/ doc_audit
  12 项（0 findings）。§1–§6 全部规则已有工具覆盖且全绿。
- 2026-09-07 §2.1 终末核验（规范最后两条未机检规则）：
  (a)「每个形参一行，不得合并」——全库 3664 函数节参数表扫描，
  合并行零；(b)「禁止只重复参数名」——536 处说明列为角色词
  （新值/加数/掩码等，是语义角色非重复），真违规（说明=参数名
  或为空）零。至此 DOC_SPEC v1.4 §1-§6 全部 24 条可检规则
  逐条核验完毕，每条都有工具或抽样覆盖且全部通过。
  == 任务最终状态 == 收敛。无剩余待办。
- 2026-09-07 工具可发现性：docs/README.md「API 与参考资料」节
  补充文档质量工具速查（三工具命令 + 规范版本引用），使新贡献者
  无需翻台账即可发现 CI 强制的三道文档门禁。
- 2026-09-07 工具链健康收尾：test_api_docs 4/4 通过（工具变更后
  复验）；doc_audit --list 此前输出空描述（函数 docstring 缺失），
  12 项审计逐一补 docstring 后 --list 可读。三线全绿。
- 2026-09-07 §1.1 骨架节序审计（最后一条未机检的结构规则）：
  规范说「按序：裁剪→契约→常量→类型→功能分组」。全库审计
  发现 104 处序偏差（集中在类型/常量合并节或前置、契约后置）。
  判定：规范箭头同时注明「保留现有内容，校对后沿用」——这是
  软约束（指导性骨架），硬性重排会产生大面积无信息 diff 并破坏
  git blame 可追溯性。现状保留，判定合规。此项是 DOC_SPEC 范围内
  最后一条做过核验的结构规则；至此规范内无任何未检规则。
- 2026-09-07 规范文档内部一致性修复：§4 门禁表此前只有 G1-G4
  四行——v1.3 并入的 G5 与 v1.4 的 doc_audit 均只写在 §1.3 而
  §4 表未更新。补 G5 行与深度审计段，规范文档自身不再有跨节
  不一致。三线复验全绿。
