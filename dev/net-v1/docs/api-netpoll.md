...
> 基于 IOCP（Windows）和 io_uring（Linux）的异步 IO 抽象层，提供统一的异步接口
...
int main() {
    ...
    // 创建连接 socket
    xnetconn conn;
    xrtSockCreate(&conn, 0);  // 0 = TCP
    xrtSockSetNonBlock(&conn);
    ...
    // 添加到 poller
    xrtPollAdd(poller, &conn, XRT_POLL_READ);

    // 发送数据
    const char* msg = "Hello, Server!";
    xrtPollPostSend(poller, &conn, msg, strlen(msg));
    ...
    switch (iEvent) {
        case XRT_POLL_READ: {
            printf("Received from %s:%d: %.*s\n",
                   pConn->tRemoteAddr.sIP, pConn->tRemoteAddr.iPort,
                   (int)iLen, pData);

            // 回显数据
            xrtPollPostSend(poller, pConn, pData, iLen);

            // 继续接收
            xrtPollPostRecv(poller, pConn);
            break;
        }
        case XRT_POLL_WRITE: {
            printf("Sent: %zu bytes\n", iLen);
...
int main() {
    ...
    // 创建 UDP socket
    xnetconn conn;
    xrtSockCreate(&conn, 1);  // 1 = UDP
    xrtSockSetReuseAddr(&conn);
    xrtSockSetNonBlock(&conn);

    xnetaddr addr;
    xrtNetAddrInit(&addr, "0.0.0.0", 8080);
    xrtSockBind(&conn, &addr);

    // 添加到 poller
    xrtPollAdd(poller, &conn, XRT_POLL_READ);
    xrtPollPostRecv(poller, &conn);
...
int main() {
    ...
    // 事件循环，带超时
    while (1) {
        xnet_result result = xrtPollWait(poller, 100);  // 100ms 超时

        if (result == XRT_NET_TIMEOUT) {
            // 超时处理
            printf("Timeout, doing other work...\n");
            // 可以执行其他任务
        } else if (result == XRT_NET_ERROR) {
...
```c
// 线程函数
uint32 worker_thread(ptr param) {
    xnetpoller* poller = (xnetpoller*)param;

    while (1) {
        xnet_result result = xrtPollWait(poller, -1);  // 无限等待
...
int main() {
    ...
    // 创建工作线程
    xthread thread = xrtThreadCreate(worker_thread, poller, 0);

    // 主线程可以执行其他任务
    sleep(1);

    // 唤醒工作线程
    xrtPollWakeup(poller);
