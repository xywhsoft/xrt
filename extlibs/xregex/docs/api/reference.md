...
此文件由 `tools/generate_api_reference.py` 从 `extlibs/xregex/config/modules.json` 与公共头生成。
不要手工维护第二份符号清单。主题语义、状态机、所有权、错误和示例见
[../../README.md](../../README.md)；每个声明的精确契约以链接的公共头中文注释为准。

当前登记 `54` 个函数、`23` 个常量或宏、
`14` 个公共类型。

## `extlibs/xregex/include/xrt/regex.h`
...
- `XREGEX_FEATURE_REGEX`
- `XREGEX_FEATURE_REGEX_CORE`
- `XREGEX_FEATURE_REGEX_MATCH`
- `XREGEX_FEATURE_REGEX_REPLACE`
- `XREGEX_FEATURE_REGEX_SET`
- `XREGEX_FEATURE_REGEX_SPLIT`
- `XREGEX_IGNORE_CASE`
- `XREGEX_MATCH`
- `XREGEX_MULTILINE`
- `XREGEX_NONE`
