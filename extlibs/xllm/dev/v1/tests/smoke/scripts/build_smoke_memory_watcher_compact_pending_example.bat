...
gcc -std=c11 -Wall -Wextra -I. -Ilib -Ilib\sqlite -Ilib\onnxruntime -DXRT_IMPLEMENTATION -DXLLM_IMPLEMENTATION ^
    examples\smoke_memory_watcher_compact_pending.c ^
    lib\sqlite\sqlite3.c ^
    -o build\smoke_memory_watcher_compact_pending.exe ^
    -lws2_32 -liphlpapi -lshell32 -lcrypt32

if not "%errorlevel%"=="0" exit /b %errorlevel%

build\smoke_memory_watcher_compact_pending.exe
if not "%errorlevel%"=="0" exit /b %errorlevel%

echo smoke_memory_watcher_compact_pending ok
