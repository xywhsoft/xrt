...
gcc -std=c11 -Wall -Wextra -I. -Ilib -DXRT_IMPLEMENTATION -DXLLM_IMPLEMENTATION ^
    examples\smoke_ollama_native_tool_loop.c ^
    -o build\smoke_ollama_native_tool_loop.exe ^
    -lws2_32 -liphlpapi -lshell32

if not "%errorlevel%"=="0" exit /b %errorlevel%

build\smoke_ollama_native_tool_loop.exe
if not "%errorlevel%"=="0" exit /b %errorlevel%

echo smoke_ollama_native_tool_loop ok
