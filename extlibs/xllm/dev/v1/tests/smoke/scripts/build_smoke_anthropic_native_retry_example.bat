...
gcc -std=c11 -Wall -Wextra -I. -Ilib -DXRT_IMPLEMENTATION -DXLLM_IMPLEMENTATION ^
    examples\smoke_anthropic_native_retry.c ^
    -o build\smoke_anthropic_native_retry.exe ^
    -lws2_32 -liphlpapi -lshell32

if not "%errorlevel%"=="0" exit /b %errorlevel%

build\smoke_anthropic_native_retry.exe
if not "%errorlevel%"=="0" exit /b %errorlevel%

echo smoke_anthropic_native_retry ok
