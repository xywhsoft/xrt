...
gcc -std=c11 -Wall -Wextra -I. -Ilib -Ilib\sqlite ^
    ...
    examples\smoke_memory_crash_consistency.c ^
    lib\sqlite\sqlite3.c ^
    -o build\smoke_memory_crash_consistency.exe ^
    -lws2_32 -liphlpapi -lshell32 -lcrypt32

if not "%errorlevel%"=="0" exit /b %errorlevel%

build\smoke_memory_crash_consistency.exe
if not "%errorlevel%"=="0" exit /b %errorlevel%

echo smoke_memory_crash_consistency ok
