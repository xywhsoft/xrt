...
gcc -std=c11 -Wall -Wextra -I. -Ilib -Ilib\sqlite -Ilib\onnxruntime ^
    ...
    examples\smoke_memory_workspace_gitignore.c ^
    lib\sqlite\sqlite3.c ^
    -o build\smoke_memory_workspace_gitignore.exe ^
    -lws2_32 -liphlpapi -lshell32 -lcrypt32

if not "%errorlevel%"=="0" exit /b %errorlevel%

build\smoke_memory_workspace_gitignore.exe
if not "%errorlevel%"=="0" exit /b %errorlevel%

echo Built and ran build\smoke_memory_workspace_gitignore.exe
