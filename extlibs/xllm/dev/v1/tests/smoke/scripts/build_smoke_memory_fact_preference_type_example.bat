...
gcc -std=c11 -Wall -Wextra -I. -Ilib -Ilib\sqlite ^
    ...
    examples\smoke_memory_fact_preference_type.c ^
    lib\sqlite\sqlite3.c ^
    -o build\smoke_memory_fact_preference_type.exe ^
    -lws2_32 -liphlpapi -lshell32 -lcrypt32

if not "%errorlevel%"=="0" exit /b %errorlevel%

build\smoke_memory_fact_preference_type.exe
if not "%errorlevel%"=="0" exit /b %errorlevel%

echo smoke_memory_fact_preference_type ok
