...
gcc -std=c11 -Wall -Wextra -I. -Ilib -DXRT_IMPLEMENTATION -DXLLM_IMPLEMENTATION ^
    examples\smoke_openai_compat_file_url.c ^
    -o build\smoke_openai_compat_file_url.exe ^
    -lws2_32 -liphlpapi -lshell32

if not "%errorlevel%"=="0" exit /b %errorlevel%

build\smoke_openai_compat_file_url.exe
if not "%errorlevel%"=="0" exit /b %errorlevel%

echo smoke_openai_compat_file_url ok
