...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"msg-anthropic-output-artifacts\","
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-output-artifacts\","
        "\"content\":["
        "{\"type\":\"text\",\"text\":\"probe anthropic output artifacts ok\"},"
        "{\"type\":\"image\",\"name\":\"probe.png\",\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/probe.png\",\"media_type\":\"image/png\"}},"
        "{\"type\":\"document\",\"name\":\"probe.pdf\",\"source\":{\"type\":\"base64\",\"media_type\":\"application/pdf\",\"data\":\"cHJvYmUtYW50aHJvcGljLXJlcG9ydA==\"}}"
        "],"
    ...
    iRespLen = snprintf(
        ...
        "request-id: req-anthropic-output-artifacts\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sSummaryPath = "build\\probe_anthropic_output_artifacts.summary.json";
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_MODEL", "claude-output-artifacts") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic output artifacts ok") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ) {
        fprintf(stderr, "unexpected request count: %d\n", tServer.iAcceptCount);
        iExitCode = 9;
        goto cleanup;
    }
    if ( !smoke_file_contains(sSummaryPath, "\"image_part_count\":1") ||
        ...
        fprintf(stderr, "summary file missing artifact counters\n");
        iExitCode = 10;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_anthropic_output_artifacts ok\n");
