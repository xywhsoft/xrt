    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRound == 0 ) {
            sBody =
                "{"
                "\"model\":\"llama-mock\","
                "\"message\":{\"role\":\"assistant\",\"tool_calls\":[{\"function\":{\"name\":\"get_weather\",\"arguments\":{\"city\":\"Shanghai\"}}}]},"
                "\"done\":true,"
                "\"done_reason\":\"stop\","
            ...
            sBody =
                "{"
                "\"model\":\"llama-mock\","
                "\"message\":{\"role\":\"assistant\",\"content\":\"weather result: sunny\"},"
                "\"done\":true,"
                "\"done_reason\":\"stop\","
...
int main(void)
{
    ...
    tProfile.sId = "ollama-tool-local";
    ...
    tCreate.sInitialProfileId = "ollama-tool-local";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "hello ollama tool") != XRT_NET_OK ) {
    ...
    if ( tServer.iAssistantToolCallCount != 1 || tServer.iToolResultCount != 1 ) {
        fprintf(stderr, "unexpected tool loop body counts: assistant=%d tool_result=%d\n", tServer.iAssistantToolCallCount, tServer.iToolResultCount);
        return 19;
    }
    ...
    if ( tLog.iOllamaLogCount < 2 || tLog.iToolLoopLogCount < 3 ) {
