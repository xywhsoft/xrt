...
typedef struct {
    ...
    int iFilteredResponseTraceCount;
} demo_trace_state;
...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sHeader =
        ...
        "x-request-id: req_openai_stream_content_filter_1\r\n"
        "\r\n";
    const char *sEvent1 =
        "data: {\"id\":\"chatcmpl-stream-filter\",\"object\":\"chat.completion.chunk\",\"model\":\"gpt-filter-mock\","
...
static void demo_trace_callback(void *pCtx, xllm_trace_kind eKind, const xvalue *pPayload)
{
    ...
    if ( eKind == XLLM_TRACE_REQUEST && sPhase && strcmp(sPhase, "request") == 0 ) {
        ...
        if ( sResponseStatus && strcmp(sResponseStatus, "content_filtered") == 0 ) {
            ++pState->iFilteredResponseTraceCount;
        }
    }
}
...
int main(void)
{
    ...
    tProfile.sId = "openai-stream-content-filter";
    ...
    tProfile.tModels.tText.sModelId = "gpt-filter-mock";
    ...
    tCreate.sInitialProfileId = "openai-stream-content-filter";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "unsafe stream prompt") != XRT_NET_OK ) {
    ...
    if ( !pResponse->tSafety.sBlockReason || strcmp(pResponse->tSafety.sBlockReason, "content_filter") != 0 ) {
        fprintf(stderr, "unexpected safety block reason\n");
        iStatus = 15;
        goto cleanup;
    }
    if ( !xllm_response_get_text(pResponse) || strcmp(xllm_response_get_text(pResponse), "partial answer") != 0 ) {
    ...
    if ( !pOutput || pOutput->eKind != XLLM_OUTPUT_MESSAGE ) {
        fprintf(stderr, "unexpected output kind\n");
        iStatus = 18;
        goto cleanup;
    }
    if ( tEvents.iTextDeltaCount != 2 || tEvents.iEndCount != 1 ) {
    ...
    if ( tTrace.iRequestTraceCount < 1 || tTrace.iStreamFinalizeCount < 1 || tTrace.iFilteredResponseTraceCount < 1 ) {
        fprintf(stderr, "unexpected trace counts request=%d finalize=%d filtered=%d\n",
            tTrace.iRequestTraceCount,
            tTrace.iStreamFinalizeCount,
            tTrace.iFilteredResponseTraceCount);
        iStatus = 21;
        goto cleanup;
    }
