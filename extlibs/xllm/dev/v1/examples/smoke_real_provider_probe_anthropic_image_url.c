...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"msg_probe_image_url\","
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-test-mm\","
        "\"content\":[{\"type\":\"text\",\"text\":\"probe anthropic image url ok\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/anthropic-probe.png\"}") != NULL ) {
            ++pState->iUrlSourceCount;
        }
        if ( strstr(aBuffer, "https://example.invalid/anthropic-probe.png") != NULL ) {
            ++pState->iImageUrlCount;
        }
        if ( strstr(aBuffer, "\"describe anthropic probe image url\"") != NULL ) {
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_probe_anthropic_image_url_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "describe anthropic probe image url") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic image url ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_IMAGE_URL", "https://example.invalid/anthropic-probe.png") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected anthropic image url probe counters: accept=%d image=%d url_source=%d image_url=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_anthropic_image_url ok\n");
