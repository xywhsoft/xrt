...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("OPENAI_MODEL", "gpt-test") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 || tServer.iAuthCount != 1 ) {
        fprintf(
            stderr,
            "unexpected request counters: accept=%d auth=%d\n",
    ...
    printf("smoke_real_provider_probe_openai_alias_envs ok\n");
