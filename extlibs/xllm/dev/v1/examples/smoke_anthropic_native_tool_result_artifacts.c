...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRecv > 0 ) {
            ...
            if ( iRound == 0 ) {
                ...
                if ( strstr(aBuffer, "\"type\":\"tool_use\"") != NULL &&
                     strstr(aBuffer, "\"id\":\"toolu_weather_artifacts_1\"") != NULL &&
                     strstr(aBuffer, "\"name\":\"get_weather\"") != NULL ) {
                    ++pState->iAssistantToolUseCount;
                }
                if ( strstr(aBuffer, "\"type\":\"tool_result\"") != NULL &&
                     strstr(aBuffer, "\"tool_use_id\":\"toolu_weather_artifacts_1\"") != NULL &&
                ...
                if ( strstr(aBuffer, "\"type\":\"image\",\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/weather-map.png\"}}") != NULL ) {
                    ++pState->iToolResultImageCount;
                }
                if ( strstr(aBuffer, "\"type\":\"document\",\"source\":{\"type\":\"file\",\"file_id\":\"file_doc_123\"}}") != NULL ) {
        ...
        if ( iRound == 0 ) {
            sBody =
                "{\"id\":\"msg_tool_artifacts\","
                "\"type\":\"message\","
                "\"role\":\"assistant\","
                "\"model\":\"claude-mock\","
                "\"content\":[{\"type\":\"tool_use\",\"id\":\"toolu_weather_artifacts_1\",\"name\":\"get_weather\",\"input\":{\"city\":\"Shanghai\"}}],"
                "\"stop_reason\":\"tool_use\","
                "\"usage\":{\"input_tokens\":11,\"output_tokens\":2}}";
        } else {
            sBody =
                "{\"id\":\"msg_final_artifacts\","
                "\"type\":\"message\","
                "\"role\":\"assistant\","
                "\"model\":\"claude-mock\","
                "\"content\":[{\"type\":\"text\",\"text\":\"weather result: artifact tool ok\"}],"
        ...
        iRespLen = snprintf(
            ...
            "request-id: req_anthropic_tool_artifacts_%d\r\n"
            "Connection: close\r\n"
            "Content-Length: %u\r\n"
            "\r\n"
            "%s",
...
)
{
    ...
    pResult->pParts[1].as.tSource.as.sUrl = demo_dupstr("https://example.invalid/weather-map.png");
    ...
    pResult->pParts[2].as.tSource.as.sFileId = demo_dupstr("file_doc_123");
...
int main(void)
{
    ...
    tProfile.sId = "anthropic-tool-artifacts";
    ...
    tCreate.sInitialProfileId = "anthropic-tool-artifacts";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "hello anthropic artifact tool") != XRT_NET_OK ) {
    ...
    if ( tServer.iToolDefCount != 1 ) {
        fprintf(stderr, "unexpected tool request counts: defs=%d\n", tServer.iToolDefCount);
        return 17;
    }
    if ( tServer.iAssistantToolUseCount != 1 ||
