...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    while ( iHandled < 2 ) {
        ...
        if ( strstr(aBuffer, "GET /assets/ollama-stream-probe.png HTTP/1.1") != NULL ) {
            char sHeader[256];
            int iHeaderLen;
            ...
            const char *sBody =
                "{\"model\":\"llava-probe\",\"message\":{\"role\":\"assistant\",\"content\":\"probe ollama stream multimodal ok\"},\"done\":false}\n"
                "{\"model\":\"llava-probe\",\"message\":{\"role\":\"assistant\",\"content\":\"\"},\"done\":true,\"done_reason\":\"stop\",\"prompt_eval_count\":11,\"eval_count\":4}\n";
            char sResponse[4096];
            int iRespLen;
            ...
            if ( strstr(aBuffer, "\"describe this ollama stream probe image\"") != NULL ) {
                ++pState->iPromptCount;
            }
...
int main(void)
{
    ...
    snprintf(sImageUrl, sizeof(sImageUrl), "http://127.0.0.1:%u/assets/ollama-stream-probe.png", (unsigned)tServer.uPort);
    snprintf(sSummaryPath, sizeof(sSummaryPath), "build\\probe_ollama_stream_multimodal.summary.json");
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "ollama_native") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_BASE_URL", sBaseUrl) != 0 ||
         smoke_setenv_cstr("XLLM_REAL_MODEL", "llama-probe") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe ollama stream multimodal ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_IMAGE_URL", sImageUrl) != 0 ||
         smoke_setenv_cstr("XLLM_REAL_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "describe this ollama stream probe image") != 0 ||
    ...
    if ( tServer.iAcceptCount != 2 ||
        ...
        fprintf(
            stderr,
            "unexpected ollama stream multimodal probe counters: accept=%d image_get=%d api=%d stream=%d images=%d base64=%d prompt=%d\n",
            tServer.iAcceptCount,
            tServer.iImageGetCount,
            tServer.iApiChatCount,
            tServer.iStreamTrueCount,
            tServer.iImageArrayCount,
            tServer.iBase64Count,
            tServer.iPromptCount
        );
        iExitCode = 9;
        goto cleanup;
    }
    ...
    {
        FILE *pFile = fopen(sSummaryPath, "rb");
        char aSummary[8192];
        size_t iRead = 0u;

        if ( !pFile ) {
            fprintf(stderr, "open summary failed\n");
            iExitCode = 10;
            goto cleanup;
        }
        iRead = fread(aSummary, 1u, sizeof(aSummary) - 1u, pFile);
        fclose(pFile);
        aSummary[iRead] = '\0';

        if ( strstr(aSummary, "\"stream_mode\":3") == NULL ||
             strstr(aSummary, "\"multimodal\":true") == NULL ||
             strstr(aSummary, "\"event_text_delta_count\":1") == NULL ||
             strstr(aSummary, "\"status\":\"completed\"") == NULL ) {
            fprintf(stderr, "unexpected summary: %s\n", aSummary);
            iExitCode = 11;
            goto cleanup;
        }
    }

    printf("smoke_real_provider_probe_ollama_stream_multimodal ok\n");
