    ...
    while ( iHandled < 2 ) {
        ...
        if ( strstr(aBuffer, "GET /assets/ollama-probe.png HTTP/1.1") != NULL ) {
            ...
            const char *sBody =
                ...
                "\"message\":{\"role\":\"assistant\",\"content\":\"probe ollama image url ok\"},"
                "\"done\":true,"
                "\"done_reason\":\"stop\","
                "\"prompt_eval_count\":9,"
                "\"eval_count\":4"
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "ollama_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe ollama image url ok") != 0 ||
    ...
    if ( tServer.iAcceptCount != 2 ||
        ...
        fprintf(
            stderr,
            "unexpected ollama image-url probe counters: accept=%d get=%d chat=%d image_array=%d base64=%d prompt=%d\n",
    ...
    printf("smoke_real_provider_probe_ollama_image_url ok\n");
