...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-file-file-id\","
        "\"object\":\"chat.completion\","
        "\"model\":\"gpt-mock-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"file document ok\"},\"finish_reason\":\"stop\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"type\":\"file\",\"file\":{\"file_id\":\"file-doc-123\"}}") != NULL ) {
            ++pState->iFilePartCount;
        }
        if ( strstr(aBuffer, "\"file_id\":\"file-doc-123\"") != NULL ) {
            ++pState->iFileIdCount;
        }
        ...
        if ( strstr(aBuffer, "\"summarize this uploaded file\"") != NULL ) {
...
int main(void)
{
    ...
    tProfile.sId = "openai-file-file-id";
    ...
    tCreate.sInitialProfileId = "openai-file-file-id";
    tCreate.sSystemPrompt = "you are a file mock";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "summarize this uploaded file") != XRT_NET_OK ) {
        fprintf(stderr, "turn add text failed\n");
        return 11;
    }
    if ( xllm_turn_add_file_file_id(&tTurn, "file-doc-123", "application/pdf") != XRT_NET_OK ) {
    ...
    if ( !sText || strcmp(sText, "file document ok") != 0 ) {
    ...
    if ( tServer.iRequestCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected file-id file counters: req=%d model=%d content=%d file_part=%d file_id=%d image_url=%d text_type=%d text_value=%d\n",
