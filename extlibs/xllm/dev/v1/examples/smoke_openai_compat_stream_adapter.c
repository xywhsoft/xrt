...
static uint32 demo_stream_server_thread(ptr pParam)
{
    ...
    const char *sHeader =
        ...
        "\r\n";
    const char *sEvent1 =
        "data: {\"id\":\"chatcmpl-stream\",\"object\":\"chat.completion.chunk\",\"model\":\"gpt-mock\","
...
int main(void)
{
    ...
    tProfile.sId = "openai-stream-local";
    ...
    tCreate.sInitialProfileId = "openai-stream-local";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "hello stream") != XRT_NET_OK ) {
    ...
    if ( !sText || strcmp(sText, "echo: hello stream") != 0 ) {
    ...
    if ( pResponse->tUsage.uInputTokens != 8u || pResponse->tUsage.uOutputTokens != 4u ) {
