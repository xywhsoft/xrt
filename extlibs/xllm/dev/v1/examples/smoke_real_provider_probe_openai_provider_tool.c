...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-provider-tool-probe\","
        "\"object\":\"chat.completion\","
        "\"model\":\"gpt-test\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe openai provider tool ok\"},\"finish_reason\":\"stop\"}],"
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_PROVIDER_TOOL_JSON", "{\"type\":\"web_search_preview\"}") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe openai provider tool ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected openai provider tool counters: accept=%d tools=%d type=%d\n",
    ...
    printf("smoke_real_provider_probe_openai_provider_tool ok\n");
