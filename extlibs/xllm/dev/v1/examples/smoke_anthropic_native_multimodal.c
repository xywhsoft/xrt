...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"media_type\":\"image/png\"") != NULL ) {
        ...
        if ( strstr(aBuffer, "\"describe anthropic image\"") != NULL ) {
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_anthropic_mm_123\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    tCreate.sInitialProfileId = "anthropic-mm";
    tCreate.sSystemPrompt = "you are an anthropic multimodal mock";
    ...
    (void)snprintf(sImagePath, sizeof(sImagePath), "build\\smoke_anthropic_mm_image.bin");
    ...
    if ( xllm_turn_add_user_text(&tTurn, "describe anthropic image") != XRT_NET_OK ) {
    ...
    if ( !sText || strcmp(sText, "anthropic image ok") != 0 ) {
    ...
    if ( tServer.iApiKeyCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected multimodal counters: key=%d version=%d model=%d content=%d image=%d media=%d base64=%d text_block=%d text_value=%d\n",
