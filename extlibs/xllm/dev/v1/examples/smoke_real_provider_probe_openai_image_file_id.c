...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-openai-probe-image-file-id\","
        "\"object\":\"chat.completion\","
        "\"model\":\"gpt-test-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe openai image file id ok\"},\"finish_reason\":\"stop\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"describe this probe image file id\"") != NULL ) {
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe openai image file id ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_IMAGE_FILE_ID", "file-image-123") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "describe this probe image file id") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected openai image-file-id probe counters: accept=%d file_part=%d file_id=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_openai_image_file_id ok\n");
