...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"type\":\"image_url\"") != NULL ) {
            pState->iImagePartCount += 2;
        }
        if ( strstr(aBuffer, "https://example.invalid/cat.png") != NULL ) {
            ++pState->iRemoteUrlCount;
        }
        if ( strstr(aBuffer, "data:image/png;base64,iVBORw0KGgo=") != NULL ) {
            ++pState->iDataUrlCount;
        }
        if ( strstr(aBuffer, "\"type\":\"text\"") != NULL ) {
            ++pState->iTextTypeCount;
        }
        if ( strstr(aBuffer, "\"describe this image\"") != NULL ) {
...
int main(void)
{
    ...
    tCreate.sInitialProfileId = "openai-mm";
    ...
    (void)snprintf(sImagePath, sizeof(sImagePath), "build\\smoke_openai_mm_image.bin");
    ...
    if ( xllm_turn_add_user_text(&tTurn, "describe this image") != XRT_NET_OK ) {
        fprintf(stderr, "turn add text failed\n");
        return 12;
    }
    if ( xllm_turn_add_image_url(&tTurn, "https://example.invalid/cat.png", "image/png") != XRT_NET_OK ) {
    ...
    if ( !sText || strcmp(sText, "image ok") != 0 ) {
    ...
    if ( tServer.iRequestCount != 1 ||
         ...
         tServer.iImagePartCount != 2 ||
         tServer.iRemoteUrlCount != 1 ||
         tServer.iDataUrlCount != 1 ||
         tServer.iTextTypeCount != 1 ||
         tServer.iTextValueCount < 1 ) {
        fprintf(
            stderr,
            "unexpected multimodal counters: req=%d model=%d content=%d image=%d remote=%d data=%d text_type=%d text_value=%d\n",
            tServer.iRequestCount,
            tServer.iMultimodalModelCount,
            tServer.iContentArrayCount,
            tServer.iImagePartCount,
            tServer.iRemoteUrlCount,
