...
typedef struct {
    ...
    uint16 uPort;
    int iAcceptCount;
    int iApiKeyHeaderCount;
    int iAuthorizationHeaderCount;
} smoke_server_state;
...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-openai-api-key-header-alias\","
    ...
    if ( iRecv > 0 ) {
        aBuffer[iRecv] = '\0';
        if ( strstr(aBuffer, "x-api-key: alias-openai-header-key\r\n") != NULL ||
             strstr(aBuffer, "X-Api-Key: alias-openai-header-key\r\n") != NULL ||
             strstr(aBuffer, "X-API-Key: alias-openai-header-key\r\n") != NULL ||
             strstr(aBuffer, "X-API-KEY: alias-openai-header-key\r\n") != NULL ||
             strstr(aBuffer, "X-Api-key: alias-openai-header-key\r\n") != NULL ) {
            ++pState->iApiKeyHeaderCount;
        }
        if ( strstr(aBuffer, "Authorization: ") != NULL ) {
            ++pState->iAuthorizationHeaderCount;
        }
    }
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("OPENAI_MODEL", "openai-gpt-test") != 0 ||
         smoke_setenv_cstr("OPENAI_API_KEY", "alias-openai-header-key") != 0 ||
         smoke_setenv_cstr("OPENAI_AUTH_KIND", "api_key_header") != 0 ||
         smoke_setenv_cstr("OPENAI_AUTH_HEADER", "x-api-key") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ||
         tServer.iApiKeyHeaderCount != 1 ||
         tServer.iAuthorizationHeaderCount != 0 ) {
        fprintf(
            stderr,
            "unexpected request counters: accept=%d api_key_header=%d authorization=%d\n",
            tServer.iAcceptCount,
            tServer.iApiKeyHeaderCount,
            tServer.iAuthorizationHeaderCount
        );
        iExitCode = 9;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_openai_api_key_header_alias_envs ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("OPENAI_AUTH_KIND", "");
    smoke_setenv_cstr("OPENAI_AUTH_HEADER", "");
