...
static void demo_http_error_trace_callback(void *pCtx, xllm_trace_kind eKind, const xvalue *pPayload)
{
    ...
    if ( sRequestId && strcmp(sRequestId, "req-rate-limit-1") == 0 ) {
...
int main(void)
{
    ...
    if ( xllm_turn_add_user_text(&tTurn, "trigger rate limit") != XRT_NET_OK ) {
    ...
    if ( tTrace.iRequestTraceCount < 1 ) {
        fprintf(stderr, "missing request trace\n");
        return 17;
    }
    if ( tTrace.iFailedResponseTraceCount < 1 ||
        ...
        fprintf(
            stderr,
            "unexpected failure trace counts: failed=%d rate=%d http429=%d reqid=%d retryable=%d\n",
    ...
    if ( tLog.iOpenAILogCount < 2 || tLog.iWarnLogCount < 1 ) {
        fprintf(stderr, "unexpected openai log counts: total=%d warn=%d\n", tLog.iOpenAILogCount, tLog.iWarnLogCount);
        return 19;
    }

    printf("ok: saw expected rate limit failure\n");
