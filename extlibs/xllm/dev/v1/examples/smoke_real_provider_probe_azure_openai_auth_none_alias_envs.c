...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-azure-openai-auth-none\","
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("AZURE_OPENAI_AUTH_KIND", "none") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 || tServer.iAuthCount != 0 ) {
        fprintf(
            stderr,
