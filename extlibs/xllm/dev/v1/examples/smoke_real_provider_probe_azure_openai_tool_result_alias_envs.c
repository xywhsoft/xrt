...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRecv > 0 ) {
            aBuffer[iRecv] = '\0';
            if ( strstr(aBuffer, "Authorization: Bearer alias-azure-openai-tool-result-key\r\n") != NULL ) {
                ++pState->iAuthCount;
            }
            if ( iRound == 1 ) {
                ...
                if ( strstr(aBuffer, "\"type\":\"text\",\"text\":\"probe-azure-value-alias\"") != NULL ) {
                    ++pState->iToolResultTextCount;
                }
                if ( strstr(aBuffer, "\"type\":\"image_url\",\"image_url\":{\"url\":\"https://example.invalid/probe-azure-openai-tool-image-alias.png\"}}") != NULL ) {
                    ++pState->iToolResultImageCount;
                }
                if ( strstr(aBuffer, "\"type\":\"file\",\"file\":{\"file_id\":\"file_doc_probe_azure_openai_tool_alias\"}}") != NULL ) {
        ...
        if ( iRound == 0 ) {
            sBody =
                "{\"id\":\"chatcmpl-azure-openai-tool-alias-1\","
                "\"object\":\"chat.completion\","
                "\"model\":\"azure-gpt-probe-alias\","
            ...
            sBody =
                "{\"id\":\"chatcmpl-azure-openai-tool-alias-2\","
                "\"object\":\"chat.completion\","
                "\"model\":\"azure-gpt-probe-alias\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe azure openai tool result alias env ok\"},\"finish_reason\":\"stop\"}],"
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sSummaryPath = "build\\probe_azure_openai_tool_result_alias_envs.summary.json";
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("AZURE_OPENAI_DEPLOYMENT", "azure-gpt-probe-alias") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_API_KEY", "alias-azure-openai-tool-result-key") != 0 ||
         ...
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_TEXT", "probe-azure-value-alias") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_IMAGE_URL", "https://example.invalid/probe-azure-openai-tool-image-alias.png") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_FILE_FILE_ID", "file_doc_probe_azure_openai_tool_alias") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_FILE_MIME", "application/pdf") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_FILE_NAME", "probe-azure-openai-tool-alias.pdf") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_STATUS", "completed") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe azure openai tool result alias env ok") != 0 ||
    ...
    if ( !smoke_file_contains(sSummaryPath, "\"status\":\"completed\"") ||
         !smoke_file_contains(sSummaryPath, "\"visible_text\":\"probe azure openai tool result alias env ok\"") ) {
        fprintf(stderr, "unexpected summary file contents\n");
        iExitCode = 10;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_azure_openai_tool_result_alias_envs ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_TEXT", "");
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_IMAGE_URL", "");
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_IMAGE_MIME", "");
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_FILE_FILE_ID", "");
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_FILE_MIME", "");
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_FILE_NAME", "");
