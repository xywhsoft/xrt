...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    while ( iHandled < 2 ) {
        ...
        if ( iHandled == 0 ) {
            const char *sBody =
                "{\"model\":\"llava-session-output\",\"message\":{\"role\":\"assistant\",\"content\":\"first ollama streamed artifact reply\",\"images\":[\"iVBORw0KGgo=\"]},\"done\":false}\n"
                "{\"model\":\"llava-session-output\",\"message\":{\"role\":\"assistant\",\"content\":\"\"},\"done\":true,\"done_reason\":\"stop\",\"prompt_eval_count\":9,\"eval_count\":4}\n";
            ...
            const char *sBody =
                "{\"model\":\"llava-session-output\",\"message\":{\"role\":\"assistant\",\"content\":\"ollama artifact follow-up ok\"},\"done\":true,\"done_reason\":\"stop\",\"prompt_eval_count\":13,\"eval_count\":3}\n";

            if ( strstr(aBuffer, "\"role\":\"assistant\"") != NULL &&
                 strstr(aBuffer, "\"content\":\"first ollama streamed artifact reply\"") != NULL &&
...
int main(void)
{
    ...
    tProfile.sId = "ollama-session-stream-output-artifacts";
    ...
    tProfile.tModels.tMultimodal.sModelId = "llava-session-output";
    ...
    tSessionOptions.sProfileId = "ollama-session-stream-output-artifacts";
    ...
    if ( !xllm_response_get_text(pResponse) ||
         strcmp(xllm_response_get_text(pResponse), "first ollama streamed artifact reply") != 0 ) {
    ...
    if ( !xllm_response_get_text(pResponse) ||
         strcmp(xllm_response_get_text(pResponse), "ollama artifact follow-up ok") != 0 ) {
    ...
    printf("smoke_ollama_native_session_stream_output_artifacts ok\n");
