...
static uint32 demo_retry_server_thread(ptr pParam)
{
    ...
    for ( iAttempt = 0; iAttempt < 2; ++iAttempt ) {
        ...
        if ( iAttempt == 0 ) {
            const char *sBody = "{\"error\":\"rate limited\"}";
            char sResponse[4096];
            ...
            const char *sBody =
                "{"
                "\"model\":\"llama-mock\","
                "\"message\":{\"role\":\"assistant\",\"content\":\"echo: after retry\"},"
                "\"done\":true,"
                "\"done_reason\":\"stop\","
                "\"prompt_eval_count\":9,"
                "\"eval_count\":5"
...
static void demo_retry_trace_callback(void *pCtx, xllm_trace_kind eKind, const xvalue *pPayload)
{
    ...
    bSuccess = xvoTableGetBool(*pPayload, (str)"success", 0u);
    if ( !bSuccess || (sStatus && strcmp(sStatus, "errored") == 0) ) {
...
int main(void)
{
    ...
    tCreate.sInitialProfileId = "ollama-retry";
    ...
    if ( tServer.iAttemptCount != 2 || tServer.iApiChatCount != 2 || tServer.iStreamFalseCount != 2 ) {
        fprintf(
            stderr,
            "unexpected server counters: attempts=%d api=%d stream_false=%d\n",
    ...
    if ( tTrace.iRequestTraceCount < 2 ||
        ...
        fprintf(
            stderr,
            "unexpected trace counters: request=%d failed=%d completed=%d retryable=%d attempt1=%d attempt2=%d\n",
    ...
    if ( tLog.iOllamaLogCount < 2 || tLog.iWarnLogCount < 1 || tLog.iInfoLogCount < 1 ) {
        fprintf(
            stderr,
            "unexpected log counters: total=%d warn=%d info=%d\n",
