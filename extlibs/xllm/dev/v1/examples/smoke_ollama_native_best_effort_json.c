...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        "{"
        "\"model\":\"llama-mock\","
        "\"message\":{\"role\":\"assistant\",\"content\":\"Wrapped JSON:\\n```json\\n{\\\"value\\\":42}\\n```\"},"
        "\"done\":true,"
        "\"done_reason\":\"stop\","
        "\"prompt_eval_count\":8,"
        "\"eval_count\":5"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"format\":") != NULL || strstr(aBuffer, "\"response_format\":") != NULL ) {
            ++pState->iUnexpectedFormatCount;
        }
    }
...
int main(void)
{
    ...
    tProfile.sId = "ollama-local-json";
    ...
    tCreate.sInitialProfileId = "ollama-local-json";
    ...
    if ( tServer.iRequestCount != 1 || tServer.iUnexpectedFormatCount != 0 ) {
        fprintf(
            stderr,
            "unexpected request counters: request=%d native_format=%d\n",
            tServer.iRequestCount,
            tServer.iUnexpectedFormatCount
        );
        return 16;
    }
