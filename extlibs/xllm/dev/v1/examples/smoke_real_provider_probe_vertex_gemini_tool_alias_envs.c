...
} smoke_vertex_gemini_tool_server_state;

static bool smoke_vertex_gemini_tool_send_all(SOCKET hSocket, const char *sData, size_t iLen)
{
    size_t iSent = 0u;
...
static uint32 smoke_vertex_gemini_tool_server_thread(ptr pParam)
{
    smoke_vertex_gemini_tool_server_state *pState = (smoke_vertex_gemini_tool_server_state *)pParam;
    int iRound;
    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRecv > 0 ) {
            aBuffer[iRecv] = '\0';
            if ( strstr(aBuffer, "x-goog-api-key: alias-vertex-gemini-tool-key\r\n") != NULL ) {
                ++pState->iAuthCount;
            }
            if ( strstr(aBuffer, "POST /v1/projects/demo/locations/us-central1/publishers/google/models/vertex-gemini-test-tool:generateContent HTTP/1.1\r\n") != NULL ) {
                ++pState->iPathCount;
            }
            if ( iRound == 0 ) {
                ...
                if ( strstr(aBuffer, "\"response\":{\"text\":\"alias-vertex-gemini-tool-value\"}") != NULL ) {
        ...
        if ( iRound == 0 ) {
            ...
            sBody =
                "{\"candidates\":[{\"finishReason\":\"STOP\",\"content\":{\"role\":\"model\",\"parts\":[{\"text\":\"probe vertex gemini tool alias ok\"}]}}],"
        ...
        if ( iRespLen > 0 && !smoke_vertex_gemini_tool_send_all(hClient, sResponse, (size_t)iRespLen) ) {
            closesocket(hClient);
            closesocket(pState->hListen);
            pState->hListen = INVALID_SOCKET;
            return 3u;
...
static int smoke_vertex_gemini_tool_setenv_cstr(const char *sName, const char *sValue)
{
#if defined(_WIN32) || defined(_WIN64)
...
int main(void)
{
    ...
    WSADATA tWsaData;
#endif
    smoke_vertex_gemini_tool_server_state tServer;
    ...
    hServerThread = xrtThreadCreate((ptr)smoke_vertex_gemini_tool_server_thread, &tServer, 0);
    if ( !hServerThread ) {
    ...
    if ( smoke_vertex_gemini_tool_setenv_cstr("XLLM_REAL_ADAPTER", "vertex_gemini_native") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("VERTEX_GEMINI_BASE_URL", sBaseUrl) != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("VERTEX_GEMINI_MODEL", "vertex-gemini-test-tool") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("VERTEX_GEMINI_API_KEY", "alias-vertex-gemini-tool-key") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("VERTEX_GEMINI_TOOL_CHOICE", "named") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("VERTEX_GEMINI_TOOL_CHOICE_NAME", "get_probe_value") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("VERTEX_GEMINI_TOOL_RESULT_TEXT", "alias-vertex-gemini-tool-value") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("XLLM_REAL_ENABLE_LOG", "1") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("XLLM_REAL_ENABLE_TRACE", "1") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("XLLM_REAL_EXPECT_STATUS", "completed") != 0 ||
         smoke_vertex_gemini_tool_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe vertex gemini tool alias ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 2 ||
        ...
        fprintf(
            stderr,
            "unexpected vertex gemini tool alias counters: accept=%d auth=%d path=%d named=%d defs=%d result=%d text=%d\n",
            tServer.iAcceptCount,
            tServer.iAuthCount,
            tServer.iPathCount,
            tServer.iNamedChoiceCount,
    ...
    printf("smoke_real_provider_probe_vertex_gemini_tool_alias_envs ok\n");
