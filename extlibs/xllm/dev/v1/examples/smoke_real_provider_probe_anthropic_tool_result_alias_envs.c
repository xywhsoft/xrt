...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRecv > 0 ) {
            aBuffer[iRecv] = '\0';
            if ( strstr(aBuffer, "x-api-key: alias-anthropic-tool-result-key\r\n") != NULL ) {
                ++pState->iApiKeyCount;
            }
            ...
            if ( iRound == 1 ) {
                if ( strstr(aBuffer, "\"type\":\"tool_use\"") != NULL &&
                     strstr(aBuffer, "\"id\":\"toolu_probe_alias_result_1\"") != NULL &&
                     strstr(aBuffer, "\"name\":\"get_probe_value\"") != NULL ) {
                    ++pState->iToolUseCount;
                }
                if ( strstr(aBuffer, "\"type\":\"tool_result\"") != NULL &&
                     strstr(aBuffer, "\"tool_use_id\":\"toolu_probe_alias_result_1\"") != NULL &&
                ...
                if ( strstr(aBuffer, "\"type\":\"image\",\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/probe-anthropic-tool-image-alias.png\"}}") != NULL ) {
                    ++pState->iToolResultImageCount;
                }
                if ( strstr(aBuffer, "\"type\":\"document\",\"source\":{\"type\":\"file\",\"file_id\":\"file_doc_probe_anthropic_tool_alias\"}}") != NULL ) {
        ...
        if ( iRound == 0 ) {
            sBody =
                "{\"id\":\"msg_probe_anthropic_tool_alias_1\","
                "\"type\":\"message\","
                "\"role\":\"assistant\","
                "\"model\":\"claude-probe-alias\","
                "\"content\":[{\"type\":\"tool_use\",\"id\":\"toolu_probe_alias_result_1\",\"name\":\"get_probe_value\",\"input\":{\"input\":\"probe\"}}],"
                "\"stop_reason\":\"tool_use\","
                "\"usage\":{\"input_tokens\":9,\"output_tokens\":2}}";
        } else {
            sBody =
                "{\"id\":\"msg_probe_anthropic_tool_alias_2\","
                "\"type\":\"message\","
                "\"role\":\"assistant\","
                "\"model\":\"claude-probe-alias\","
                "\"content\":[{\"type\":\"text\",\"text\":\"probe anthropic tool result alias ok\"}],"
        ...
        iRespLen = snprintf(
            ...
            "request-id: req_probe_anthropic_tool_result_alias_%d\r\n"
            "Connection: close\r\n"
            "Content-Length: %u\r\n"
            "\r\n"
            "%s",
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sSummaryPath = "build\\probe_anthropic_tool_result_alias_envs.summary.json";
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", XLLM_ADAPTER_ANTHROPIC_NATIVE) != 0 ||
         ...
         smoke_setenv_cstr("ANTHROPIC_MULTIMODAL_MODEL", "claude-probe-alias") != 0 ||
         smoke_setenv_cstr("ANTHROPIC_API_KEY", "alias-anthropic-tool-result-key") != 0 ||
         ...
         smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_TEXT", "probe-value-alias") != 0 ||
         smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_IMAGE_URL", "https://example.invalid/probe-anthropic-tool-image-alias.png") != 0 ||
         smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_FILE_FILE_ID", "file_doc_probe_anthropic_tool_alias") != 0 ||
         smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_FILE_NAME", "probe-anthropic-tool-alias.pdf") != 0 ||
         smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_FILE_MIME", "application/pdf") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic tool result alias ok") != 0 ||
    ...
    if ( tServer.iAcceptCount != 2 ||
        ...
        fprintf(
            stderr,
            "unexpected tool result counters: accept=%d auth=%d version=%d use=%d result=%d text=%d image=%d file=%d\n",
    ...
    if ( !smoke_file_contains(sSummaryPath, "\"status\":\"completed\"") ||
         !smoke_file_contains(sSummaryPath, "\"visible_text\":\"probe anthropic tool result alias ok\"") ) {
        fprintf(stderr, "unexpected summary json\n");
        iExitCode = 10;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_anthropic_tool_result_alias_envs ok\n");

cleanup:
    smoke_setenv_cstr("XLLM_REAL_ADAPTER", "");
    smoke_setenv_cstr("ANTHROPIC_BASE_URL", "");
    smoke_setenv_cstr("ANTHROPIC_MODEL", "");
    smoke_setenv_cstr("ANTHROPIC_MULTIMODAL_MODEL", "");
    ...
    smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_IMAGE_URL", "");
    smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_IMAGE_MIME", "");
    smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_FILE_FILE_ID", "");
    smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_FILE_NAME", "");
    smoke_setenv_cstr("ANTHROPIC_TOOL_RESULT_FILE_MIME", "");
