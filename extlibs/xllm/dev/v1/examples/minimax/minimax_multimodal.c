...
{
    const char *sImageUrl = getenv("MINIMAX_IMAGE_URL");
    const char *sImagePath = getenv("MINIMAX_IMAGE_PATH");
    const char *sImageMime = demo_env("MINIMAX_IMAGE_MIME", "image/png");
    int iStatus;
...
int main(void)
{
    ...
    printf("=== MiniMax Native Multimodal Example ===\n");
    printf("base url : %s\n", sBaseUrl);
    printf("model    : %s\n\n", sModel);
    printf("Optional envs:\n");
    printf("  MINIMAX_API_KEY\n");
    printf("  MINIMAX_IMAGE_URL / MINIMAX_IMAGE_PATH\n");
    printf("  MINIMAX_TIMEOUT_MS\n");
    printf("  MINIMAX_DEBUG=1\n");
    printf("  MINIMAX_MULTIMODAL_STREAM=1\n\n");
    ...
    if ( demo_env_enabled("MINIMAX_DEBUG") ) {
    ...
    if ( getenv("MINIMAX_IMAGE_URL") && getenv("MINIMAX_IMAGE_URL")[0] ) bHasInput = true;
    if ( getenv("MINIMAX_IMAGE_PATH") && getenv("MINIMAX_IMAGE_PATH")[0] ) bHasInput = true;
    ...
    if ( !bHasInput ) {
        printf("[assistant] No image input configured; set MINIMAX_IMAGE_URL or MINIMAX_IMAGE_PATH.\n");
        xllm_destroy(pLlm);
        xllm_error_free(&tError);
        xllm_turn_reset(&tTurn);
        xllm_runtime_destroy(pRuntime);
    ...
    tCallOpts.uTimeoutMs = demo_env_u32("MINIMAX_TIMEOUT_MS", 180000u);
    if ( demo_env_enabled("MINIMAX_MULTIMODAL_STREAM") ) {
