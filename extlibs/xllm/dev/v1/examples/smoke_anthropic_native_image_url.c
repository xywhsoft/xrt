...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        ...
        "\"content\":[{\"type\":\"text\",\"text\":\"anthropic image url ok\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/anthropic-cat.png\"}") != NULL ) {
            ++pState->iUrlSourceCount;
        }
        if ( strstr(aBuffer, "https://example.invalid/anthropic-cat.png") != NULL ) {
            ++pState->iImageUrlCount;
        }
        if ( strstr(aBuffer, "\"type\":\"text\"") != NULL ) {
            ++pState->iTextBlockCount;
        }
        if ( strstr(aBuffer, "\"describe anthropic image url\"") != NULL ) {
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_anthropic_url_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    tCreate.sInitialProfileId = "anthropic-mm-url";
    tCreate.sSystemPrompt = "you are an anthropic multimodal url mock";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "describe anthropic image url") != XRT_NET_OK ) {
        fprintf(stderr, "turn add text failed\n");
        return 11;
    }
    if ( xllm_turn_add_image_url(&tTurn, "https://example.invalid/anthropic-cat.png", "image/png") != XRT_NET_OK ) {
    ...
    if ( !sText || strcmp(sText, "anthropic image url ok") != 0 ) {
    ...
    if ( tServer.iApiKeyCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected request counters: key=%d version=%d model=%d content=%d image=%d url_source=%d url=%d text_type=%d text=%d\n",
