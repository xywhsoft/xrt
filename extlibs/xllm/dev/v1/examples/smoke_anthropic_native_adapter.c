...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        ...
        "\"role\":\"assistant\","
        "\"model\":\"claude-mock\","
        "\"content\":[{\"type\":\"text\",\"text\":\"echo: hello anthropic\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"system\":\"You are anthropic test\"") != NULL ) {
            ++pState->iSystemCount;
        }
        if ( strstr(aBuffer, "\"messages\":[{\"role\":\"user\",\"content\":\"hello anthropic\"}]") != NULL ) {
            ++pState->iMessageCount;
        }
        if ( strstr(aBuffer, "\"max_tokens\":128") != NULL ) {
            ++pState->iMaxTokensCount;
        }
        if ( strstr(aBuffer, "\"stop_sequences\":[\"<END>\"]") != NULL ) {
            ++pState->iStopCount;
        }
    }
...
int main(void)
{
    ...
    tProfile.tDefaults.tGeneration.tMaxOutputTokens.iValue = 128u;
    ...
    tCreate.sSystemPrompt = "You are anthropic test";
    ...
    if ( !sText || strcmp(sText, "echo: hello anthropic") != 0 ) {
    ...
    if ( tServer.iApiKeyCount != 1 ||
         ...
         tServer.iMaxTokensCount != 1 ||
         tServer.iStopCount != 1 ) {
        fprintf(
            stderr,
            "unexpected request counters: key=%d version=%d system=%d messages=%d max_tokens=%d stop=%d\n",
            tServer.iApiKeyCount,
            tServer.iVersionCount,
            tServer.iSystemCount,
            tServer.iMessageCount,
            tServer.iMaxTokensCount,
            tServer.iStopCount
        );
        return 18;
    }
    if ( !pResponse->tEffectiveParams.tGeneration.tMaxOutputTokens.bSet ||
         pResponse->tEffectiveParams.tGeneration.tMaxOutputTokens.iValue != 128u ) {
