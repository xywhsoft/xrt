...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRecv > 0 ) {
            aBuffer[iRecv] = '\0';
            if ( strstr(aBuffer, "Authorization: Bearer alias-azure-openai-key\r\n") != NULL ) {
                ++pState->iAuthCount;
            }
            if ( iRound == 0 ) {
                ...
                if ( strstr(aBuffer, "\"name\":\"get_probe_value\"") != NULL ) {
                    ++pState->iToolDefCount;
                }
            } else {
                if ( strstr(aBuffer, "\"role\":\"tool\"") != NULL &&
                     strstr(aBuffer, "\"tool_call_id\":\"call_probe_tool_alias_1\"") != NULL ) {
                    ++pState->iToolResultCount;
                }
                if ( strstr(aBuffer, "\"content\":\"alias-azure-tool-value\"") != NULL ||
                     strstr(aBuffer, "\"type\":\"text\",\"text\":\"alias-azure-tool-value\"") != NULL ) {
        ...
        if ( iRound == 0 ) {
            sBody =
                ...
                "\"model\":\"azure-gpt-test-tool\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":null,\"tool_calls\":[{\"id\":\"call_probe_tool_alias_1\",\"type\":\"function\",\"function\":{\"name\":\"get_probe_value\",\"arguments\":\"{\\\"input\\\":\\\"probe\\\"}\"}}]},\"finish_reason\":\"tool_calls\"}],"
            ...
            sBody =
                ...
                "\"object\":\"chat.completion\","
                "\"model\":\"azure-gpt-test-tool\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe azure openai tool alias ok\"},\"finish_reason\":\"stop\"}],"
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("AZURE_OPENAI_DEPLOYMENT", "azure-gpt-test-tool") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_API_KEY", "alias-azure-openai-key") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_CHOICE", "named") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_CHOICE_NAME", "get_probe_value") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_TOOL_RESULT_TEXT", "alias-azure-tool-value") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe azure openai tool alias ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 2 ||
        ...
        fprintf(
            stderr,
            "unexpected tool alias counters: accept=%d auth=%d named=%d defs=%d result=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_azure_openai_tool_alias_envs ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_CHOICE", "");
    smoke_setenv_cstr("AZURE_OPENAI_TOOL_CHOICE_NAME", "");
