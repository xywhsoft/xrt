...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"msg_probe_image_file_id\","
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-test-mm\","
        "\"content\":[{\"type\":\"text\",\"text\":\"probe anthropic image file id ok\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"source\":{\"type\":\"file\",\"file_id\":\"file_img_123\"}") != NULL ) {
            ++pState->iFileSourceCount;
        }
        if ( strstr(aBuffer, "\"file_id\":\"file_img_123\"") != NULL ) {
            ++pState->iFileIdCount;
        }
        if ( strstr(aBuffer, "\"describe anthropic probe image file id\"") != NULL ) {
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_probe_anthropic_image_file_id_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_MODEL", "claude-test") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_MULTIMODAL_MODEL", "claude-test-mm") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_ANTHROPIC_BETA_HEADERS", "files-api-2025-04-14") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "describe anthropic probe image file id") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic image file id ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_IMAGE_FILE_ID", "file_img_123") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected anthropic image file_id probe counters: accept=%d beta=%d image=%d source=%d file_id=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_anthropic_image_file_id ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("XLLM_REAL_IMAGE_FILE_ID", "");
