...
typedef struct {
    ...
    int iVersionCount;
    int iPathCount;
} smoke_anthropic_alias_server_state;

static uint32 smoke_anthropic_alias_server_thread(ptr pParam)
{
    smoke_anthropic_alias_server_state *pState = (smoke_anthropic_alias_server_state *)pParam;
    ...
    const char *sBody =
        ...
        "\"content\":[{\"type\":\"text\",\"text\":\"probe anthropic alias ok\"}],"
...
static int smoke_anthropic_alias_setenv_cstr(const char *sName, const char *sValue)
{
#if defined(_WIN32) || defined(_WIN64)
...
int main(void)
{
    ...
    WSADATA tWsaData;
#endif
    smoke_anthropic_alias_server_state tServer;
    ...
    hServerThread = xrtThreadCreate((ptr)smoke_anthropic_alias_server_thread, &tServer, 0);
    if ( !hServerThread ) {
    ...
    if ( smoke_anthropic_alias_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         smoke_anthropic_alias_setenv_cstr("ANTHROPIC_BASE_URL", sBaseUrl) != 0 ||
         smoke_anthropic_alias_setenv_cstr("ANTHROPIC_MODEL", "claude-alias-test") != 0 ||
         smoke_anthropic_alias_setenv_cstr("ANTHROPIC_API_KEY", "alias-anthropic-key") != 0 ||
         smoke_anthropic_alias_setenv_cstr("XLLM_REAL_ANTHROPIC_VERSION", "2025-02-01") != 0 ||
         smoke_anthropic_alias_setenv_cstr("XLLM_REAL_ENABLE_LOG", "0") != 0 ||
         smoke_anthropic_alias_setenv_cstr("XLLM_REAL_ENABLE_TRACE", "0") != 0 ||
         smoke_anthropic_alias_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic alias ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected anthropic alias counters: accept=%d auth=%d version=%d path=%d\n",
    ...
    printf("smoke_real_provider_probe_anthropic_alias_envs ok\n");
