...
static uint32 demo_stream_server_thread(ptr pParam)
{
    ...
    const char *sHeader =
        ...
        "request-id: req_anthropic_stream_artifacts_123\r\n"
        "Cache-Control: no-cache\r\n"
        "Connection: close\r\n"
        "Transfer-Encoding: chunked\r\n"
        "\r\n";
    const char *sEvent1 =
        "data: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_stream_artifacts\",\"type\":\"message\",\"role\":\"assistant\",\"model\":\"claude-stream-artifacts\",\"usage\":{\"input_tokens\":11,\"output_tokens\":0}}}\n\n";
    const char *sEvent2 =
        "data: {\"type\":\"content_block_start\",\"index\":0,\"content_block\":{\"type\":\"text\",\"text\":\"Here is the streamed anthropic output.\"}}\n\n";
    const char *sEvent3 =
        "data: {\"type\":\"content_block_start\",\"index\":1,\"content_block\":{\"type\":\"image\",\"name\":\"streamed.png\",\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/anthropic-streamed.png\",\"media_type\":\"image/png\"}}}\n\n";
    const char *sEvent4 =
        "data: {\"type\":\"content_block_start\",\"index\":2,\"content_block\":{\"type\":\"document\",\"name\":\"streamed.pdf\",\"source\":{\"type\":\"base64\",\"media_type\":\"application/pdf\",\"data\":\"YW50aHJvcGljLXN0cmVhbS1yZXBvcnQ=\"}}}\n\n";
    const char *sEvent5 =
...
int main(void)
{
    ...
    tProfile.sId = "anthropic-stream-output-artifacts";
    ...
    tProfile.tModels.tText.sModelId = "claude-stream-artifacts";
    if ( xllm_register_profile(pRuntime, &tProfile) != XRT_NET_OK ) {
        fprintf(stderr, "register profile failed\n");
        return 9;
    }

    tCreate.sInitialProfileId = "anthropic-stream-output-artifacts";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "stream anthropic output artifacts") != XRT_NET_OK ) {
    ...
    if ( xllm_send(pLlm, &tTurn, &tCall, &pResponse) != XRT_NET_OK || !pResponse ) {
        fprintf(stderr, "stream request failed\n");
        iStatus = 12;
        goto cleanup;
    }

    sText = xllm_response_get_text(pResponse);
    if ( !sText || strcmp(sText, "Here is the streamed anthropic output.") != 0 ) {
    ...
    if ( pResponse->iOutputCount < 1u ) {
        fprintf(stderr, "missing outputs\n");
        iStatus = 14;
        goto cleanup;
    }
    pOutput = &pResponse->pOutputs[0];
    if ( pOutput->eKind != XLLM_OUTPUT_MESSAGE || pOutput->as.tMessage.iPartCount != 3u ) {
        fprintf(stderr, "unexpected message output structure\n");
        iStatus = 15;
        goto cleanup;
    }
    if ( pOutput->as.tMessage.pParts[0].eKind != XLLM_PART_TEXT ||
         pOutput->as.tMessage.pParts[1].eKind != XLLM_PART_IMAGE ||
         pOutput->as.tMessage.pParts[2].eKind != XLLM_PART_FILE ) {
        fprintf(stderr, "unexpected message part kinds\n");
        iStatus = 16;
        goto cleanup;
    }
    ...
    if ( tEvents.iTextDeltaCount < 1 ||
         ...
         tEvents.iEndCount != 1 ) {
        fprintf(stderr, "unexpected event counts: text=%d begin=%d chunk=%d ready=%d out_begin=%d out_end=%d end=%d\n",
    ...
    if ( tSink.iBeginCount != 2 || tSink.iWriteCount != 1 || tSink.iEndCount != 2 || tSink.iTotalBytes == 0u ) {
        fprintf(stderr, "unexpected sink counts: begin=%d write=%d end=%d bytes=%u\n",
                tSink.iBeginCount,
                tSink.iWriteCount,
                tSink.iEndCount,
    ...
    if ( tServer.iApiKeyCount != 1 || tServer.iVersionCount != 1 || tServer.iStreamRequestCount != 1 ) {
        fprintf(stderr, "unexpected server counters: key=%d version=%d stream=%d\n",
    ...
    printf("smoke_anthropic_native_stream_output_artifacts ok\n");
