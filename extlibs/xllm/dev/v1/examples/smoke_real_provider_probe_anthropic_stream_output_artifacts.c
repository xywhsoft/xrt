...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    const char *sHeader =
        ...
        "request-id: req-anthropic-probe-stream-artifacts\r\n"
        "Cache-Control: no-cache\r\n"
        "Connection: close\r\n"
        "Transfer-Encoding: chunked\r\n"
        "\r\n";
    const char *sEvent1 =
        "data: {\"type\":\"message_start\",\"message\":{\"id\":\"msg_probe_stream_artifacts\",\"type\":\"message\",\"role\":\"assistant\",\"model\":\"claude-probe-stream-artifacts\",\"usage\":{\"input_tokens\":3,\"output_tokens\":0}}}\n\n";
    const char *sEvent2 =
        "data: {\"type\":\"content_block_start\",\"index\":0,\"content_block\":{\"type\":\"text\",\"text\":\"probe anthropic stream output artifacts ok\"}}\n\n";
    const char *sEvent3 =
        "data: {\"type\":\"content_block_start\",\"index\":1,\"content_block\":{\"type\":\"image\",\"name\":\"probe-stream.png\",\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/probe-stream-anthropic.png\",\"media_type\":\"image/png\"}}}\n\n";
    const char *sEvent4 =
        "data: {\"type\":\"content_block_start\",\"index\":2,\"content_block\":{\"type\":\"document\",\"name\":\"probe-stream.pdf\",\"source\":{\"type\":\"base64\",\"media_type\":\"application/pdf\",\"data\":\"cHJvYmUtYW50aHJvcGljLXN0cmVhbS1maWxl\"}}}\n\n";
    const char *sEvent5 =
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sSummaryPath = "build\\probe_anthropic_stream_output_artifacts.summary.json";
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_MODEL", "claude-probe-stream-artifacts") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic stream output artifacts ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_IMAGE_PARTS_MIN", "1") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_FILE_PARTS_MIN", "1") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_PARTS_MIN", "2") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_BEGIN_MIN", "2") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_CHUNK_MIN", "1") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_READY_MIN", "2") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 || tServer.iStreamRequestCount != 1 ) {
        fprintf(stderr, "unexpected request counters: accept=%d stream=%d\n", tServer.iAcceptCount, tServer.iStreamRequestCount);
        iExitCode = 9;
        goto cleanup;
    }
    if ( !smoke_file_contains(sSummaryPath, "\"stream_mode\":3") ||
         !smoke_file_contains(sSummaryPath, "\"image_part_count\":1") ||
         !smoke_file_contains(sSummaryPath, "\"file_part_count\":1") ||
         !smoke_file_contains(sSummaryPath, "\"artifact_part_count\":2") ||
         !smoke_file_contains(sSummaryPath, "\"event_artifact_begin_count\":2") ||
         !smoke_file_contains(sSummaryPath, "\"event_artifact_chunk_count\":1") ||
         !smoke_file_contains(sSummaryPath, "\"event_artifact_ready_count\":2") ) {
        fprintf(stderr, "summary file missing streamed artifact counters\n");
        iExitCode = 10;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_anthropic_stream_output_artifacts ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("XLLM_REAL_EXPECT_IMAGE_PARTS_MIN", "");
    smoke_setenv_cstr("XLLM_REAL_EXPECT_FILE_PARTS_MIN", "");
    smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_PARTS_MIN", "");
    smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_BEGIN_MIN", "");
    smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_CHUNK_MIN", "");
    smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_READY_MIN", "");
