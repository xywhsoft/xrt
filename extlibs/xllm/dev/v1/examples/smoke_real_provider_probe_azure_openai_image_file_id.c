...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-azure-openai-probe-image-file-id\","
        "\"object\":\"chat.completion\","
        "\"model\":\"azure-gpt-test-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe azure openai image file id ok\"},\"finish_reason\":\"stop\"}],"
    ...
    if ( iRecv > 0 ) {
        aBuffer[iRecv] = '\0';
        if ( strstr(aBuffer, "\"type\":\"file\",\"file\":{\"file_id\":\"file-azure-image-123\"}}") != NULL ) {
            ++pState->iFilePartCount;
        }
        if ( strstr(aBuffer, "\"file_id\":\"file-azure-image-123\"") != NULL ) {
            ++pState->iFileIdCount;
        }
        if ( strstr(aBuffer, "\"describe this azure probe image file id\"") != NULL ) {
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe azure openai image file id ok") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_IMAGE_FILE_ID", "file-azure-image-123") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "describe this azure probe image file id") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected azure openai image-file-id probe counters: accept=%d file_part=%d file_id=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_azure_openai_image_file_id ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("AZURE_OPENAI_IMAGE_FILE_ID", "");
    smoke_setenv_cstr("AZURE_OPENAI_IMAGE_MIME", "");
