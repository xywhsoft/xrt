...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        ...
        "\"message\":{\"role\":\"assistant\",\"content\":\"echo: hello ollama\"},"
        "\"done\":true,"
        "\"done_reason\":\"stop\","
        "\"prompt_eval_count\":7,"
        "\"eval_count\":4"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"role\":\"system\",\"content\":\"You are ollama test\"") != NULL ) {
            ++pState->iSystemCount;
        }
        if ( strstr(aBuffer, "\"role\":\"user\",\"content\":\"hello ollama\"") != NULL ) {
            ++pState->iMessageCount;
        }
        if ( strstr(aBuffer, "\"temperature\":0.2") != NULL ) {
            ++pState->iTemperatureCount;
        }
        if ( strstr(aBuffer, "\"top_p\":0.9") != NULL ) {
            ++pState->iTopPCount;
        }
        if ( strstr(aBuffer, "\"num_predict\":64") != NULL ) {
            ++pState->iNumPredictCount;
        }
...
int main(void)
{
    ...
    tProfile.tDefaults.tGeneration.tMaxOutputTokens.iValue = 64u;
    if ( xllm_register_profile(pRuntime, &tProfile) != XRT_NET_OK ) {
        fprintf(stderr, "register profile failed\n");
        return 11;
    }

    tCreate.sInitialProfileId = "ollama-local";
    tCreate.sSystemPrompt = "You are ollama test";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "hello ollama") != XRT_NET_OK ) {
    ...
    tTurn.tGeneration.tTopP.fValue = 0.9;
    arrStop[0] = "<END>";
    if ( xllm_turn_set_stop_sequences(&tTurn, arrStop, 1u) != XRT_NET_OK ) {
        fprintf(stderr, "set stop sequences failed\n");
        return 14;
    }
    ...
    if ( pResponse->eStatus != XLLM_STATUS_COMPLETED ||
         !sText ||
         strcmp(sText, "echo: hello ollama") != 0 ) {
        fprintf(stderr, "unexpected response text: %s\n", sText ? sText : "(null)");
        return 16;
    }
    if ( tEvents.iTextDeltaCount != 1 || tEvents.iEndCount != 1 ) {
    ...
    if ( tServer.iApiChatCount != 1 ||
         ...
         tServer.iMessageCount != 1 ||
         tServer.iTemperatureCount != 1 ||
         tServer.iTopPCount != 1 ||
         tServer.iNumPredictCount != 1 ||
         tServer.iStopCount != 1 ) {
        fprintf(
            stderr,
            "unexpected request counters: api=%d stream=%d system=%d msg=%d temp=%d top_p=%d num_predict=%d stop=%d\n",
            tServer.iApiChatCount,
            tServer.iStreamFalseCount,
            tServer.iSystemCount,
            tServer.iMessageCount,
            tServer.iTemperatureCount,
            tServer.iTopPCount,
            tServer.iNumPredictCount,
            tServer.iStopCount
        );
        return 18;
    }
    if ( pResponse->tUsage.uInputTokens != 7u || pResponse->tUsage.uOutputTokens != 4u ) {
    ...
    if ( tLog.iOllamaLogCount < 2 ) {
        fprintf(stderr, "unexpected ollama log count: %d\n", tLog.iOllamaLogCount);
        return 21;
    }
