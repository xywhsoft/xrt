...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"msg_probe_tool_none\","
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-mock\","
        "\"content\":[{\"type\":\"text\",\"text\":\"tool choice none probe ok\"}],"
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_TOOL_CHOICE", "none") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "tool choice none probe ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected tool-choice counters: accept=%d tool_defs=%d tool_choice_none=%d tool_name=%d\n",
    ...
    printf("smoke_real_provider_probe_tool_choice_none ok\n");
