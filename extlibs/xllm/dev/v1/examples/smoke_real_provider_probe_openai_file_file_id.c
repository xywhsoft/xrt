...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-openai-probe-file\","
        "\"object\":\"chat.completion\","
        "\"model\":\"gpt-test-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe openai file ok\"},\"finish_reason\":\"stop\"}],"
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe openai file ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_FILE_FILE_ID", "file-doc-123") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected openai file probe counters: accept=%d file_part=%d file_id=%d image_url=%d\n",
    ...
    printf("smoke_real_provider_probe_openai_file_file_id ok\n");
