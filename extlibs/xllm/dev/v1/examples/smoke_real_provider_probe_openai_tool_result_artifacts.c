    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRecv > 0 ) {
            ...
            if ( iRound == 1 ) {
                ...
                if ( strstr(aBuffer, "\"type\":\"image_url\",\"image_url\":{\"url\":\"https://example.invalid/probe-openai-tool-image.png\"}}") != NULL ) {
                    ++pState->iToolResultImageCount;
                }
                if ( strstr(aBuffer, "\"type\":\"file\",\"file\":{\"file_id\":\"file_doc_probe_openai_tool\"}}") != NULL ) {
        ...
        if ( iRound == 0 ) {
            ...
            sBody =
                "{\"id\":\"chatcmpl-probe-tool-2\","
                "\"object\":\"chat.completion\","
                "\"model\":\"gpt-probe\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe openai tool artifacts ok\"},\"finish_reason\":\"stop\"}],"
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sSummaryPath = "build\\probe_openai_tool_result_artifacts.summary.json";
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_MODEL", "gpt-probe") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_MULTIMODAL_MODEL", "gpt-probe") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_TOOL_RESULT_IMAGE_URL", "https://example.invalid/probe-openai-tool-image.png") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_TOOL_RESULT_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_TOOL_RESULT_FILE_FILE_ID", "file_doc_probe_openai_tool") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_TOOL_RESULT_FILE_MIME", "application/pdf") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_TOOL_RESULT_FILE_NAME", "probe-openai-tool.pdf") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_STATUS", "completed") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe openai tool artifacts ok") != 0 ||
    ...
    if ( tServer.iAcceptCount != 2 ||
        ...
        fprintf(
            stderr,
            "unexpected counters: accept=%d result=%d text=%d image=%d file=%d\n",
    ...
    if ( !smoke_file_contains(sSummaryPath, "\"status\":\"completed\"") ||
         !smoke_file_contains(sSummaryPath, "\"visible_text\":\"probe openai tool artifacts ok\"") ) {
...
#endif
    if ( iExitCode == 0 ) {
        printf("smoke_real_provider_probe_openai_tool_result_artifacts ok\n");
    }
    return iExitCode;
}
