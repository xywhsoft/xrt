/*
 * GLM 会话对话范例（Anthropic 兼容）
 *
 * 演示使用 xllm_session 通过 Anthropic 原生协议连接智谱 AI GLM 进行多轮对话。
 * 两轮关联对话 + 一轮需要上下文感知的追问。
 *
 * 编译:
 *   gcc -std=c11 -Wall -Wextra -Isinglehead -Ilib ^
 *       -DXRT_IMPLEMENTATION -DXLLM_IMPLEMENTATION ^
 *       examples\anthropic\glm\glm_session.c ^
 *       -o build\glm_session.exe -lws2_32 -liphlpapi -lshell32 -lcrypt32
 */

#define GLM_API_KEY getenv("GLM_API_KEY")
#define GLM_BASE_URL   "https://open.bigmodel.cn/api/anthropic/v1"
