/*
 * MiniMax 会话对话范例（Anthropic 兼容）
 *
 * 演示使用 xllm_session 通过 Anthropic 原生协议连接 MiniMax 进行多轮对话。
 * 两轮关联对话 + 一轮需要上下文感知的追问。
 *
 * 编译:
 *   gcc -std=c11 -Wall -Wextra -Isinglehead -Ilib ^
 *       -DXRT_IMPLEMENTATION -DXLLM_IMPLEMENTATION ^
 *       examples\anthropic\minimax\minimax_session.c ^
 *       -o minimax_session.exe -lws2_32 -liphlpapi -lshell32 -lcrypt32
 */
...
int main(void)
{
    ...
    printf("=== MiniMax 会话对话范例 (Anthropic) ===\n\n");
    ...
    tSessionOpts.sProfileId     = "minimax";
