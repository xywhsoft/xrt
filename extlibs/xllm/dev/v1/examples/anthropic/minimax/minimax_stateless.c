/*
 * MiniMax 无状态对话范例（Anthropic 兼容）
 *
 * 演示使用底层无状态 API（xllm_chat_ex）通过 Anthropic 原生协议连接 MiniMax 平台。
 * 单轮对话，无会话上下文。
 *
 * 编译:
 *   gcc -std=c11 -Wall -Wextra -Isinglehead -Ilib ^
 *       -DXRT_IMPLEMENTATION -DXLLM_IMPLEMENTATION ^
 *       examples\anthropic\minimax\minimax_stateless.c ^
 *       -o minimax_stateless.exe -lws2_32 -liphlpapi -lshell32 -lcrypt32
 */

#define MINIMAX_API_KEY getenv("MINIMAX_API_KEY")
#define MINIMAX_BASE_URL   "https://api.minimaxi.com/anthropic/v1"
...
int main(void)
{
    ...
    printf("=== MiniMax 无状态对话范例 (Anthropic) ===\n\n");
    ...
    tRequest.sProfileId = (const char *)xrtCopyStr((str)"minimax", 0u);
    ...
    pParts[0].as.tSource.as.sText = (const char *)xrtCopyStr((str)"用一句话介绍 MiniMax 是什么。", 0u);
    ...
    /* MiniMax 的兼容流式收尾与当前其他 provider 存在差异，示例默认走非流式以保证稳定运行。 */
    ...
    printf("[user]  用一句话介绍 MiniMax 是什么。\n");
    printf("[mini]  ");
