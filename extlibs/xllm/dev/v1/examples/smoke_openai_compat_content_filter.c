...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        ...
        "\"model\":\"gpt-filter-mock\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"partial answer\"},\"finish_reason\":\"content_filter\"}],"
    ...
    iRespLen = snprintf(
        ...
        "Connection: close\r\n"
        "x-request-id: req_openai_content_filter_1\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    tProfile.sId = "openai-content-filter";
    ...
    tCreate.sInitialProfileId = "openai-content-filter";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "unsafe prompt") != XRT_NET_OK ) {
    ...
    if ( tTrace.iRequestTraceCount < 1 || tTrace.iFilteredResponseTraceCount < 1 ) {
        fprintf(stderr, "unexpected trace counts request=%d filtered=%d\n", tTrace.iRequestTraceCount, tTrace.iFilteredResponseTraceCount);
        iStatus = 20;
        goto cleanup;
    }
