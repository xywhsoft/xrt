...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    if ( iRecv > 0 ) {
        aBuffer[iRecv] = '\0';
        if ( strstr(aBuffer, "OpenAI-Organization: org-demo\r\n") != NULL ) {
            ++pState->iOrgCount;
        }
        if ( strstr(aBuffer, "OpenAI-Project: proj-demo\r\n") != NULL ) {
            ++pState->iProjectCount;
        }
        if ( strstr(aBuffer, "X-Demo: yes\r\n") != NULL ) {
            ++pState->iDemoHeaderCount;
        }
        if ( strstr(aBuffer, "X-Trace: 123\r\n") != NULL ) {
            ++pState->iTraceHeaderCount;
        }
    }
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_OPENAI_ORGANIZATION", "org-demo") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_OPENAI_PROJECT", "proj-demo") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_DEFAULT_HEADERS", "X-Demo=yes;X-Trace=123") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
         tServer.iOrgCount != 1 ||
         tServer.iProjectCount != 1 ||
         tServer.iDemoHeaderCount != 1 ||
         tServer.iTraceHeaderCount != 1 ) {
        fprintf(
            stderr,
            "unexpected header counters: accept=%d org=%d project=%d demo=%d trace=%d\n",
            tServer.iAcceptCount,
            tServer.iOrgCount,
            tServer.iProjectCount,
            tServer.iDemoHeaderCount,
            tServer.iTraceHeaderCount
        );
        iExitCode = 9;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_openai_headers ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("XLLM_REAL_OPENAI_ORGANIZATION", "");
    smoke_setenv_cstr("XLLM_REAL_OPENAI_PROJECT", "");
