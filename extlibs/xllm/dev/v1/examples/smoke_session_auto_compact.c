...
)
{
    ...
    pResponse->sId = demo_dupstr("mock-session-auto-compact");
...
static void demo_trace_callback(void *pCtx, xllm_trace_kind eKind, const xvalue *pPayload)
{
    ...
    if ( sPhase && strcmp(sPhase, "auto_check") == 0 ) {
        if ( xvoTableGetBool(*pPayload, (str)"triggered", 0u) ) {
            pState->bSawAutoCheckTriggered = true;
        }
    } else if ( sPhase && strcmp(sPhase, "compact_result") == 0 ) {
        if ( xvoTableGetBool(*pPayload, (str)"compacted", 0u) ) {
            pState->bSawCompactResult = true;
        }
    }
}
...
int main(void)
{
    ...
    tAdapter.sName = "mock_session_auto_compact";
    ...
    tProfile.sId = "mock-session-auto-compact";
    tProfile.sProvider = "mock";
    tProfile.sAdapter = "mock_session_auto_compact";
    ...
    tSessionOptions.sProfileId = "mock-session-auto-compact";
    tSessionOptions.sSystemPrompt = "auto compact recent turns";
    tSessionOptions.bEnableAutoCompact = true;
    tSessionOptions.fCompactTriggerRatio = 0.8;
    tSessionOptions.uCompactTriggerTurns = 3u;
    ...
    xllm_turn_init(&tTurn);
    if ( xllm_turn_add_user_text(&tTurn, "delta") != XRT_NET_OK ) return 14;
    if ( xllm_session_chat(pSession, &tTurn, NULL, &pResponse) != XRT_NET_OK ) return 15;
    iStatus = demo_expect_text(pResponse, "users=2 last=delta", 16);
    if ( iStatus != 0 ) return iStatus;
    if ( !tTraceState.bSawAutoCheckTriggered || !tTraceState.bSawCompactResult ) {
        fprintf(stderr, "trace callback did not observe auto compact flow\n");
        return 17;
    }
