...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-openai-probe-inline-file\","
        "\"object\":\"chat.completion\","
        "\"model\":\"gpt-test-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe openai inline file ok\"},\"finish_reason\":\"stop\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"filename\":\"probe-inline.txt\"") != NULL ) {
            ++pState->iFilenameCount;
        }
        if ( strstr(aBuffer, "\"mime_type\":\"text/plain\"") != NULL ) {
            ++pState->iMimeTypeCount;
        }
        if ( strstr(aBuffer, "\"file_data\":\"aGVsbG8gcmVhbCBwcm9iZSBmaWxlCg==\"") != NULL ) {
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sUploadPath = "build/probe_inline_file.txt";
    FILE *pUpload = NULL;
    ...
    if ( fwrite("hello real probe file\n", 1u, 22u, pUpload) != 22u ) {
        fclose(pUpload);
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "summarize this inline probe file") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe openai inline file ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_FILE_PATH", sUploadPath) != 0 ||
         smoke_setenv_cstr("XLLM_REAL_FILE_NAME", "probe-inline.txt") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected openai inline file probe counters: accept=%d file_part=%d filename=%d mime=%d file_data=%d\n",
    ...
    printf("smoke_real_provider_probe_openai_file_inline_bytes ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("XLLM_REAL_FILE_PATH", "");
