...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-openai-alias-image-url\","
        "\"object\":\"chat.completion\","
        "\"model\":\"gpt-test-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe openai alias image url ok\"},\"finish_reason\":\"stop\"}],"
    ...
    if ( iRecv > 0 ) {
        aBuffer[iRecv] = '\0';
        if ( strstr(aBuffer, "\"type\":\"image_url\",\"image_url\":{\"url\":\"https://example.invalid/alias-cat.png\"}}") != NULL ) {
            ++pState->iImageUrlPartCount;
        }
        if ( strstr(aBuffer, "https://example.invalid/alias-cat.png") != NULL ) {
            ++pState->iImageUrlValueCount;
        }
        if ( strstr(aBuffer, "\"describe this alias probe image\"") != NULL ) {
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("AZURE_OPENAI_DEPLOYMENT", "gpt-test") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_API_KEY", "alias-openai-key") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_ENABLE_LOG", "0") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_ENABLE_TRACE", "0") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe openai alias image url ok") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_IMAGE_URL", "https://example.invalid/alias-cat.png") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "describe this alias probe image") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected openai alias image-url probe counters: accept=%d image_part=%d image_value=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_openai_multimodal_alias_envs ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("AZURE_OPENAI_IMAGE_URL", "");
