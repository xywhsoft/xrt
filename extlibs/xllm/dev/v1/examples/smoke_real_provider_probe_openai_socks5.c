...
static bool smoke_recv_exact(SOCKET hSocket, void *pData, size_t iLen)
{
...
static uint32 smoke_upstream_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        "{\"id\":\"chatcmpl-probe-socks5\","
        "\"object\":\"chat.completion\","
        "\"model\":\"gpt-test\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"pong via socks5\"},\"finish_reason\":\"stop\"}],"
...
static uint32 smoke_socks5_server_thread(ptr pParam)
{
    smoke_socks5_server_state *pState = (smoke_socks5_server_state *)pParam;
    SOCKET hClient = INVALID_SOCKET;
    ...
    if ( !smoke_recv_exact(hClient, aGreeting, 4u) ) {
        goto fail;
    }
    ...
    if ( !smoke_send_all(hClient, "\x05\x02", 2u) ) {
        goto fail;
    }

    if ( !smoke_recv_exact(hClient, aAuthHeader, 2u) ) {
        goto fail;
    }
    aAuthUserLen = aAuthHeader[1];
    if ( (size_t)aAuthUserLen + 1u >= sizeof(aAuthData) || !smoke_recv_exact(hClient, aAuthData, aAuthUserLen + 1u) ) {
        goto fail;
    }
    aAuthPassLen = (unsigned char)aAuthData[aAuthUserLen];
    if ( (size_t)aAuthUserLen + 1u + (size_t)aAuthPassLen > sizeof(aAuthData) ||
         !smoke_recv_exact(hClient, aAuthData + aAuthUserLen + 1u, aAuthPassLen) ) {
        goto fail;
    }
    ...
    if ( !smoke_send_all(hClient, "\x01\x00", 2u) ) {
        goto fail;
    }

    if ( !smoke_recv_exact(hClient, aConnectHeader, 4u) ) {
        goto fail;
    }
    ...
    if ( aConnectHeader[3] == 0x01 ) {
        if ( !smoke_recv_exact(hClient, aAddrBuf, 4u) ) {
            goto fail;
        }
        ...
        if ( !smoke_recv_exact(hClient, aAddrBuf, 1u) ) {
            goto fail;
        }
        iHostLen = aAddrBuf[0];
        if ( iHostLen == 0u || (size_t)iHostLen > sizeof(aAddrBuf) || !smoke_recv_exact(hClient, aAddrBuf, iHostLen) ) {
            goto fail;
        }
    ...
    if ( !smoke_recv_exact(hClient, aPortBuf, 2u) ) {
        goto fail;
    }
    ...
    }
    {
        ...
        if ( !smoke_send_all(hClient, aConnectResp, sizeof(aConnectResp)) ) {
            goto fail;
        }
    }
...
int main(void)
{
    ...
    smoke_socks5_server_state tProxy;
    ...
    hProxyThread = xrtThreadCreate((ptr)smoke_socks5_server_thread, &tProxy, 0);
    if ( !hProxyThread ) {
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "pong via socks5") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROXY_KIND", "socks5") != 0 ||
    ...
    if ( tProxy.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected socks5 probe counters: proxy_accept=%d greeting=%d auth_method=%d userpass=%d connect_ipv4=%d connect_domain=%d connect_port=%d tunnel=%d upstream_accept=%d upstream_auth=%d upstream_path=%d\n",
            tProxy.iAcceptCount,
    ...
    printf("smoke_real_provider_probe_openai_socks5 ok\n");
