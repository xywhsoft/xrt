...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"msg_probe_doc_url\","
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-test-mm\","
        "\"content\":[{\"type\":\"text\",\"text\":\"probe anthropic document url ok\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"source\":{\"type\":\"url\",\"url\":\"https://example.invalid/probe.pdf\"}") != NULL ) {
            ++pState->iUrlSourceCount;
        }
        if ( strstr(aBuffer, "\"url\":\"https://example.invalid/probe.pdf\"") != NULL ) {
            ++pState->iPdfUrlCount;
        }
        if ( strstr(aBuffer, "\"summarize anthropic probe pdf\"") != NULL ) {
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_probe_anthropic_doc_url_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "summarize anthropic probe pdf") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic document url ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_FILE_URL", "https://example.invalid/probe.pdf") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected anthropic document url probe counters: accept=%d doc=%d url_source=%d pdf_url=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_anthropic_document_url ok\n");
