...
int main(void)
{
    ...
    const char *sRootDir = "build\\smoke_memory_workspace_gitignore_tmp";
    const char *sSrcDir = "build\\smoke_memory_workspace_gitignore_tmp\\src";
    const char *sDocsPrivateDir = "build\\smoke_memory_workspace_gitignore_tmp\\docs\\private";
    const char *sReadmePath = "build\\smoke_memory_workspace_gitignore_tmp\\README.md";
    const char *sMainPath = "build\\smoke_memory_workspace_gitignore_tmp\\src\\main.c";
    const char *sGeneratedPath = "build\\smoke_memory_workspace_gitignore_tmp\\src\\generated.auto.c";
    const char *sPrivatePath = "build\\smoke_memory_workspace_gitignore_tmp\\docs\\private\\secret.md";
    const char *sGitIgnorePath = "build\\smoke_memory_workspace_gitignore_tmp\\.gitignore";
    const char *sCustomIgnorePath = "build\\smoke_memory_workspace_gitignore_tmp\\.xllmignore";
    int iStatus;
    ...
    if ( write_text_file(sReadmePath, "readme should be ignored by custom ignore file\n") != 0 ||
         write_text_file(sMainPath, "int main(void) { return 0; } /* keep me */\n") != 0 ||
         write_text_file(sGeneratedPath, "generated file should be ignored\n") != 0 ||
         write_text_file(sPrivatePath, "private docs should be ignored\n") != 0 ||
         write_text_file(sGitIgnorePath, "*.auto.c\ndocs/private/\n") != 0 ||
         write_text_file(sCustomIgnorePath, "# custom ignores\nREADME.md\n") != 0 ) {
    ...
    tMemoryOptions.sNamespace = "smoke-workspace-gitignore";
    ...
    if ( tResult.uVisitedFileCount != 6u ||
         tResult.uIngestedFileCount != 1u ||
         tResult.uSkippedFileCount != 5u ||
    ...
    if ( xllm_memory_record_count(pMemory, XLLM_MEMORY_SCOPE_ANY) != 1u ) {
        fprintf(stderr, "expected only one retained workspace record\n");
        xllm_memory_destroy(pMemory);
        xllm_runtime_destroy(pRuntime);
        xllm_error_free(&tError);
        return 8;
    ...
    printf("smoke_memory_workspace_gitignore ok\n");
