...
int main(void)
{
    ...
    tMemoryOptions.sNamespace = "list-recency-sort";
    ...
    iStatus = xllm_turn_add_user_text(&tTurn, "Remember the latest timeline.");
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "add user text failed: %d\n", iStatus);
        goto cleanup;
    }

    if ( init_mock_response(&tResponse, "Old timeline entry.") != 0 ) {
    ...
    tTurnResponseOptions.sConversationId = "thread-list-recency";
    ...
    if ( init_mock_response(&tResponse, "New timeline entry.") != 0 ) {
    ...
    tListOptions.sConversationId = "thread-list-recency";
    tListOptions.uMaxCharsPerText = 4096u;
    iStatus = xllm_memory_list_records(pMemory, &tListOptions, &tRecords, &tError);
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "default list records failed: %s\n", tError.sMessage ? tError.sMessage : "(null)");
        goto cleanup;
    }
    if ( require_true(tRecords.iRecordCount == 2u, "expected two default record results") != 0 ||
         require_true(tRecords.pRecords[0].sSourceUri != NULL, "default record source uri missing") != 0 ||
         require_true(strcmp(tRecords.pRecords[0].sSourceUri, "conversation://thread-list-recency/turn-old") == 0, "default record order should preserve insertion order") != 0 ) {
        goto cleanup;
    }
    ...
    iStatus = xllm_memory_list_records(pMemory, &tListOptions, &tRecords, &tError);
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "sorted list records failed: %s\n", tError.sMessage ? tError.sMessage : "(null)");
        goto cleanup;
    }
    if ( require_true(tRecords.iRecordCount == 2u, "expected two sorted record results") != 0 ||
         require_true(tRecords.pRecords[0].sSourceUri != NULL, "sorted record source uri missing") != 0 ||
         require_true(strcmp(tRecords.pRecords[0].sSourceUri, "conversation://thread-list-recency/turn-new") == 0, "sorted records should place newest entry first") != 0 ) {
        goto cleanup;
    }
    xllm_memory_record_list_result_reset(&tRecords);
    ...
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "default list chunks failed: %s\n", tError.sMessage ? tError.sMessage : "(null)");
        goto cleanup;
    }
    if ( require_true(tChunks.iChunkCount == 2u, "expected two default chunk results") != 0 ||
         require_true(tChunks.pChunks[0].sSourceUri != NULL, "default chunk source uri missing") != 0 ||
         require_true(strcmp(tChunks.pChunks[0].sSourceUri, "conversation://thread-list-recency/turn-old") == 0, "default chunk order should preserve insertion order") != 0 ) {
        goto cleanup;
    }
    ...
    iStatus = xllm_memory_list_chunks(pMemory, &tListOptions, &tChunks, &tError);
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "sorted list chunks failed: %s\n", tError.sMessage ? tError.sMessage : "(null)");
        goto cleanup;
    }
    if ( require_true(tChunks.iChunkCount == 2u, "expected two sorted chunk results") != 0 ||
         require_true(tChunks.pChunks[0].sSourceUri != NULL, "sorted chunk source uri missing") != 0 ||
         require_true(strcmp(tChunks.pChunks[0].sSourceUri, "conversation://thread-list-recency/turn-new") == 0, "sorted chunks should place newest entry first") != 0 ) {
        goto cleanup;
    }

    printf("smoke_memory_list_recency_sort ok\n");
    iRc = 0;
