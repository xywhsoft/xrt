...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        "{"
        "\"model\":\"llama-probe\","
        "\"message\":{\"role\":\"assistant\",\"content\":\"pong from ollama alias env\"},"
        "\"done\":true,"
        "\"done_reason\":\"stop\","
        "\"prompt_eval_count\":7,"
        "\"eval_count\":3"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"model\":\"llama-probe\"") != NULL ) {
            ++pState->iModelCount;
        }
    }
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", XLLM_ADAPTER_OLLAMA_NATIVE) != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "pong from ollama alias env") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
         ...
         tServer.iModelCount != 1 ) {
        fprintf(stderr, "unexpected counters: accept=%d api=%d model=%d\n",
                tServer.iAcceptCount, tServer.iApiChatCount, tServer.iModelCount);
        iExitCode = 9;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_ollama_alias_envs ok\n");
