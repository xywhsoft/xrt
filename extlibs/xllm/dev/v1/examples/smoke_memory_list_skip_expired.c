...
int main(void)
{
    ...
    tMemoryOptions.sNamespace = "list-skip-expired";
    ...
    iStatus = xllm_turn_add_user_text(&tTurn, "Remember the retention notes.");
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "add user text failed: %d\n", iStatus);
        goto cleanup;
    }

    if ( init_mock_response(&tResponse, "Expired retention answer.") != 0 ) {
    ...
    tTurnResponseOptions.sConversationId = "thread-list-expired";
    ...
    if ( init_mock_response(&tResponse, "Future retention answer.") != 0 ) {
    ...
    if ( require_true(tRecords.iRecordCount == 2u, "expected two records before skip-expired filtering") != 0 ) {
        goto cleanup;
    }
    ...
    tListOptions.eScope = XLLM_MEMORY_SCOPE_MEMORY;
    tListOptions.uMaxCharsPerText = 4096u;
    tListOptions.bSkipExpired = true;
    tListOptions.iNowUnix = 2000;
    iStatus = xllm_memory_list_records(pMemory, &tListOptions, &tRecords, &tError);
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "skip-expired list records failed: %s\n", tError.sMessage ? tError.sMessage : "(null)");
        goto cleanup;
    }
    if ( require_true(tRecords.iRecordCount == 1u, "expected one record after skip-expired filtering") != 0 ||
         require_true(tRecords.pRecords[0].sSourceUri != NULL, "filtered record source uri missing") != 0 ||
         require_true(strcmp(tRecords.pRecords[0].sSourceUri, "conversation://thread-list-expired/future") == 0, "list records should keep only non-expired memory") != 0 ) {
        goto cleanup;
    }
    xllm_memory_record_list_result_reset(&tRecords);
    ...
    if ( require_true(tChunks.iChunkCount == 2u, "expected two chunks before skip-expired filtering") != 0 ) {
        goto cleanup;
    }
    ...
    iStatus = xllm_memory_list_chunks(pMemory, &tListOptions, &tChunks, &tError);
    if ( iStatus != XRT_NET_OK ) {
        fprintf(stderr, "skip-expired list chunks failed: %s\n", tError.sMessage ? tError.sMessage : "(null)");
        goto cleanup;
    }
    if ( require_true(tChunks.iChunkCount == 1u, "expected one chunk after skip-expired filtering") != 0 ||
         require_true(tChunks.pChunks[0].sSourceUri != NULL, "filtered chunk source uri missing") != 0 ||
         require_true(strcmp(tChunks.pChunks[0].sSourceUri, "conversation://thread-list-expired/future") == 0, "list chunks should keep only non-expired memory") != 0 ) {
        goto cleanup;
    }

    printf("smoke_memory_list_skip_expired ok\n");
    iRc = 0;
