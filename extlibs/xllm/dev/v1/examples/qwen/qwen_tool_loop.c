...
int main(void)
{
    ...
    const char *sToolResult = demo_env("QWEN_TOOL_RESULT_TEXT", "tool-result-from-local-executor");
    const char *sToolChoice = getenv("QWEN_TOOL_CHOICE");
    const char *sToolChoiceName = getenv("QWEN_TOOL_CHOICE_NAME");
    ...
    printf("=== Qwen Native Tool Loop Example ===\n");
    printf("base url : %s\n", sBaseUrl);
    printf("model    : %s\n\n", sModel);
