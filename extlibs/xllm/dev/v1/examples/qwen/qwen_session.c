...
int main(void)
{
    ...
    const char *sBaseUrl = demo_env("QWEN_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1");
    const char *sModel = demo_env("QWEN_MODEL", "qwen-max-latest");
    ...
    printf("=== Qwen Native Session Example ===\n");
    printf("base url : %s\n", sBaseUrl);
    printf("model    : %s\n\n", sModel);
    ...
    tSessionOptions.sProfileId = "qwen-native";
    ...
    if ( iStatus != XRT_NET_OK || !pResponse ) {
        demo_print_error("qwen session turn 1 failed", iStatus, &tError);
    ...
    printf("\n");
    if ( iStatus != XRT_NET_OK || !pResponse ) {
        demo_print_error("qwen session turn 2 failed", iStatus, &tError);
        xllm_turn_reset(&tTurn);
        xllm_error_free(&tError);
        xllm_session_destroy(pSession);
        xllm_runtime_destroy(pRuntime);
