...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        "{"
        "\"model\":\"llava-probe-output\","
        "\"message\":{\"role\":\"assistant\",\"content\":\"probe ollama output artifacts ok\",\"images\":[\"iVBORw0KGgo=\"]},"
        "\"done\":true,"
        "\"done_reason\":\"stop\","
        "\"prompt_eval_count\":9,"
        "\"eval_count\":4"
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sSummaryPath = "build\\probe_ollama_output_artifacts.summary.json";
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "ollama_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_MODEL", "llava-probe-output") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe ollama output artifacts ok") != 0 ||
    ...
    if ( !smoke_file_contains(sSummaryPath, "\"image_part_count\":1") ||
        ...
        fprintf(stderr, "summary file missing ollama artifact counters\n");
        iExitCode = 10;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_ollama_output_artifacts ok\n");
