...
static bool demo_send_json_response(SOCKET hSocket, const char *sRequestId, const char *sBody)
{
    ...
    iRespLen = snprintf(
        ...
        "\r\n"
        "%s",
        sRequestId ? sRequestId : "req_openai_session_tool_artifacts",
...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    while ( iHandled < 3 ) {
        ...
        if ( iHandled == 0 ) {
            const char *sBody =
                "{\"id\":\"chatcmpl-session-tool-artifacts-1\","
                "\"object\":\"chat.completion\","
                "\"model\":\"gpt-session-tool-mm\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":null,\"tool_calls\":[{\"id\":\"call_session_tool_artifacts_1\",\"type\":\"function\",\"function\":{\"name\":\"get_weather\",\"arguments\":\"{\\\"city\\\":\\\"Shanghai\\\"}\"}}]},\"finish_reason\":\"tool_calls\"}],"
                "\"usage\":{\"prompt_tokens\":9,\"completion_tokens\":2,\"total_tokens\":11}}";

            if ( strstr(aBuffer, "\"hello session tool artifacts\"") != NULL &&
                 strstr(aBuffer, "\"tools\":[{\"type\":\"function\",\"function\":{\"name\":\"get_weather\"") != NULL ) {
                pState->iFirstRequestOk = 1;
            }
            if ( !demo_send_json_response(hClient, "req_openai_session_tool_artifacts_1", sBody) ) {
                closesocket(hClient);
                closesocket(pState->hListen);
                pState->hListen = INVALID_SOCKET;
                return 4u;
            ...
            const char *sBody =
                "{\"id\":\"chatcmpl-session-tool-artifacts-2\","
                "\"object\":\"chat.completion\","
                "\"model\":\"gpt-session-tool-mm\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"session tool artifacts first ok\"},\"finish_reason\":\"stop\"}],"
                "\"usage\":{\"prompt_tokens\":15,\"completion_tokens\":4,\"total_tokens\":19}}";

            if ( strstr(aBuffer, "\"tool_calls\":[{\"id\":\"call_session_tool_artifacts_1\"") != NULL &&
                 strstr(aBuffer, "\"role\":\"tool\"") != NULL &&
                 strstr(aBuffer, "\"tool_call_id\":\"call_session_tool_artifacts_1\"") != NULL &&
                 strstr(aBuffer, "\"type\":\"text\",\"text\":\"sunny map attached\"") != NULL &&
                 strstr(aBuffer, "\"type\":\"image_url\",\"image_url\":{\"url\":\"https://example.invalid/session-tool-map.png\"}}") != NULL &&
                 strstr(aBuffer, "\"type\":\"file\",\"file\":{\"file_id\":\"file_session_tool_doc_1\"}}") != NULL ) {
                pState->iToolRoundRequestOk = 1;
            }
            if ( !demo_send_json_response(hClient, "req_openai_session_tool_artifacts_2", sBody) ) {
                closesocket(hClient);
                closesocket(pState->hListen);
                pState->hListen = INVALID_SOCKET;
                return 5u;
            ...
            const char *sBody =
                "{\"id\":\"chatcmpl-session-tool-artifacts-3\","
                "\"object\":\"chat.completion\","
                "\"model\":\"gpt-session-tool-mm\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"session tool artifacts follow-up ok\"},\"finish_reason\":\"stop\"}],"
            ...
            if ( strstr(aBuffer, "\"tool_calls\":[{\"id\":\"call_session_tool_artifacts_1\"") != NULL &&
                 ...
                 strstr(aBuffer, "\"type\":\"image_url\",\"image_url\":{\"url\":\"https://example.invalid/session-tool-map.png\"}}") != NULL &&
                 strstr(aBuffer, "\"type\":\"file\",\"file\":{\"file_id\":\"file_session_tool_doc_1\"}}") != NULL &&
                 strstr(aBuffer, "\"role\":\"assistant\",\"content\":\"session tool artifacts first ok\"") != NULL &&
                 strstr(aBuffer, "\"follow-up turn\"") != NULL ) {
                pState->iFollowupRequestOk = 1;
            }
            if ( !demo_send_json_response(hClient, "req_openai_session_tool_artifacts_3", sBody) ) {
                closesocket(hClient);
                closesocket(pState->hListen);
                pState->hListen = INVALID_SOCKET;
                return 6u;
...
)
{
    ...
    pResult->pParts[1].as.tSource.as.sUrl = demo_dupstr("https://example.invalid/session-tool-map.png");
    ...
    pResult->pParts[2].as.tSource.sName = demo_dupstr("session-tool-report.pdf");
    pResult->pParts[2].as.tSource.as.sFileId = demo_dupstr("file_session_tool_doc_1");
...
int main(void)
{
    ...
    tProfile.sId = "openai-session-tool-mm";
    ...
    tSessionOptions.sProfileId = "openai-session-tool-mm";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "hello session tool artifacts") != XRT_NET_OK ) {
    ...
    if ( !xllm_response_get_text(pResponse) ||
         strcmp(xllm_response_get_text(pResponse), "session tool artifacts first ok") != 0 ) {
    ...
    if ( !xllm_response_get_text(pResponse) ||
         strcmp(xllm_response_get_text(pResponse), "session tool artifacts follow-up ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 3 ||
        ...
        fprintf(
            stderr,
            "unexpected session tool counters: accept=%d first=%d tool_round=%d followup=%d\n",
    ...
    printf("smoke_openai_compat_session_tool_result_artifacts ok\n");
