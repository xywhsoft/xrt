...
int main(void)
{
    ...
    printf("=== Kimi Native Session Example ===\n\n");
    ...
    tSessionOptions.sProfileId = "kimi-native";
    ...
    if ( iStatus != XRT_NET_OK || !pResponse ) {
        demo_print_error("kimi session turn 1 failed", iStatus, &tError);
    ...
    printf("\n");
    if ( iStatus != XRT_NET_OK || !pResponse ) {
        demo_print_error("kimi session turn 2 failed", iStatus, &tError);
        xllm_turn_reset(&tTurn);
        xllm_error_free(&tError);
        xllm_session_destroy(pSession);
        xllm_runtime_destroy(pRuntime);
