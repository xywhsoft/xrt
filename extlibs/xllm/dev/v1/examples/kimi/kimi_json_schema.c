...
int main(void)
{
    ...
    const char *sBaseUrl = demo_env("KIMI_BASE_URL", "https://api.moonshot.cn/v1/chat/completions");
    const char *sModel = demo_env("KIMI_MODEL", "kimi-k2.5");
    ...
    printf("=== Kimi Native JSON Schema Example ===\n");
    printf("base url : %s\n", sBaseUrl);
    printf("model    : %s\n", sModel);
    ...
    if ( xllm_turn_set_json_schema_response(&tTurn, "kimi_demo_schema", tSchema, tFormatExtra) != XRT_NET_OK ) {
