...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"model\":\"llava-probe-stream-output\",\"message\":{\"role\":\"assistant\",\"content\":\"probe ollama stream output artifacts ok\",\"images\":[\"iVBORw0KGgo=\"]},\"done\":false}\n"
        "{\"model\":\"llava-probe-stream-output\",\"message\":{\"role\":\"assistant\",\"content\":\"\"},\"done\":true,\"done_reason\":\"stop\",\"prompt_eval_count\":11,\"eval_count\":4}\n";
...
int main(void)
{
    ...
    char sBaseUrl[256];
    const char *sSummaryPath = "build\\probe_ollama_stream_output_artifacts.summary.json";
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "ollama_native") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_BASE_URL", sBaseUrl) != 0 ||
         smoke_setenv_cstr("XLLM_REAL_MODEL", "llava-probe-stream-output") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe ollama stream output artifacts ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_IMAGE_PARTS_MIN", "1") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_PARTS_MIN", "1") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_BEGIN_MIN", "1") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_CHUNK_MIN", "1") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_ARTIFACT_READY_MIN", "1") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 || tServer.iApiChatCount != 1 || tServer.iStreamTrueCount != 1 ) {
        fprintf(
            stderr,
            "unexpected request counters: accept=%d api=%d stream=%d\n",
    ...
    if ( !smoke_file_contains(sSummaryPath, "\"stream_mode\":3") ||
         ...
         !smoke_file_contains(sSummaryPath, "\"artifact_part_count\":1") ||
         !smoke_file_contains(sSummaryPath, "\"event_artifact_begin_count\":1") ||
         !smoke_file_contains(sSummaryPath, "\"event_artifact_chunk_count\":1") ||
         !smoke_file_contains(sSummaryPath, "\"event_artifact_ready_count\":1") ) {
        fprintf(stderr, "summary file missing ollama stream artifact counters\n");
        iExitCode = 10;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_ollama_stream_output_artifacts ok\n");
