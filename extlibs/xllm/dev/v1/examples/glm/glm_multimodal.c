...
{
    const char *sImageUrl = getenv("GLM_IMAGE_URL");
    const char *sImagePath = getenv("GLM_IMAGE_PATH");
    const char *sImageMime = demo_env("GLM_IMAGE_MIME", "image/png");
    int iStatus;
...
int main(void)
{
    const char *sApiKey = getenv("GLM_API_KEY");
    const char *sModel = demo_env("GLM_MODEL", GLM_MODEL);
    ...
    printf("=== GLM Native Multimodal Example ===\n");
    printf("Optional envs:\n");
    printf("  GLM_API_KEY\n");
    printf("  GLM_MODEL\n");
    printf("  GLM_IMAGE_URL / GLM_IMAGE_PATH\n\n");

    if ( !sApiKey || !sApiKey[0] ) {
        fprintf(stderr, "set GLM_API_KEY first\n");
        return 1;
    }
    ...
    if ( getenv("GLM_IMAGE_URL") && getenv("GLM_IMAGE_URL")[0] ) bHasInput = true;
    if ( getenv("GLM_IMAGE_PATH") && getenv("GLM_IMAGE_PATH")[0] ) bHasInput = true;
    ...
    if ( !bHasInput ) {
        printf("[assistant] No image input configured; set GLM_IMAGE_URL or GLM_IMAGE_PATH.\n");
        xllm_destroy(pLlm);
        xllm_error_free(&tError);
        xllm_turn_reset(&tTurn);
        xllm_runtime_destroy(pRuntime);
    ...
    tCallOpts.uTimeoutMs = demo_env_u32("GLM_TIMEOUT_MS", 180000u);
