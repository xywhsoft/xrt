...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        ...
        "\"content\":[{\"type\":\"text\",\"text\":\"anthropic image file id ok\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"describe anthropic file id image\"") != NULL ) {
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_anthropic_file_id_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    tProfile.sId = "anthropic-mm-file-id";
    ...
    tCreate.sInitialProfileId = "anthropic-mm-file-id";
    tCreate.sSystemPrompt = "you are an anthropic file id multimodal mock";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "describe anthropic file id image") != XRT_NET_OK ) {
        fprintf(stderr, "turn add text failed\n");
        return 11;
    }
    if ( xllm_turn_add_image_file_id(&tTurn, "file_img_123", "image/png") != XRT_NET_OK ) {
    ...
    if ( !sText || strcmp(sText, "anthropic image file id ok") != 0 ) {
    ...
    if ( tServer.iApiKeyCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected request counters: key=%d version=%d beta=%d model=%d content=%d image=%d file_source=%d file_id=%d text_block=%d text_value=%d\n",
