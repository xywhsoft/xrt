...
int main(void)
{
    const char *sOutputDir = "D:\\Git\\xllm\\build\\smoke_real_provider_probe_matrix_provider_tool_disabled";
    const char *sReportPath = "D:\\Git\\xllm\\build\\smoke_real_provider_probe_matrix_provider_tool_disabled\\real_provider_probe_report.json";
    char *sReport = NULL;
    int iRc;
    ...
    smoke_set_env("XLLM_REAL_RUN_PROVIDER_TOOL_CASES", "0");
    ...
    if ( iRc != 0 ) {
        fprintf(stderr, "matrix runner should succeed for disabled provider_tool case, rc=%d\n", iRc);
        return 1;
    }
    ...
    if ( strstr(sReport, "\"reason\":  \"provider_tool case is disabled by XLLM_REAL_RUN_PROVIDER_TOOL_CASES\"") == NULL ) {
        fprintf(stderr, "expected provider_tool disabled reason in report\n");
        free(sReport);
        return 1;
    }

    free(sReport);
    puts("smoke_real_provider_probe_matrix_provider_tool_disabled ok");
    return 0;
}
