...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        "{"
        "\"model\":\"llama-mock\","
        "\"message\":{\"role\":\"assistant\",\"content\":\"provider tool ok\"},"
        "\"done\":true,"
        "\"done_reason\":\"stop\","
        "\"prompt_eval_count\":8,"
        "\"eval_count\":3"
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "ollama_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_MODEL", "llama-mock") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_TOOL_KIND", "provider") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROVIDER_TOOL_JSON", "{\"type\":\"web_search\",\"name\":\"web_search\",\"max_results\":3}") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "provider tool ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
         tServer.iToolCount != 1 ||
         tServer.iToolTypeCount != 1 ||
         tServer.iToolNameCount != 1 ||
         tServer.iToolMaxResultsCount != 1 ) {
        fprintf(
            stderr,
            "unexpected provider tool counters: accept=%d tools=%d type=%d name=%d max_results=%d\n",
            tServer.iAcceptCount,
            tServer.iToolCount,
            tServer.iToolTypeCount,
            tServer.iToolNameCount,
            tServer.iToolMaxResultsCount
        );
        iExitCode = 9;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_ollama_provider_tool ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("XLLM_REAL_TOOL_KIND", "");
    smoke_setenv_cstr("XLLM_REAL_PROVIDER_TOOL_JSON", "");
