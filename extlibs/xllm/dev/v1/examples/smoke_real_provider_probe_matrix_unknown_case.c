...
int main(void)
{
    const char *sOutputDir = "D:\\Git\\xllm\\build\\smoke_real_provider_probe_matrix_unknown_case";
    const char *sReportPath = "D:\\Git\\xllm\\build\\smoke_real_provider_probe_matrix_unknown_case\\real_provider_probe_report.json";
    char *sReport = NULL;
    int iRc;
    ...
    smoke_set_env("XLLM_REAL_CASE_FILTER", "unknown_case");
    ...
    if ( iRc != 0 ) {
        fprintf(stderr, "matrix runner should succeed for unknown case filter, rc=%d\n", iRc);
        return 1;
    }
    ...
    if ( strstr(sReport, "\"executed_case_count\":  0") == NULL ) {
        fprintf(stderr, "expected executed_case_count=0 in report\n");
        free(sReport);
        return 1;
    }
    ...
    if ( strstr(sReport, "\"name\":  \"unknown_case\"") == NULL ) {
        fprintf(stderr, "expected unknown_case skipped result in report\n");
        free(sReport);
        return 1;
    }
    if ( strstr(sReport, "\"reason\":  \"unknown case name in XLLM_REAL_CASE_FILTER\"") == NULL ) {
        fprintf(stderr, "expected unknown case reason in report\n");
        free(sReport);
        return 1;
    }

    free(sReport);
    puts("smoke_real_provider_probe_matrix_unknown_case ok");
    return 0;
}
