...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"chatcmpl-azure-openai-alias-image-url\","
        "\"object\":\"chat.completion\","
        "\"model\":\"azure-gpt-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe azure alias image url ok\"},\"finish_reason\":\"stop\"}],"
    ...
    if ( iRecv > 0 ) {
        aBuffer[iRecv] = '\0';
        if ( strstr(aBuffer, "\"type\":\"image_url\",\"image_url\":{\"url\":\"https://example.invalid/azure-alias-cat.png\"}}") != NULL ) {
            ++pState->iImageUrlPartCount;
        }
        if ( strstr(aBuffer, "https://example.invalid/azure-alias-cat.png") != NULL ) {
            ++pState->iImageUrlValueCount;
        }
        if ( strstr(aBuffer, "\"describe this azure alias probe image\"") != NULL ) {
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe azure alias image url ok") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_IMAGE_URL", "https://example.invalid/azure-alias-cat.png") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_IMAGE_MIME", "image/png") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "describe this azure alias probe image") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected azure alias image-url probe counters: accept=%d image_part=%d image_value=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_azure_openai_multimodal_alias_envs ok\n");
