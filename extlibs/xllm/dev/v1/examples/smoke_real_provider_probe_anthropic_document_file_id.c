...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"msg_probe_doc_file_id\","
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-test-mm\","
        "\"content\":[{\"type\":\"text\",\"text\":\"probe anthropic document file id ok\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"summarize anthropic probe file id pdf\"") != NULL ) {
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_probe_anthropic_doc_file_id_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_PROMPT", "summarize anthropic probe file id pdf") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe anthropic document file id ok") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_FILE_FILE_ID", "file_doc_123") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_FILE_MIME", "application/pdf") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected anthropic document file_id probe counters: accept=%d beta=%d doc=%d source=%d file_id=%d text=%d\n",
    ...
    printf("smoke_real_provider_probe_anthropic_document_file_id ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("XLLM_REAL_FILE_FILE_ID", "");
