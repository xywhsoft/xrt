...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        "{"
        "\"model\":\"llava-output-mock\","
        "\"message\":{\"role\":\"assistant\",\"content\":\"Here is the ollama image.\",\"images\":[\"iVBORw0KGgo=\"]},"
        "\"done\":true,"
        "\"done_reason\":\"stop\","
...
int main(void)
{
    ...
    tCreate.sInitialProfileId = "ollama-output-artifacts";
    ...
    if ( xllm_turn_add_user_text(&tTurn, "show ollama output image") != XRT_NET_OK ) {
    ...
    if ( !sText || strcmp(sText, "Here is the ollama image.") != 0 ) {
    ...
    if ( !pOutput || pOutput->eKind != XLLM_OUTPUT_MESSAGE || pOutput->as.tMessage.iPartCount != 2u ) {
        fprintf(stderr, "unexpected message output parts\n");
        return 15;
    }
    ...
    if ( pOutput->as.tMessage.pParts[0].eKind != XLLM_PART_TEXT ||
        ...
        fprintf(stderr, "unexpected output part layout\n");
        return 16;
    }
    ...
    if ( tEvents.iTextDeltaCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected event counters text=%d artifact_begin=%d artifact_chunk=%d artifact_ready=%d output_begin=%d output_end=%d\n",
