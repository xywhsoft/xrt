...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    for ( iRound = 0; iRound < 2; ++iRound ) {
        ...
        if ( iRecv > 0 ) {
            aBuffer[iRecv] = '\0';
            if ( strstr(aBuffer, "Authorization: Bearer alias-openai-key\r\n") != NULL ) {
                ++pState->iAuthCount;
            }
            if ( iRound == 0 ) {
        ...
        if ( iRound == 0 ) {
            ...
            sBody =
                ...
                "\"object\":\"chat.completion\","
                "\"model\":\"gpt-test-tool\","
                "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe openai tool alias ok\"},\"finish_reason\":\"stop\"}],"
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         smoke_setenv_cstr("OPENAI_BASE_URL", sBaseUrl) != 0 ||
         smoke_setenv_cstr("OPENAI_MODEL", "gpt-test-tool") != 0 ||
         smoke_setenv_cstr("OPENAI_API_KEY", "alias-openai-key") != 0 ||
         smoke_setenv_cstr("OPENAI_TOOL_CHOICE", "named") != 0 ||
         smoke_setenv_cstr("OPENAI_TOOL_CHOICE_NAME", "get_probe_value") != 0 ||
         smoke_setenv_cstr("OPENAI_TOOL_RESULT_TEXT", "alias-tool-value") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe openai tool alias ok") != 0 ) {
    ...
    printf("smoke_real_provider_probe_openai_tool_alias_envs ok\n");

cleanup:
    smoke_setenv_cstr("XLLM_REAL_ADAPTER", "");
    smoke_setenv_cstr("OPENAI_BASE_URL", "");
    smoke_setenv_cstr("OPENAI_MODEL", "");
    smoke_setenv_cstr("OPENAI_API_KEY", "");
    smoke_setenv_cstr("OPENAI_TOOL_CHOICE", "");
    smoke_setenv_cstr("OPENAI_TOOL_CHOICE_NAME", "");
    smoke_setenv_cstr("OPENAI_TOOL_RESULT_TEXT", "");
