...
{
    static const char sFileBody[] = "hello azure file\n";
    ...
    if ( smoke_read_http_message(hClient, sRequest, sizeof(sRequest)) ) {
        if ( strstr(sRequest, "GET /azure-doc.txt HTTP/1.1\r\n") != NULL ) {
...
static uint32 smoke_chat_server_thread(ptr pParam)
{
    ...
    char sRequest[16384];
    const char *sBody =
        "{\"id\":\"chatcmpl-azure-probe-file-url\","
        "\"object\":\"chat.completion\","
        "\"model\":\"azure-gpt-test-mm\","
        "\"choices\":[{\"index\":0,\"message\":{\"role\":\"assistant\",\"content\":\"probe azure openai file url ok\"},\"finish_reason\":\"stop\"}],"
    ...
    ++pState->iAcceptCount;
    if ( smoke_read_http_message(hClient, sRequest, sizeof(sRequest)) ) {
        if ( strstr(sRequest, "Authorization: Bearer alias-azure-openai-key\r\n") != NULL ) {
            ++pState->iAuthHeaderCount;
        }
        if ( strstr(sRequest, "\"type\":\"file\",\"file\":{") != NULL ) {
            ++pState->iFilePartCount;
        }
        if ( strstr(sRequest, "\"filename\":\"probe-azure-url.txt\"") != NULL ) {
            ++pState->iFilenameCount;
        }
        if ( strstr(sRequest, "\"mime_type\":\"text/plain\"") != NULL ) {
            ++pState->iMimeTypeCount;
        }
        if ( strstr(sRequest, "\"file_data\":\"aGVsbG8gYXp1cmUgZmlsZQo=\"") != NULL ) {
...
int main(void)
{
    ...
    snprintf(sFileUrl, sizeof(sFileUrl), "http://127.0.0.1:%u/azure-doc.txt", (unsigned)tFileServer.uPort);
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "openai_compat") != 0 ||
         ...
         smoke_setenv_cstr("AZURE_OPENAI_MULTIMODAL_MODEL", "azure-gpt-test-mm") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "probe azure openai file url ok") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_FILE_URL", sFileUrl) != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_FILE_MIME", "text/plain") != 0 ||
         smoke_setenv_cstr("AZURE_OPENAI_FILE_NAME", "probe-azure-url.txt") != 0 ) {
...
cleanup:
    ...
    smoke_setenv_cstr("AZURE_OPENAI_FILE_URL", "");
    smoke_setenv_cstr("AZURE_OPENAI_FILE_MIME", "");
    smoke_setenv_cstr("AZURE_OPENAI_FILE_NAME", "");
