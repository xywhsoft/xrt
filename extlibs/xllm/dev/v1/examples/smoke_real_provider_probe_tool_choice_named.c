...
typedef struct {
    ...
    int iNamedChoiceDisableParallelCount;
} smoke_server_state;
...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    char aBuffer[8192];
    const char *sBody =
        "{\"id\":\"msg_probe_tool_named\","
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-mock\","
        "\"content\":[{\"type\":\"text\",\"text\":\"tool choice named probe ok\"}],"
    ...
    if ( iRecv > 0 ) {
        ...
        if ( strstr(aBuffer, "\"tool_choice\":{\"type\":\"tool\",\"name\":\"get_probe_value\",\"disable_parallel_tool_use\":true}") != NULL ) {
            ++pState->iNamedChoiceDisableParallelCount;
        }
    }
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_TOOL_CHOICE", "named") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_TOOL_CHOICE_NAME", "get_probe_value") != 0 ||
         smoke_setenv_cstr("XLLM_REAL_EXPECT_TEXT_CONTAINS", "tool choice named probe ok") != 0 ) {
    ...
    if ( tServer.iAcceptCount != 1 ||
         ...
         tServer.iNamedChoiceDisableParallelCount != 1 ) {
        fprintf(
            stderr,
            "unexpected named tool-choice counters: accept=%d tool_defs=%d named=%d named_disable_parallel=%d\n",
            tServer.iAcceptCount,
            tServer.iToolDefCount,
            tServer.iNamedChoiceCount,
            tServer.iNamedChoiceDisableParallelCount
        );
        iExitCode = 9;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_tool_choice_named ok\n");
...
cleanup:
    ...
    smoke_setenv_cstr("XLLM_REAL_TOOL_CHOICE", "");
    smoke_setenv_cstr("XLLM_REAL_TOOL_CHOICE_NAME", "");
