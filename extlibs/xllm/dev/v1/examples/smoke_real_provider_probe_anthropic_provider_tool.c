...
static uint32 smoke_server_thread(ptr pParam)
{
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_probe_provider_tool_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    if ( smoke_setenv_cstr("XLLM_REAL_ADAPTER", "anthropic_native") != 0 ||
         ...
         smoke_setenv_cstr("XLLM_REAL_PROVIDER_TOOL_JSON", "{\"type\":\"web_search_20250305\",\"name\":\"web_search\",\"max_uses\":2}") != 0 ||
    ...
    if ( tServer.iAcceptCount != 1 ||
         ...
         tServer.iToolMaxUsesCount != 1 ) {
        fprintf(
            stderr,
            "unexpected provider tool counters: accept=%d tools=%d type=%d name=%d max_uses=%d\n",
            tServer.iAcceptCount,
            tServer.iToolCount,
            tServer.iToolTypeCount,
            tServer.iToolNameCount,
            tServer.iToolMaxUsesCount
        );
        iExitCode = 9;
        goto cleanup;
    }

    printf("smoke_real_provider_probe_anthropic_provider_tool ok\n");
