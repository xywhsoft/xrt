...
static uint32 demo_server_thread(ptr pParam)
{
    ...
    const char *sBody =
        ...
        "\"type\":\"message\","
        "\"role\":\"assistant\","
        "\"model\":\"claude-thinking-mock\","
        "\"content\":[{\"type\":\"thinking\",\"thinking\":\"Plan it first.\"},{\"type\":\"text\",\"text\":\"answer ready\"}],"
    ...
    iRespLen = snprintf(
        ...
        "request-id: req_anthropic_thinking_1\r\n"
        "Connection: close\r\n"
        "Content-Length: %u\r\n"
        "\r\n"
        "%s",
...
int main(void)
{
    ...
    tProfile.sId = "anthropic-thinking";
    ...
    tCreate.sInitialProfileId = "anthropic-thinking";
    ...
    if ( !sText || strcmp(sText, "answer ready") != 0 ) {
    ...
    if ( pResponse->iOutputCount < 2 ||
         ...
         strcmp(pResponse->pOutputs[0].as.tThinking.sText, "Plan it first.") != 0 ) {
    ...
    if ( tServer.iApiKeyCount != 1 ||
        ...
        fprintf(
            stderr,
            "unexpected request counters: key=%d version=%d type=%d budget=%d display=%d\n",
