...
#define XRT_MODULE_HTTP_CLIENT_POOL
#define XRT_MODULE_WEBSOCKET_CONNECTION_FUTURE
#define XRT_MODULE_WEBSOCKET_SERVER
#define XRT_MODULE_WEBSOCKET_CLIENT_FUTURE
...
static bool benchWsLoopback(uint32 iIterations, size_t iPayloadSize)
{
	...
	if (
		...
		(ClientStats.RequestsStarted != 1) ||
		(ClientStats.RequestsCompleted != 1) ||
		(ClientStats.ConnectionsOpened != 1) ||
		(ClientStats.IdleConnections != 0) ||
