...
)
{
	...
	if ( !__xrtRangeValid(pOperation, sizeof(*pOperation)) ) {
		__xrtWsGroupError(
			XERR_ARGUMENT,
	...
	if ( !__xrtRangeValid(pOperation, iSize) ) {
		__xrtWsGroupError(
			XERR_ARGUMENT,
			XWS_GROUP_ERROR_ARGUMENT,
			sOperation,
...
)
{
	...
	pItem->Waiter.Proc = __xrtWsGroupOpSourceDone;
	pItem->Waiter.Release = __xrtWsGroupOpWaiterRelease;
	...
	if ( !__xrtFutureWaiterAdd(pFuture, &pItem->Waiter) ) {
...
)
{
	...
	if ( !__xrtRangeValid(pResult, sizeof(Result)) ||
		__xrtRangesOverlap(
			pResult,
			sizeof(Result),
			pOperation,
