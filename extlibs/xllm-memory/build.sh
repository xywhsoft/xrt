...
$CC $CFLAGS -I. -I"$XLLM_DIR" -I"$XRT_INCLUDE" -c xllm-memory.c -o "$RELEASE_DIR/xllm-memory.o"
$CC $XRT_CFLAGS -I. -I"$XRT_INCLUDE" -c xllm-memory-xrt.c -o "$RELEASE_DIR/xllm-memory-xrt.o"

$CC $XRT_CFLAGS $LDFLAGS -I. -I"$XLLM_DIR" -I"$XRT_INCLUDE" \
    tests/test_xllm_memory.c "$RELEASE_DIR/xllm-memory-xrt.o" $LIBS -o "$BUILD_DIR/test_xllm_memory"

if [ "$RUN_TESTS" = "1" ]; then
    "$BUILD_DIR/test_xllm_memory"
fi

printf '\nxllm-memory build: PASS\n'
