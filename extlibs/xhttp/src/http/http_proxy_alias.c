...
#if defined(XHTTP_FEATURE_HTTP_PROXY_ALIAS)

/* 判断线路字节是否可以不经转义地出现在 RFC 3986 标识中。 */
static bool __xrtHttpProxyAliasUnreserved(uint8 iValue)
{
...
static bool __xrtHttpProxyAliasValid(xstrview Alias)
{
	...
	while ( iOffset < Alias.Size ) {
		...
		if ( (iWire != (uint8)'%') &&
			!__xrtHttpProxyAliasUnreserved(iWire) ) {
			return false;
		}
		if ( xrtPercentNext(
