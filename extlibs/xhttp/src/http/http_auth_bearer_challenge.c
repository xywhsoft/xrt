...
)
{
	...
	if ( !__xrtHttpBearerUriCharactersValid(
		pParam, &bAbsolute
	) || !bAbsolute ) {
	...
	if ( bEscaped ) {
		bValid = xrtUrlParamValid(pParam);
	} else {
		bValid = xrtUrlParse(pParam->Value, &Url) &&
...
)
{
	...
	return __xrtHttpBearerUriValid(pParam);
}
