...
{
	uint8 Digest[XHTTP_DIGEST_MAX_SIZE];
...
)
{
	...
	char Text[XHTTP_DIGEST_MAX_TEXT_SIZE];
	size_t iRequired;
	bool bWrite;
...
)
{
	for ( size_t i = 0; i < Text.Size; i++ ) {
		sOutput[i] = (char)__xhttpAsciiLower(
			(uint8)Text.Data[i]
		);
	}
}
...
)
{
	...
	char A1[XHTTP_DIGEST_MAX_TEXT_SIZE];
	char A2[XHTTP_DIGEST_MAX_TEXT_SIZE];
