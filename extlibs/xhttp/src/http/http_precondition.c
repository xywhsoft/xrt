...
/* 使用公开时间 API 进行无错误副作用的 HTTP-date 探测。 */
static bool __xrtHttpDateParseValue(
	xstrview Text,
	xtime* pTime
)
{
	xerror* pPrevious = xrtTakeError();
	bool bParsed = xrtTimeParseHTTPDate(Text, pTime);
...
)
{
	...
	*pValid = __xrtHttpDateParseValue(Value, pDate);
	return true;
}
...
)
{
	...
	if ( !__xrtHttpDateParseValue(Value, &iDate) ) {
		return false;
	}
	return Current.Exists &&
		Current.HasLastModified &&
