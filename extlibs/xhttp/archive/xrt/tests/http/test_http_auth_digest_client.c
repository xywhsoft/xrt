...
static xhttpdigestproof testClientAuth(
	...
	char Output[XRT_HTTP_DIGEST_MAX_TEXT_SIZE * 2u],
	xhttpdigestauth* pAuth
)
{
	xhttpdigestclientauth Input;
	xhttpdigestproof Proof;
	char UserHash[XRT_HTTP_DIGEST_MAX_TEXT_SIZE];
	...
	) && (iSize == (XRT_HTTP_DIGEST_MAX_TEXT_SIZE * 2u)) &&
	...
	testRequire(xrtHttpDigestClientAuth(
		&Input,
		Output,
		XRT_HTTP_DIGEST_MAX_TEXT_SIZE * 2u,
		&iSize,
		pAuth
	) && (iSize == (XRT_HTTP_DIGEST_MAX_TEXT_SIZE * 2u)) &&
...
static void testBuild(void)
{
	...
	) && (iSize == XRT_HTTP_DIGEST_MAX_TEXT_SIZE) &&
...
static void testMemoryContract(void)
{
	...
	testRequire((iSize == sizeof(Output)) &&
		(Auth.Response.Size == XRT_HTTP_DIGEST_MAX_TEXT_SIZE),
