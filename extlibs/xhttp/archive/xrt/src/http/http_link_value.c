...
/* 把参数语义读取器恢复到首字节。 */
static void __xrtHttpLinkReaderReset(ptr pContext)
{
	xrt_http_link_reader* pReader =
		(xrt_http_link_reader*)pContext;
...
/* 在一个参数的解码语义字节上建立半开区间读取器。 */
typedef struct xrt_http_link_range_reader {
	const xhttpparam* Param;
	size_t Start;
	size_t End;
	size_t Index;
	size_t Offset;
} xrt_http_link_range_reader;



/* 把 Link 参数区间读取器恢复到首字节。 */
static void __xrtHttpLinkRangeReaderReset(ptr pContext)
{
	xrt_http_link_range_reader* pReader =
		(xrt_http_link_range_reader*)pContext;
...
/* 从 Link 参数的指定解码区间读取一个语义字节。 */
static bool __xrtHttpLinkRangeReaderNext(ptr pContext, uint8* pByte)
{
	xrt_http_link_range_reader* pReader =
		(xrt_http_link_range_reader*)pContext;
	uint8 iByte;

	while ( pReader->Index < pReader->Start ) {
		if ( !__xrtHttpParamSemanticNext(
			pReader->Param, &pReader->Offset, &iByte
		) ) {
			return false;
		}
		pReader->Index++;
	}
	if ( pReader->Index == pReader->End ) {
		return false;
	}
	if ( !__xrtHttpParamSemanticNext(
		pReader->Param, &pReader->Offset, pByte
...
{
	xrt_http_link_range_reader Reader = {
		pParam, iStart, iEnd, 0, 0
	};
	uint32 iFlags;
	...
	if ( !__xrtUrlReaderValid(
		&Reader,
		__xrtHttpLinkRangeReaderNext,
		__xrtHttpLinkRangeReaderReset,
		&iFlags
	) ) {
		return false;
	}
	return ((iFlags & XURL_HAS_SCHEME) != 0) &&
		((iFlags & XURL_HAS_FRAGMENT) == 0);
}
...
bool __xrtHttpLinkUriParamValid(const xhttpparam* pParam)
{
	...
	if ( !__xrtHttpLinkParamDirect(pParam, &Direct) ) {
		...
		Reader = (xrt_http_link_reader){ pParam, 0 };
		return __xrtUrlReaderValid(
			&Reader,
			__xrtHttpLinkRead,
			__xrtHttpLinkReaderReset,
			NULL
		);
	}
	return __xrtUrlParseValue(Direct, &Url);
}
...
)
{
	...
	Reader = (xrt_http_link_reader){ &Link.Rel, 0 };
	while ( __xrtHttpLinkRead(&Reader, &iByte) ) {
		if ( iByte == (uint8)' ' ) {
			if ( bItem && bEqual &&
