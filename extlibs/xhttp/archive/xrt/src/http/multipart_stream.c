...
{
	return __xrtRangeValid(pReader, sizeof(*pReader)) &&
		(((uintptr_t)pReader %
		 XRT_INTERNAL_OBJECT_ALIGNOF(xmultipartreader)) == 0);
}
...
)
{
	if ( !__xrtMultipartReaderPointerValid(pReader) ||
		...
		!__xrtRangeValid(pPart, sizeof(*pPart)) ||
		!__xrtRangeValid(pData, sizeof(*pData)) ||
...
)
{
	...
	if ( !__xrtMultipartReaderPointerValid(pReader) ||
		...
		((pLimits != NULL) &&
		 !__xrtRangeValid(pLimits, sizeof(*pLimits))) ||
		__xrtRangesOverlap(
			pReader, sizeof(*pReader),
