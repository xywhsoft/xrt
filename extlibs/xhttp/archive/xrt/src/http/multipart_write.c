...
)
{
	...
	if ( !__xrtMultipartWriteBoundaryMeasure(
		pBoundary, pSize
	) || !__xrtHttpFieldWriteMeasure(
		pFields, iFieldCount, true, &iFields
	) ) {
		return false;
	}
...
static size_t __xrtMultipartWritePartHead(
	uint8* pOutput,
	const xmultipartboundary* pBoundary,
	const xhttpfield* pFields,
	size_t iFieldCount
)
{
	...
	iOffset += __xrtHttpFieldWriteUnchecked(
...
)
{
	...
	*pSize = __xrtMultipartWritePartHead(
...
)
{
	...
	iOffset = __xrtMultipartWritePartHead(
		(uint8*)pOutput, pBoundary,
		pFields, iFieldCount
	);
	__xrtMultipartWriteBytes(
...
)
{
	...
	if ( !__xrtMultipartWriteViewsValid(
		...
		pSize, sizeof(*pSize),
		pBoundary, sizeof(*pBoundary)
	) || ((pFilename != NULL) && __xrtRangesOverlap(
		pSize, sizeof(*pSize),
