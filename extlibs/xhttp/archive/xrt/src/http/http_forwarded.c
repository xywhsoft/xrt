...
/* 把参数语义读取器适配到 URL 的共享 Host 语法核心。 */
static bool __xrtHttpForwardedHostRead(ptr pContext, uint8* pByte)
{
	return __xrtHttpForwardedRead(
		(xrt_http_forwarded_reader*)pContext, pByte
	);
}
...
)
{
	...
	return __xrtUrlHostReaderValid(
		&Reader, __xrtHttpForwardedHostRead, &bIpLiteral
	);
}
