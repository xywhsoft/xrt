...
{
	return __xrtCookieAsciiEqual(Name, (xstrview){ sName, iSize });
}
...
static int __xrtCookieDateMonth(xstrview Token)
{
	...
	for ( i = 0; i < 12; i++ ) {
		if ( (__xrtHttpAsciiLower((unsigned char)Token.Data[0]) ==
			 (unsigned char)Months[i][0]) &&
			(__xrtHttpAsciiLower((unsigned char)Token.Data[1]) ==
			 (unsigned char)Months[i][1]) &&
			(__xrtHttpAsciiLower((unsigned char)Token.Data[2]) ==
...
static bool __xrtCookieDateParseValue(xstrview Text, xtime* pTime)
{
	...
	return __xrtTimeMakeValue(&DateTime, pTime) == __XRT_TIME_MAKE_OK;
}
...
XRT_API bool xrtCookieDateParse(xstrview Text, xtime* pTime)
{
	...
	if ( (pTime == NULL) || !__xrtCookieViewValid(Text) ||
		__xrtRangesOverlap(pTime, sizeof(*pTime), Text.Data, Text.Size) ) {
		__xrtErrorSetInvalidArgument();
		return false;
	}
...
XRT_API bool xrtSetCookieParse(xstrview Text, xsetcookie* pCookie)
{
	...
	if ( (pCookie == NULL) || !__xrtCookieViewValid(Text) ||
		__xrtRangesOverlap(
...
static bool __xrtSetCookieDomainValid(xstrview Domain)
{
	...
	if ( !__xrtCookieViewValid(Domain) || (Domain.Size == 0) ||
		(Domain.Size > 253) ) {
		return false;
	}
...
)
{
	...
	while ( iPosition < iCurrent ) {
		...
		if ( __xrtCookieAsciiEqual(Previous.Name, Name) ) {
			return true;
		}
		iPosition = iNext;
	}
...
{
	return (Text.Size >= iPrefix) && __xrtCookieAsciiEqual(
...
)
{
	...
	if ( !__xrtCookieAttributeValueValid(Segment) ||
		(Segment.Size == 0) ) {
		return false;
	}
	if ( __xrtSetCookieName(Attribute.Name, "Expires", 7) ) {
	...
	if ( __xrtSetCookieName(Attribute.Name, "Path", 4) ) {
		return bValue &&
			__xrtCookieAttributeValueValid(Attribute.Value);
	}
...
XRT_API bool xrtSetCookieValidate(xstrview Text)
{
	...
	if ( (iEqual == 0) || (iEqual == Pair.Size) ||
		!xrtHttpTokenValid(__xrtSetCookieTrim((xstrview){
			Pair.Data, iEqual
		})) || !__xrtCookieValueValid(__xrtSetCookieTrim((xstrview){
			Pair.Data + iEqual + 1u, Pair.Size - iEqual - 1u
		})) ) {
		__xrtErrorSetValue();
		return false;
...
static bool __xrtSetCookieBuildValid(const xsetcookie* pCookie)
{
	...
	if ( !__xrtCookieViewValid(pCookie->Name) ||
		!__xrtCookieViewValid(pCookie->Value) ||
		!__xrtCookieViewValid(pCookie->Domain) ||
		!__xrtCookieViewValid(pCookie->Path) ||
		!__xrtCookieViewValid(pCookie->RawAttributes) ||
	...
	if ( !xrtHttpTokenValid(pCookie->Name) ||
		!__xrtCookieValueValid(pCookie->Value) ) {
		__xrtErrorSetValue();
		return false;
	}
	...
	if ( ((pCookie->Flags & XSET_COOKIE_HAS_PATH) != 0) &&
		!__xrtCookieAttributeValueValid(pCookie->Path) ) {
		__xrtErrorSetValue();
		return false;
	}
	if ( ((pCookie->Flags & XSET_COOKIE_HAS_SAME_SITE) != 0) &&
	...
	for ( i = 0; i < pCookie->ExtensionCount; i++ ) {
		...
		if ( ((pAttribute->Flags & ~XCOOKIE_ATTRIBUTE_HAS_VALUE) != 0) ||
			!__xrtCookieViewValid(pAttribute->Name) ||
			!__xrtCookieViewValid(pAttribute->Value) ||
		...
		if ( !xrtHttpTokenValid(pAttribute->Name) ||
			...
			!__xrtCookieAttributeValueValid(pAttribute->Value) ) {
			__xrtErrorSetValue();
			return false;
		}
		for ( j = 0; j < i; j++ ) {
			if ( __xrtCookieAsciiEqual(
...
)
{
	...
	if ( __xrtRangesOverlap(pCookie, sizeof(*pCookie), pData, iSize) ||
		__xrtRangesOverlap(
