...
- `XRT_FEATURE_HTTP_FORWARDED`：解析、重复字段组合、标准值校验和扩展参数遍历；
  依赖 `http_param` 与 `http_host`，Host 语义最终复用 URL authority 的唯一语法核心。
- `XRT_FEATURE_HTTP_FORWARDED_WRITE`：规范写出单个元素或完整链路，以及分配型
  Build；只增加 writer 实现。
