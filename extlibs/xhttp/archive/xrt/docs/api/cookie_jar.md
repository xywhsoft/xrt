...
- `XRT_FEATURE_COOKIE_JAR`：存储、策略、请求选择和稳定快照。
- `XRT_FEATURE_COOKIE_JAR_HEADERS`：动态 Header 批量接收和请求字段应用。
...
启用 `XRT_FEATURE_COOKIE_JAR_HEADERS` 后：
...
启用 `XRT_FEATURE_HTTP_CLIENT_COOKIES` 后，高层 Client 直接组合这两个适配入口：
