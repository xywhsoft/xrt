...
| 宏 | 能力 | 依赖 |
|---|---|---|
| `XRT_FEATURE_HTTP_ETAG` | entity-tag 解析、列表、比较、写出 | `HTTP` |
| `XRT_FEATURE_HTTP_PRECONDITION` | 条件请求顺序、HTTP-date、If-Range | `HTTP_ETAG`、`TIME_TEXT` |
| `XRT_FEATURE_HTTP_RANGE` | Range、字节范围、Content-Range | `HTTP` |
| `XRT_FEATURE_HTTP_RANGE_MULTIPART` | multipart/byteranges 长度与分段写出 | `HTTP_RANGE` |
...
`XRT_FEATURE_HTTP_RANGE_MULTIPART` 提供与文件、Body、客户端和服务器都无关的
