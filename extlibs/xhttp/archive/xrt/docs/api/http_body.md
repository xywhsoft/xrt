...
启用 `XRT_FEATURE_HTTP_BODY_ASYNC` 后，Reader 操作可以提供 `Wait`。来源只有在
...
启用 `XRT_FEATURE_HTTP_BODY_STREAM` 后，`xrtHttpBodyStreamCreate()` 同时返回一个
...
等待组合，不公开第二套正文 API。`XRT_FEATURE_HTTP_BODY_DEFLATE` 和
`XRT_FEATURE_HTTP_BODY_INFLATE` 分别提供 `xrtHttpBodyDeflate` 与
...
启用 `XRT_FEATURE_HTTP_BODY_DECODE` 后，`xrtHttpBodyDecodeFields` 可以把公共
