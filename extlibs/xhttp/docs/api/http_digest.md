...
- `XHTTP_FEATURE_HTTP_DIGEST`：类型化解析、重复字段组合和 Byte Sequence 解码；
  只依赖 `http_structured`。
- `XHTTP_FEATURE_HTTP_DIGEST_WRITE`：单算法摘要和偏好的常用写出；依赖
  `http_structured_write`。
- `XHTTP_FEATURE_HTTP_DIGEST_SHA2`：连续数据的 SHA-256/SHA-512 一步生成和验证；
