...
- **XHTTP_FEATURE_HTTP_BODY_INFLATE**：依赖 **XHTTP_FEATURE_HTTP_BODY_TRANSFORM**、
  **XHTTP_FEATURE_HTTP_BODY** 和 **XRT_FEATURE_INFLATE**。
- **XHTTP_FEATURE_HTTP_BODY_DECODE**：额外依赖 **XRT_FEATURE_HTTP_ENCODING**，
  提供 Content-Encoding 到多层 Body 的便利组合；不用时可以单独裁掉。
- 异步 Body 同时启用 **XHTTP_FEATURE_HTTP_BODY_ASYNC** 时，变换 Reader 会透明转发
