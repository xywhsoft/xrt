...
`xhttpstaticreplyconfig` 由 `xrtHttpStaticReplyConfigInit` 初始化，负责协议计划、
显式 Content-Type、Cache-Control 和 multipart boundary；`xhttpstaticserveconfig`
再增加挂载路径与目录索引策略。组合层失败使用 `xhttpserverstaticerror` 保留准确阶段：
`XHTTP_SERVER_STATIC_ERROR_CONFIG`、`XHTTP_SERVER_STATIC_ERROR_TARGET`、
`XHTTP_SERVER_STATIC_ERROR_PATH`、`XHTTP_SERVER_STATIC_ERROR_BOUNDARY`、
`XHTTP_SERVER_STATIC_ERROR_PLAN`、`XHTTP_SERVER_STATIC_ERROR_RESPONSE`、
`XHTTP_SERVER_STATIC_ERROR_BODY` 和 `XHTTP_SERVER_STATIC_ERROR_REPLY`。
...
`XHTTP_FEATURE_HTTP_SERVER_STATIC` 只在需要完整静态服务时启用。它依赖静态协议、受限文件根、异步文件 Body、Server Future、MIME、HEX 和安全随机源。只需要协议计算、路径映射或文件 Body 时，应单独启用对应低层特性。
