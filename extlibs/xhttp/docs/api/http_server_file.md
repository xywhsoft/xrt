...
启用 `XHTTP_FEATURE_HTTP_SERVER_FILE` 会引入：

- `XHTTP_FEATURE_HTTP_BODY_FILE`：异步文件正文与严格区间。
- `XHTTP_FEATURE_HTTP_SERVER_FUTURE`：把 Reply Future 绑定到 Connection Worker。
- `XHTTP_FEATURE_HTTP_SERVER_BODY_ASYNC`：在 TCP 背压和写时限下等待异步正文。
...
`XHTTP_FEATURE_HTTP_RANGE`、`XHTTP_FEATURE_HTTP_ETAG` 与
`XHTTP_FEATURE_HTTP_PRECONDITION` 提供；本模块刻意不强制组合它们。直接使用
...
当前 `http_static_path`、`http_static_plan`、`http_static_response`、`http_static_file`、
`http_static_multipart_body` 与 `http_server_static` 已经承接：
...
这些能力没有直接复制旧代码：旧实现先按路径查询再打开文件，存在 TOCTOU；路径文本检查
不能阻止符号链接逃出根目录；仅按大小和修改时间生成的值却使用强 ETag 外形；
因此旧的路径与验证器生成不能作为当前实现。协议向量已经迁入
`tests/http/test_http_etag.c`、`test_http_precondition.c` 和
`test_http_range.c`；静态层测试还覆盖路径策略、文件替换、多范围响应和同句柄读取边界。
