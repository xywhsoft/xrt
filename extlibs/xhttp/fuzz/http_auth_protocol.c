...
/* 报告模糊契约失败位置，避免优化器合并冷分支后丢失诊断。 */
static void __xrtHttpAuthFuzzAbort(size_t iLine)
{
	fprintf(stderr, "HTTP authentication fuzz invariant at line %zu\n", iLine);
	abort();
}

#define XRT_HTTP_AUTH_FUZZ_ABORT() __xrtHttpAuthFuzzAbort(__LINE__)
#define abort() XRT_HTTP_AUTH_FUZZ_ABORT()
...
static void __xrtHttpAuthFuzzRoundTrip(const xhttpauth* pAuth)
{
	...
	if ( !xrtHttpAuthWrite(
		...
		/* 读取端容忍 #list 空成员，发送端必须拒绝生成非规范值。 */
		if ( xrtErrorKind(xrtGetError()) != XERR_VALUE ) {
			abort();
		}
		xrtClearError();
		return;
