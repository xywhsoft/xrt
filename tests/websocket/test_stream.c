...
)
{
	...
	testRequire(
		(pClose->Flags & XWS_STREAM_CLOSE_SENT) != 0,
		"WebSocket Close sent flag missing"
	);
	testRequire(
		(pClose->Flags & XWS_STREAM_CLOSE_RECEIVED) != 0,
...
/* 初始化客户端或服务端的统一已建立会话。 */
static xwsstream* testWsConnAttach(
	xnetstream* pStream,
	test_ws_endpoint* pEndpoint
)
{
	uint8 ConfigStorage[sizeof(xwsstreamconfig) + 2u];
	uint8 EventsStorage[sizeof(xwsstreamevents) + 2u];
	...
	pConnection = xrtWsStreamAttach(
		pStream,
		0,
		(const xwsstreamconfig*)(const void*)(ConfigStorage + 1u),
		(const xwsstreamevents*)(const void*)(EventsStorage + 1u),
		pEndpoint
	);
	if ( pConnection != NULL ) {
		testRequire(
...
)
{
	...
	while ( iHeld < (sizeof(Held) / sizeof(Held[0])) ) {
		Held[iHeld] = xrtMalloc(sizeof(xwsstream));
		if ( Held[iHeld] == NULL ) {
			break;
		}
		iHeld++;
...
static void testWsConnRejectConfigs(xnetstream* pStream)
{
	...
	testRequire(
		...
		(xrtErrorCode(xrtGetError()) == XWS_STREAM_ERROR_ARGUMENT) &&
		(xrtNetStreamState(pStream) == XNET_STREAM_OPEN),
	...
	testRequire(
		(xrtWsStreamAttach(
			...
			0,
			&Config,
			(const xwsstreamevents*)(uintptr_t)(UINTPTR_MAX - 1u),
			NULL
		 ) == NULL) &&
		(xrtErrorCode(xrtGetError()) == XWS_STREAM_ERROR_ARGUMENT) &&
		(xrtNetStreamState(pStream) == XNET_STREAM_OPEN),
	...
	iAvailable = xrtNetStreamAvailable(pStream);
	testRequire(
		(iAvailable < SIZE_MAX) &&
		(xrtWsStreamAttach(
			pStream,
		...
		(xrtErrorCode(xrtGetError()) == XWS_STREAM_ERROR_STATE) &&
		(xrtNetStreamState(pStream) == XNET_STREAM_OPEN) &&
		...
		"WebSocket accepted or partially consumed an invalid prefix"
	);
	xrtWsStreamConfigInit(&Config);
	Config.Role = (xwsrole)99;
	testRequire(
		(xrtWsStreamAttach(pStream, 0, &Config, NULL, NULL) == NULL) &&
...
)
{
	...
	testRequire(
		(xrtWsStreamState(pConnection) == XWS_STREAM_OPEN) &&
		(xrtWsStreamPending(pConnection) == iPending) &&
...
static void testWsConnControlReserve(xwsstream* pConnection)
{
	xnetstream* pStream = xrtWsStreamTcp(pConnection);
...
)
{
	...
	size_t iPending = xrtWsStreamPending(pConnection);
	size_t iWritable = xrtWsStreamWritable(pConnection);
	xnetresult Result;
	...
	testRequire(
		(xrtWsStreamState(pConnection) == XWS_STREAM_OPEN) &&
		(xrtWsStreamPending(pConnection) == iPending) &&
		(xrtWsStreamWritable(pConnection) == iWritable),
...
static void testWsConnRejectOperations(xwsstream* pConnection)
{
...
)
{
	...
	testRequire(
		(xrtWsStreamBinaryRef(
			(xwsstream*)(uintptr_t)(UINTPTR_MAX - 1u),
			&Invalid
		 ) == XNET_RESULT_ERROR) &&
		(xrtWsStreamBinaryRef(pConnection, NULL) ==
		 XNET_RESULT_ERROR) &&
		(xrtWsStreamBinaryRef(
			pConnection,
	...
	Invalid.Size = 4u;
	testRequire(
		xrtWsStreamBinaryRef(pConnection, &Invalid) ==
			XNET_RESULT_ERROR,
	...
	Result = xrtWsStreamTextRef(pConnection, &Ref);
	testRequire(
		(Result == XNET_RESULT_OK) &&
		(xrtAtomic32Load(
			&pTest->ClientRefRelease,
...
)
{
	...
	while ( iExhausted <
		TEST_WS_CONNECTION_OOM_BLOCKS ) {
		ptr pBlock = __xrtNetWorkerNodeAlloc(
			pWorker,
...
)
{
	...
	testRequire(
		(xrtWsStreamRole(pConnection) == XWS_ROLE_CLIENT) &&
...
)
{
	...
	xrtClearError();
	pWriter = xrtWsStreamBeginText(pConnection);
	xrtAtomic32Store(
		&pTest->FailAlloc,
		0,
		XMEMORY_RELEASE
	...
	xrtClearError();
	testRequire(
		(xrtWsStreamBeginBinary(pConnection) == NULL) &&
	...
	pWriter = xrtWsStreamBeginBinary(pConnection);
	testRequire(
		pWriter != NULL,
	...
	xrtWsWriterDestroy(pWriter);
		...
		xrtClearError();
		pWriter = xrtWsStreamBeginTextCompressed(pConnection);
		testRequire(
			pWriter != NULL,
		...
		pWriter = xrtWsStreamBeginBinaryCompressed(pConnection);
		testRequire(
			pWriter != NULL,
...
)
{
	...
	xwswriter* pWriter = xrtWsStreamBeginBinary(pConnection);
...
)
{
	...
	(void)pWorker;
		...
		{
			xwswriter* pWriter =
				xrtWsStreamBeginBinary(pConnection);
...
{
	test_ws_connection Test;
	uint8 CloseStorage[sizeof(xwsstreamclose) + 2u];
	...
	testRequire(
		(xrtWsStreamRole(pClient) == XWS_ROLE_CLIENT) &&
		(xrtWsStreamRole(pServer) == XWS_ROLE_SERVER) &&
		(xrtWsStreamProtocol(pClient).Size == 7) &&
		(memcmp(
			xrtWsStreamProtocol(pClient).Data,
			"chat.v1",
			7
		 ) == 0) &&
		(xrtWsStreamProtocol(pServer).Size == 7) &&
		(memcmp(
			xrtWsStreamProtocol(pServer).Data,
			"chat.v1",
			7
		 ) == 0) &&
	...
	xrtClearError();
	pClientTransport = xrtWsStreamTcpRef(pClient);
	pServerTransport = xrtWsStreamTcpRef(pServer);
	...
	testRequire(
		...
		xrtWsStreamCloseInfo(
			pClient,
			(xwsstreamclose*)(void*)(CloseStorage + 1u)
		) &&
		(CloseStorage[0] == 0xA5) &&
	...
	testRequire(
		!xrtWsStreamCloseInfo(
			pClient,
			(xwsstreamclose*)(uintptr_t)(UINTPTR_MAX - 1u)
		) &&
		(xrtErrorCode(xrtGetError()) == XWS_STREAM_ERROR_ARGUMENT) &&
		!xrtWsStreamCloseInfo(
			pClient,
			(xwsstreamclose*)(void*)pClient
		) &&
		(xrtWsStreamState(pClient) == XWS_STREAM_OPEN) &&
		(xrtWsStreamProtocol(pClient).Size == 7u),
	...
	testWsConnRejectCall(
	...
	#if defined(XRT_FEATURE_WEBSOCKET_STREAM_REF)
		{
			...
			testWsConnRejectCall(
				pClient,
				xrtWsStreamBinaryRef(pClient, &Ref),
				XWS_STREAM_ERROR_STATE,
		...
		xrtClearError();
		testRequire(
			(xrtWsStreamBeginText(pClient) == NULL) &&
			(xrtErrorCode(xrtGetError()) ==
			 XWS_STREAM_ERROR_STATE),
	...
	xrtWsStreamPause(pClient);
	testRequire(
		xrtWsStreamPaused(pClient),
	...
	testRequire(
		xrtWsStreamCloseInfo(pClient, &OpenClose) &&
		((OpenClose.Flags & XWS_STREAM_CLOSE_CLEAN) != 0) &&
		(OpenClose.LocalCode ==
	...
	testRequire(
		!xrtWsStreamAbort(pClient) &&
		!xrtWsStreamAbort(pServer) &&
		(xrtWsStreamState(pClient) == XWS_STREAM_CLOSED) &&
		(xrtWsStreamState(pServer) == XWS_STREAM_CLOSED),
