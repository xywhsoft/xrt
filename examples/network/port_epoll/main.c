...
/*
 * 范例：network/port_epoll —— Linux epoll 事件端口
 * ----------------------------------------------------------------
 ...
 *       examples/network/port_epoll/main.c -lws2_32 -liphlpapi
 * 预期输出：（Linux 平台输出观察结果）
 */


/* 展示 Linux epoll 的 one-shot 可读观察和显式重新武装模型。 */
int main(void)
{
	...
	Cleanup:
		...
		return iResult;
	#else
		puts("epoll is available on Linux only.");
		return 0;
	#endif
}
